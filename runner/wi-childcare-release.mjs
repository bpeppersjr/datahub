...
import { replayWiChildcareAcquisition } from "./wi-childcare-acquisition.mjs";
import { normalizeWiChildcareAcquisition, WI_NORMALIZATION_VERSION } from "./wi-childcare-normalization.mjs";
import { WI_LAYER, WI_WHERE } from "./wi-childcare-preflight.mjs";

const DATASET = "wi-dhs-licensed-group-childcare";
const POLICY_SHA256 = "cd62a0a4b83c84b5b5e7107dfd3281f87f67cc34f18d99e5dcacce665f1a328b";
const POLICY_PATH = path.join(APP_ROOT, "config/source-policies/wi-childcare-local-review.json");
...
function requireValue(ok, label) { if (!ok) throw new Error(`Wisconsin childcare release rejected: ${label}.`); }
...
async function outputLocation(target, signal) {
  ...
  for (let ancestor = absolute; ancestor !== APP_ROOT; ancestor = path.dirname(ancestor)) {
    ...
    throw new Error("Wisconsin output cannot be inside an existing manifest-bearing bundle.");
  }
  return absolute;
}
export async function readWiChildcareEvidence(filename, options = {}) {
  strictOptions(options, ["signal"]);
  const bytes = await boundedRead(filename, 150_000_000, options.signal);
  try { return JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(bytes)); }
  catch { throw new Error("Wisconsin acquisition evidence must be valid bounded UTF-8 JSON."); }
}
...
async function policyFor(signal) {
  ...
  return { profile: "wi-childcare-local-review@1.0.0", configuration_sha256: POLICY_SHA256, ...policy };
}
...
async function derive(evidence, runId, processedAt, signal) {
  ...
  const acquired = replayWiChildcareAcquisition(evidence, { signal });
  const sourceReleaseId = `wi-childcare-${sha(json({ layer_url: WI_LAYER, where: WI_WHERE,
    metadata: evidence.preflight_before.observations.slice(0, 9).map(({ kind, payload }) => ({ kind, payload })),
    features: acquired.features.map(feature => sha(json(feature))) }))}`;
  const result = normalizeWiChildcareAcquisition(evidence, { runId, sourceReleaseId, processedAt }, { signal });
  const xml = Buffer.from(evidence.preflight_before.observations.find(o => o.kind === "xml").payload.base64, "base64");
  return { acquired, normalized: result.records, quarantine: result.quarantine, sourceReleaseId, xml, quality: result.summary,
...
function manifestFor(runId, result, artifacts, policy, processedAt) {
  return { schema_version: "1.0.0", dataset_id: DATASET, connector_id: DATASET, connector_version: "1.0.0", transformation_version: WI_NORMALIZATION_VERSION,
    run_id: runId, release_id: `wi-childcare-${runId}`, source_release_id: result.sourceReleaseId, status: "offline-review-only", observed_at: result.acquired.evidence.observed_at, processed_at: processedAt,
    source_url: WI_LAYER, source_filter: WI_WHERE, policy, counts: result.counts, quality: result.quality, quarantine_max_fraction: null,
    ...
    scope: "Wisconsin publisher-listed LICENSED GROUP rows only; other categories excluded, operation and license dates unverified",
...
export async function verifyWiChildcareRelease(manifestPath, options = {}) {
  ...
  requireValue(uuid.test(manifest.run_id) && manifest.release_id === `wi-childcare-${manifest.run_id}`, "release identity");
...
export async function buildWiChildcareRelease(options = {}) {
  ...
  signal?.throwIfAborted();
  const snapshot = structuredClone(evidence), acquired = replayWiChildcareAcquisition(snapshot, { signal });
  ...
  const staging = path.join(stagingRoot, runId), release = path.join(releasesRoot, `wi-childcare-${runId}`), pointerTemp = path.join(root, `.current-${runId}.tmp`);
  ...
  function inspection() { return Object.assign(new Error("Wisconsin childcare publication ownership changed; inspection required. Foreign paths and prior releases preserved."), { code: "WI_CHILDCARE_INSPECTION_REQUIRED" }); }
  ...
  try {
    ...
    const selected = await lines(acquired.features, signal), observation = `${json(acquired.evidence)}\n`, xml = Buffer.from(acquired.evidence.preflight_before.observations.find(o => o.kind === "xml").payload.base64, "base64");
    ...
    await logger("verify"); await verifyWiChildcareRelease(path.join(staging, "manifest.json"), { signal });
    ...
    try {
      ...
      requireValue(json(Object.keys(prior).sort()) === json(["dataset_id", "release_id", "manifest", "manifest_sha256"].sort())
        && prior.dataset_id === DATASET && typeof prior.release_id === "string" && uuid.test(prior.release_id.slice("wi-childcare-".length))
        && prior.release_id.startsWith("wi-childcare-") && prior.manifest === `releases/${prior.release_id}/manifest.json`
    ...
    const verification = await verifyWiChildcareRelease(path.join(staging, "manifest.json"), { signal });
