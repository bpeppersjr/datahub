...
import { acquirePaChildcareWithTransport, acquirePaChildcareNative, replayPaChildcareAcquisition, assertPaChildcareAcquisitionConfiguration } from './pa-childcare-acquisition.mjs';

const VERSION='pa-childcare-acquired-release@1.0.0';
const UUID=/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/;
const MAXIMUM=200_000_000;
const bytes=value=>Buffer.from(JSON.stringify(value)+'\n');
const hash=value=>createHash('sha256').update(value).digest('hex');
const check=value=>{if(!value)throw Error('Pennsylvania acquired evidence rejected.');};
...
const claims=()=>({native_execution_independently_verified:false,source_authenticity_verified:false,public_export_authorized:false,national_reporting_integrated:false,app_enrolled:false,scheduled:false});
const inspection=()=>Object.assign(Error('Pennsylvania acquisition publication may exist; preserve evidence and inspect before retry.'),{code:'PA_CHILDCARE_PUBLICATION_INCOMPLETE'});
async function derive(prerequisite,evidence,journal,signal){
  const replayed=await replayPaChildcareAcquisition(evidence,{signal});
...
function manifestFor(runId,mode,replayed,artifacts){return {schema_version:VERSION,run_id:runId,status:'acquired-evidence-only',execution_mode:mode,source:replayed.source,record_count:replayed.features.length,claims:claims(),artifacts};}

export async function readPaChildcareAcquiredEvidence(manifestPath,options={}){
...
export async function buildPaChildcareAcquiredReleaseWithTransport(options={}){
...
export async function buildPaChildcareAcquiredRelease(options={}){
  optionsOnly(options,['outputRoot','signal']);return build(options,'fixed-native-fetch');
}
async function build(options,mode){
  const {signal,now=()=>new Date(),outputRoot=path.join(APP_ROOT,'data/business-sources/pa-childcare-centers/acquired')}=options;
  check(typeof now==='function'&&typeof outputRoot==='string'&&outputRoot===path.resolve(outputRoot));signal?.throwIfAborted();
  await assertPaChildcareAcquisitionConfiguration(signal);
  ...
  try{
    ...
    const evidence=await(mode==='fixed-native-fetch'?acquirePaChildcareNative(callbacks):acquirePaChildcareWithTransport(callbacks));
    ...
    const {verification}=await readPaChildcareAcquiredEvidence(path.join(directory,'manifest.json'));
