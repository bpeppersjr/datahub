...
import { acquireCoChildcareWithTransport, acquireCoChildcareNative, replayCoChildcareAcquisition, assertCoChildcareAcquisitionConfiguration } from './co-childcare-acquisition.mjs';

const VERSION='co-childcare-acquired-release@1.0.0';
const UUID=/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/;
const MAXIMUM=200_000_000;
const bytes=value=>Buffer.from(JSON.stringify(value)+'\n');
const hash=value=>createHash('sha256').update(value).digest('hex');
const check=value=>{if(!value)throw Error('Colorado acquired evidence rejected.');};
...
const inspection=()=>Object.assign(Error('Colorado acquisition publication may exist; preserve evidence and inspect before retry.'),{code:'CO_CHILDCARE_PUBLICATION_INCOMPLETE'});
async function derive(prerequisite,evidence,journal,signal){
  const replayed=await replayCoChildcareAcquisition(evidence,{signal});
...
export async function readCoChildcareAcquiredEvidence(manifestPath,options={}){
  optionsOnly(options,['signal']);const {signal}=options;signal?.throwIfAborted();
  await assertCoChildcareAcquisitionConfiguration(signal);
...
export const verifyCoChildcareAcquiredEvidence=readCoChildcareAcquiredEvidence;

export async function buildCoChildcareAcquiredReleaseWithTransport(options={}){
...
export async function buildCoChildcareAcquiredRelease(options={}){
  optionsOnly(options,['outputRoot','signal']);return build(options,'fixed-native-fetch');
}
async function build(options,mode){
  const {signal,now=()=>new Date(),outputRoot=path.join(APP_ROOT,'data/business-sources/co-childcare/acquired')}=options;
  ...
  try{
    ...
    const evidence=await(mode==='fixed-native-fetch'?acquireCoChildcareNative(callbacks):acquireCoChildcareWithTransport(callbacks));
    ...
    const {verification}=await readCoChildcareAcquiredEvidence(path.join(directory,'manifest.json'));
