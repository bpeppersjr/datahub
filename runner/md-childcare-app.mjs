...
import {assertMdChildcareAcquisitionConfiguration} from './md-childcare-acquisition.mjs';
import {buildMdChildcareAcquiredRelease,buildMdChildcareAcquiredReleaseWithTransport,readMdChildcareAcquiredEvidence} from './md-childcare-acquired-release.mjs';
import {buildMdChildcareNormalizedRelease,readMdChildcareNormalizedRelease} from './md-childcare-normalized-release.mjs';

export const MD_CHILDCARE_APP_VERSION='md-childcare-app@1.0.0';
export const MD_CHILDCARE_APP_CONTRACT_SHA256='860005003d77805e1bf65d1e6e315ed77f3dc6e1b858c1f930967a01e5deba83';
const NORMALIZATION='aff3e92723456753b1790aed2155603798ea7a61bba69650df08ca5b8136fe52';
const UUID=/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/;
const hash=v=>createHash('sha256').update(v).digest('hex');
const encode=v=>Buffer.from(JSON.stringify(v)+'\n');
const check=v=>{if(!v)throw Object.assign(Error('Maryland app evidence rejected.'),{code:'MD_CHILDCARE_APP_REJECTED'});};
const time=v=>typeof v==='string'&&Number.isFinite(Date.parse(v))&&new Date(v).toISOString()===v;
const claims=()=>({export_policy:'internal',public_export_authorized:false,national_reporting_integrated:false,native_execution_independently_verified:false,source_authenticity_verified:false,scheduled:false,physical_site_verified:false,identity_matching_eligible:false,address_role:'reported-address-unverified',source_key_stability_verified:false,credential_deduplication_applied:false,geocodes_inferred:false,current_operations_verified:false,publisher_cohort_date:'2026-02-13',coordinate_basis:'provider-transformed-epsg4326-point-not-address-verified'});
const incomplete=()=>Object.assign(Error('Maryland app output requires inspection; preserve retained evidence before retry.'),{code:'MD_CHILDCARE_APP_INCOMPLETE'});
...
async function config(signal){const acquisition=await assertMdChildcareAcquisitionConfiguration(signal);check(hash(JSON.stringify(await readJson(path.join(APP_ROOT,'config/connectors/md-childcare-centers-normalization.json'),100000,signal)))===NORMALIZATION);check(hash(JSON.stringify(await readJson(path.join(APP_ROOT,'config/connectors/md-childcare-centers-app.json'),100000,signal)))===MD_CHILDCARE_APP_CONTRACT_SHA256);return {app_sha256:MD_CHILDCARE_APP_CONTRACT_SHA256,normalization_sha256:NORMALIZATION,...acquisition};}
...
async function inspect(receiptPath,signal,candidate=false){
  ...
  const s=start.value;check(s.schema_version===MD_CHILDCARE_APP_VERSION&&s.run_id===id&&same(s.configuration,configuration)&&time(s.started_at)&&['fixed-native-fetch','injected-test-transport','retained-local-verification'].includes(s.execution_mode));
  ...
  const a=await readMdChildcareAcquiredEvidence(acq.value.verification.manifest_path,{signal}),n=await readMdChildcareNormalizedRelease(norm.value.verification.manifest_path,{signal});
  ...
  check(same((await readMdChildcareAcquiredEvidence(a.verification.manifest_path,{signal})).verification,a.verification)&&same((await readMdChildcareNormalizedRelease(n.verification.manifest_path,{signal})).verification,n.verification));
...
export async function verifyMdChildcareAppJob(receiptPath,options={}){opts(options,['signal']);return inspect(receiptPath,options.signal);}
export async function runMdChildcareAppJob(options={}){opts(options,['outputRoot','signal','industryRunId','acquiredManifestPath']);return run(options,options.acquiredManifestPath===undefined?'fixed-native-fetch':'retained-local-verification');}
export async function runMdChildcareAppJobWithTransport(options={}){opts(options,['outputRoot','signal','industryRunId','fetchImpl','now','logger']);check(typeof options.fetchImpl==='function'&&(options.now===undefined||typeof options.now==='function')&&(options.logger===undefined||typeof options.logger==='function'));return run(options,'injected-test-transport');}
async function run(options,mode){
  ...
  const expire=()=>controller.abort(Object.assign(Error('Maryland app deadline exceeded.'),{code:'MD_CHILDCARE_APP_DEADLINE'}));const timer=setTimeout(expire,1_800_000),signal=controller.signal;
  ...
  try{
    ...
    const retained=mode==='retained-local-verification'?await readMdChildcareAcquiredEvidence(options.acquiredManifestPath,{signal}):null;
    const root=options.outputRoot??path.join(APP_ROOT,'data/business-sources/md-childcare/app');check(typeof root==='string'&&root===path.resolve(root)&&!path.relative(APP_ROOT,root).split(path.sep).some(p=>p.toLowerCase()==='jobs'));await canonical(root,{output:true,signal});await rejectNestedAppWork(root);await canonical(root,{output:true,create:true,signal});
    ...
    start={schema_version:MD_CHILDCARE_APP_VERSION,run_id:id,industry_run_id:industry,execution_mode:mode,started_at:new Date().toISOString(),configuration,retained_acquired:retained?.verification??null};await assertOwned();startHash=await write(path.join(directory,'start.json'),start);
    const work=path.join(root,'runs',id);checkpoint();
    if(mode==='fixed-native-fetch'){
      const publisherRoot=path.join(APP_ROOT,'data/business-sources/md-childcare/runtime');await canonical(publisherRoot,{create:true,output:true,signal});publisherPath=path.join(publisherRoot,'publisher.lock');
      try{publisher=await open(publisherPath,'wx');}catch(error){if(error.code==='EEXIST')throw Object.assign(Error('Maryland publisher ownership requires inspection.'),{code:'MD_CHILDCARE_PUBLISHER_BUSY'});throw error;}publisherIdentity=await publisher.stat({bigint:true});publisherValue={run_id:id,pid:process.pid};check(publisherIdentity.isFile()&&publisherIdentity.nlink===1n);await publisher.writeFile(encode(publisherValue));await publisher.sync();checkpoint();
    }
    acquired=retained?.verification??await (mode==='fixed-native-fetch'?buildMdChildcareAcquiredRelease({outputRoot:path.join(work,'acquired'),signal}):buildMdChildcareAcquiredReleaseWithTransport({outputRoot:path.join(work,'acquired'),signal,fetchImpl:options.fetchImpl,...(options.now?{now:options.now}:{})}));
    ...
    normalized=await buildMdChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:path.join(work,'normalized'),signal});await assertOwned();await write(path.join(directory,'normalized.json'),{retained_at:new Date().toISOString(),verification:normalized});if(options.logger)await options.logger({phase:'normalized-checkpoint'});checkpoint();
    ...
    const codes=['MD_CHILDCARE_PUBLICATION_INCOMPLETE','MD_CHILDCARE_NORMALIZATION_INCOMPLETE','MD_CHILDCARE_APP_DEADLINE','MD_CHILDCARE_APP_REJECTED','MD_CHILDCARE_PUBLISHER_BUSY','MD_CHILDCARE_DEFERRED','MD_CHILDCARE_PREFLIGHT_FAILED','MD_CHILDCARE_ACQUISITION_FAILED'];const code=codes.includes(errorCode)?errorCode:signal.aborted?'MD_CHILDCARE_APP_CANCELLED':'MD_CHILDCARE_APP_FAILED';
    ...
    throw Object.assign(Error('Maryland app did not complete; preserve and inspect retained evidence before retry.'),{code});
  }finally{
