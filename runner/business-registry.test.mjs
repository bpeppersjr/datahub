...
import {
  ...
  reconcileIlBusinessOrganization,
...
import { normalizeIllinoisOrganization } from "./il-business-registry.mjs";
...
function normalizedAkBusinessRecord(licenseNumber = "1001", overrides = {}) {
  const source = {
    license_number: licenseNumber,
    business_name: `FIXTURE ALASKA ORGANIZATION ${licenseNumber}`,
    status: "Active",
    issue_date: "1/15/2020 9:46:14 AM",
  ...
  return normalizeAkBusinessLicense(source, [{
    license_number: licenseNumber,
    ...
    runId: "ak-business-source-fixture",
    retrievedAt: "2026-09-01T12:00:00.000Z",
    sourceObservedFrom: "2026-09-01T11:59:00.000Z",
    sourceObservedThrough: "2026-09-01T12:00:00.000Z",
    sourceReleaseId: "ak-business-source-fixture",
    baselineByZip: new Map([["99501", { geography: { status: "2020-zcta-polygon-available", geo_id: "zcta:99501", geoid: "99501" } }]]),
  });
}

async function writeFixtureAkBusinessRelease(root) {
  const releaseId = "ak-active-business-licenses-fixture";
  const releaseDirectory = path.join(root, "releases", releaseId);
  await mkdir(releaseDirectory, { recursive: true });
  const records = [
    normalizedAkBusinessRecord(),
    normalizedAkBusinessRecord("1002", { physical_country: "CANADA", physical_state: "BC", physical_zip: "V6B 1A1", physical_zip_plus: null }),
  ];
  const artifacts = [];
  for (const prefix of "0123456789abcdef") {
    const partitionRecords = records.filter((record) => sha256(record.external_identifiers[0].value)[0] === prefix);
    const buffer = gzipSync(partitionRecords.map((record) => JSON.stringify(record)).join("\n") + (partitionRecords.length ? "\n" : ""));
    const relativePath = `derived/licenses/id-hash-prefix=${prefix}.jsonl.gz`;
    const destination = path.join(releaseDirectory, relativePath);
    await mkdir(path.dirname(destination), { recursive: true });
    await writeFile(destination, buffer);
    artifacts.push({ path: relativePath, bytes: buffer.length, sha256: sha256(buffer), record_count: partitionRecords.length, artifact_type: "normalized-ak-active-business-license-jsonl-gzip", export_policy: "local-review-only" });
  }
  const zipRows = [{
    zip_code: "99501",
    ak_active_business_license_snapshot: { active_license_organization_reported_address_count: 1, provisional_physical_site_count: 1 },
    current_usps_validity: { status: "unverified" },
    geography: { status: "2020-zcta-polygon-available", geo_id: "zcta:99501" },
    employer_baseline: { status: "published", establishments: 100 },
    baseline_coverage_status: "zbp-and-zcta",
  }];
  const zipBuffer = Buffer.from(`${zipRows.map((row) => JSON.stringify(row)).join("\n")}\n`);
  await mkdir(path.join(releaseDirectory, "derived"), { recursive: true });
  await writeFile(path.join(releaseDirectory, "derived/zip-coverage.jsonl"), zipBuffer);
  artifacts.push({ path: "derived/zip-coverage.jsonl", bytes: zipBuffer.length, sha256: sha256(zipBuffer), record_count: zipRows.length, artifact_type: "ak-active-business-license-zip-coverage-jsonl", distribution_policy: "local-aggregate-review-required" });
  const manifest = {
    schema_version: "1.0.0",
    ...
    source_release_id: "ak-business-source-fixture",
    source_observation_window: { from: "2026-09-01T11:59:00.000Z", through: "2026-09-01T12:00:00.000Z" },
    policy: { record_level_distribution: "local-review-only", aggregate_distribution: "local-aggregate-review-required" },
    coverage: {
      source_active_license_rows: 3,
      active_license_organizations: 2,
      provisional_physical_sites: 1,
      provisional_establishments: 1,
      organizations_without_eligible_physical_site: 1,
      reported_us_address_zip_contributions: 1,
      quarantined_source_records: 1,
      source_naics_rows: 2,
      distinct_license_naics_pairs: 2,
      accepted_license_naics_pairs: 2,
      duplicate_license_naics_rows_collapsed: 0,
    },
    dependencies: [],
    artifacts,
  };
...
function normalizedIlBusinessRecord(entityKind = "llc", fileNumber = entityKind === "corporation" ? "12345678" : "87654321") {
  const isCorporation = entityKind === "corporation";
  const master = isCorporation ? {
    ...
    organized_date: "2020-02-03",
    dissolution_date: null,
    management_type: "M",
    jurisdiction_organized_code: "IL",
    records_office_street: "100 CAPITOL AVE",
    records_office_city: "SPRINGFIELD",
    records_office_zip: "62701-4321",
    records_office_jurisdiction: "IL",
    source_flags: null,
  };
  return normalizeIllinoisOrganization({
    entityKind,
    master,
    name: { file_number: fileNumber, name: isCorporation ? "FIXTURE ILLINOIS CORPORATION" : "FIXTURE ILLINOIS LLC" },
    annual: isCorporation ? {
      file_number: fileNumber,
      current_report_run_date: "2026-07-01",
      current_report_paid_date: "2026-07-02",
    } : null,
  }, {
    runId: "il-business-source-fixture",
    retrievedAt: "2026-09-02T16:00:00.000Z",
    sourceRunDate: "2026-09-02",
    sourceReleaseId: "il-business-source-fixture",
    baselineByZip: new Map([["62701", { geography: { status: "2020-zcta-polygon-available", geo_id: "zcta:62701", geoid: "62701" } }]]),
  });
}

async function writeFixtureIlBusinessRelease(root) {
  const releaseId = "il-business-registry-fixture";
  const releaseDirectory = path.join(root, "releases", releaseId);
  await mkdir(releaseDirectory, { recursive: true });
  const records = [normalizedIlBusinessRecord("corporation"), normalizedIlBusinessRecord("llc")];
  const artifacts = [];
  for (const prefix of "0123456789abcdef") {
    const partitionRecords = records.filter((record) => {
      const identifier = record.external_identifiers[0];
      return sha256(`${identifier.entity_kind}:${identifier.value}`)[0] === prefix;
    });
    const buffer = gzipSync(partitionRecords.map((record) => JSON.stringify(record)).join("\n") + (partitionRecords.length ? "\n" : ""));
    const relativePath = `derived/organizations/id-hash-prefix=${prefix}.jsonl.gz`;
    const destination = path.join(releaseDirectory, relativePath);
    await mkdir(path.dirname(destination), { recursive: true });
    await writeFile(destination, buffer);
    artifacts.push({ path: relativePath, bytes: buffer.length, sha256: sha256(buffer), record_count: partitionRecords.length, artifact_type: "normalized-il-business-organization-jsonl-gzip", export_policy: "local-review-only" });
  }
  const zipRows = [{
    zip_code: "62701",
    current_usps_validity: { status: "unverified", reason: "Fixture has no governed current USPS operational ZIP dependency." },
    geography: { status: "2020-zcta-polygon-available", geo_id: "zcta:62701", geoid: "62701", geometry_file: "source/zctas/prefix=6.geojson" },
    census_zbp_coverage_status: "zbp-and-zcta",
    il_business_registry_active_snapshot: {
      status: "published-eligible-llc-records-office-addresses",
      organization_records_office_address_count: 1,
      source_release_id: "il-business-source-fixture",
      source_run_date: "2026-09-02",
  ...
  await mkdir(path.join(releaseDirectory, "derived"), { recursive: true });
  await writeFile(path.join(releaseDirectory, "derived/zip-coverage.jsonl"), zipBuffer);
  artifacts.push({ path: "derived/zip-coverage.jsonl", bytes: zipBuffer.length, sha256: sha256(zipBuffer), record_count: zipRows.length, artifact_type: "il-business-registry-zip-coverage-jsonl", export_policy: "local-review-only" });
  const manifest = {
    schema_version: "1.0.0",
    ...
    coverage: {
      source_records: { corporation_master: 1, corporation_name: 1, corporation_annual: 1, llc_master: 1, llc_name: 1 },
      active_organizations_published: records.length,
      active_corporations_published: 1,
      active_llcs_published: 1,
      eligible_llc_records_office_addresses: 1,
      source_zip_codes: 1,
      zip_union_records: zipRows.length,
      possible_corporation_ngs_month_rule_not_evaluated: 0,
      physical_sites: null,
      establishments: null,
    },
    dependencies: [],
...
function normalizedDcBasicBusinessLicenseSite(customerNumber = "500526000983", zipCode = "20001") {
  const base = {
    ...
    PREMISEADDRESS: `100 TEST ST NW, STE 200, Washington, DC, ${zipCode}, USA`,
    PREMISEINDC: "Yes",
    ...
    DATAREFRESHEDON: Date.parse("2026-09-01T04:00:00.000Z"),
    WARD: "Ward 2",
    ANC: "ANC 2C",
    SMD: "2C01",
    DISTRICT: "FIRST",
  ...
  return normalizeDcBasicBusinessLicenseSite([
    { ...base, BUSINESSACTIVITY: "Grocery Store", PRIMARYACTIVITYFLAG: "Yes", GLOBALID: "{11111111-1111-1111-1111-111111111111}", OBJECTID: 1001 },
    { ...base, BUSINESSACTIVITY: "Delicatessen", PRIMARYACTIVITYFLAG: "No", GLOBALID: "{22222222-2222-2222-2222-222222222222}", OBJECTID: 1002 },
  ], {
    runId: "dc-license-source-fixture",
    retrievedAt: "2026-09-01T12:00:00.000Z",
    sourceRefreshedAt: "2026-09-01T04:00:00.000Z",
    sourceReleaseId: "dc-license-source-fixture",
    baselineByZip: new Map([[zipCode, { postal_label: { preferred_state: "DC" }, geography: { status: "2020-zcta-polygon-available", geo_id: `zcta:${zipCode}`, geoid: zipCode } }]]),
  });
}
...
async function writeFixtureDcBasicBusinessLicenseRelease(root) {
  const releaseId = "dc-basic-business-license-fixture";
  const releaseDirectory = path.join(root, "releases", releaseId);
  await mkdir(releaseDirectory, { recursive: true });
  const records = [normalizedDcBasicBusinessLicenseSite()];
  const artifacts = [];
  for (const prefix of "0123456789abcdef") {
    const partitionRecords = records.filter((record) => sha256(record.normalized_record_id)[0] === prefix);
    const buffer = gzipSync(partitionRecords.map((record) => JSON.stringify(record)).join("\n") + (partitionRecords.length ? "\n" : ""));
    const relativePath = `normalized/sites/prefix=${prefix}.jsonl.gz`;
    const destination = path.join(releaseDirectory, relativePath);
    await mkdir(path.dirname(destination), { recursive: true });
    await writeFile(destination, buffer);
    artifacts.push({ path: relativePath, bytes: buffer.length, sha256: sha256(buffer), record_count: partitionRecords.length, artifact_type: "normalized-dc-basic-business-license-site-jsonl-gzip", export_policy: "local-review-only" });
  }
  const zipRows = [
    { zip_code: "20001", count: 1 },
    { zip_code: "99999", count: 0 },
  ].map(({ zip_code: zip, count }) => ({
    zip_code: zip,
    dc_basic_business_license_snapshot: { licensed_site_count: count },
    current_usps_validity: { status: "unverified" },
    geography: { status: "2020-zcta-polygon-available", geo_id: `zcta:${zip}` },
    employer_baseline: { status: "published", establishments: 1000 },
    baseline_coverage_status: "zbp-and-zcta",
  ...
  await mkdir(path.join(releaseDirectory, "derived"), { recursive: true });
  await writeFile(path.join(releaseDirectory, "derived/zip-coverage.jsonl"), zipBuffer);
  artifacts.push({ path: "derived/zip-coverage.jsonl", bytes: zipBuffer.length, sha256: sha256(zipBuffer), record_count: zipRows.length, artifact_type: "dc-basic-business-license-zip-coverage-jsonl", distribution_policy: "public-aggregate-with-cc-by-4.0-attribution-and-source-limitations" });
  const manifest = {
    schema_version: "1.0.0",
    ...
    source_release_id: "dc-license-source-fixture",
    source: { data_refreshed_on: "2026-09-01T04:00:00.000Z" },
    coverage: {
      source_active_business_license_rows: 2,
      source_customer_groups: 1,
      accepted_active_business_license_rows: 2,
      normalized_licensed_sites: 1,
      organizations: 1,
      physical_sites: 1,
      establishments: 1,
      quarantined_source_records: 0,
      quarantined_customer_groups: 0,
      source_geocoded_sites: 1,
...
test("reconciles Alaska active-license evidence with conditional provisional site semantics", () => {
  const located = reconcileAkActiveBusinessLicense(normalizedAkBusinessRecord());
  assert.deepEqual(located.entities.map((entity) => entity.entity_type), ["organization", "physical_site", "establishment"]);
  assert.deepEqual(located.relationships.map((item) => item.relationship_type), ["operates", "located_at"]);
  assert.equal(located.zipCode, "99501");
  assert(located.organizationAssertions.some((item) => item.predicate === "organization.ak-active-business-license"));
  assert(located.locationAssertions.some((item) => item.predicate === "establishment.source-naics"));
  assert([...located.organizationAssertions, ...located.locationAssertions].every((item) => item.export_policy === "local-review-only"));
  const foreign = reconcileAkActiveBusinessLicense(normalizedAkBusinessRecord("1002", { physical_country: "CANADA", physical_state: "BC", physical_zip: "V6B 1A1", physical_zip_plus: null }));
  assert.deepEqual(foreign.entities.map((entity) => entity.entity_type), ["organization"]);
  assert.equal(foreign.relationships.length, 0);
...
test("reconciles Illinois daily-file registrations as organization-only evidence with split postal fields", () => {
  const normalizedLlc = normalizedIlBusinessRecord("llc");
  assert.deepEqual({
    zip_code: normalizedLlc.records_office_address.zip_code,
    postal_code: normalizedLlc.records_office_address.postal_code,
    zip4: normalizedLlc.records_office_address.zip4,
  }, { zip_code: "62701", postal_code: "62701", zip4: "4321" });
  const llc = reconcileIlBusinessOrganization(normalizedLlc);
  assert.equal(llc.entity.entity_id, "organization:il_sos_llc_87654321");
  assert.equal(llc.entity.entity_type, "organization");
  assert.equal(llc.zipCode, "62701");
  const address = llc.assertions.find((item) => item.predicate === "organization.records-office-address");
  assert.equal(address.value.postal_code, "62701");
  assert.equal(address.value.zip4, "4321");
  assert.equal(llc.assertions.some((item) => item.predicate.startsWith("site.") || item.predicate.startsWith("establishment.")), false);
  assert(llc.assertions.every((item) => item.export_policy === "local-review-only"));
  assert.equal(llc.relationships, undefined);

  const corporation = reconcileIlBusinessOrganization(normalizedIlBusinessRecord("corporation"));
  assert.equal(corporation.entity.entity_id, "organization:il_sos_corporation_12345678");
  assert.equal(corporation.zipCode, null);
  assert(corporation.assertions.some((item) => item.predicate === "organization.il-annual-report-evidence"));
  assert.equal(corporation.assertions.some((item) => item.predicate === "organization.records-office-address"), false);
});
...
test("reconciles grouped DC Basic Business License activities into one organization, site, and establishment", () => {
  const result = reconcileDcBasicBusinessLicenseSite(normalizedDcBasicBusinessLicenseSite());
  assert.deepEqual(result.entities.map((entity) => entity.entity_type), ["organization", "physical_site", "establishment"]);
  assert.deepEqual(result.relationships.map((item) => item.relationship_type), ["operates", "located_at"]);
  assert(result.organizationAssertions.some((item) => item.predicate === "organization.legal-name"));
  assert.equal(result.locationAssertions.filter((item) => item.predicate === "establishment.dc-active-basic-business-license-activity").length, 2);
  assert.equal(result.locationAssertions.filter((item) => item.predicate === "establishment.name").length, 1);
  assert([...result.organizationAssertions, ...result.locationAssertions].every((item) => item.export_policy === "local-review-only"));
...
test("publishes and verifies a combined partial registry while retaining denominator-only ZIPs", async (t) => {
  ...
  const akBusinessPointer = await writeFixtureAkBusinessRelease(path.join(root, "ak-business"));
  ...
  const ilBusinessPointer = await writeFixtureIlBusinessRelease(path.join(root, "il-business"));
  ...
  assert.equal(result.manifest.coverage.organizations, 26);
  ...
  assert.equal(result.manifest.coverage.ak_active_business_license_source_rows, 3);
  assert.equal(result.manifest.coverage.ak_active_business_license_organizations, 2);
  assert.equal(result.manifest.coverage.ak_active_business_license_provisional_physical_sites, 1);
  assert.equal(result.manifest.coverage.ak_active_business_license_quarantined_source_records, 1);
  ...
  assert.equal(result.manifest.coverage.il_business_registry_source_records, 5);
  assert.equal(result.manifest.coverage.il_business_registry_active_organization_records, 2);
  assert.equal(result.manifest.coverage.il_business_registry_active_corporation_records, 1);
  assert.equal(result.manifest.coverage.il_business_registry_active_llc_records, 1);
  assert.equal(result.manifest.coverage.il_business_registry_eligible_llc_records_office_addresses, 1);
  assert.equal(result.manifest.coverage.il_business_registry_possible_corporation_ngs_month_rule_not_evaluated, 0);
  ...
  assert.equal(result.manifest.coverage.dc_basic_business_license_source_rows, 2);
  ...
  const ilOrganizationArtifacts = result.manifest.artifacts.filter((artifact) => artifact.path.startsWith("entities/organizations/il-file-hash-prefix="));
  const ilAssertionArtifacts = result.manifest.artifacts.filter((artifact) => artifact.path.startsWith("assertions/organizations/il-file-hash-prefix="));
  assert.equal(ilOrganizationArtifacts.length, 16);
  assert.equal(ilOrganizationArtifacts.reduce((sum, artifact) => sum + artifact.record_count, 0), 2);
  assert.equal(ilAssertionArtifacts.length, 16);
  assert.equal(ilAssertionArtifacts.reduce((sum, artifact) => sum + artifact.record_count, 0), 12);
  const ilAssertions = [];
  for (const artifact of ilAssertionArtifacts) {
    const lines = gunzipSync(await readFile(path.join(result.releaseDirectory, artifact.path))).toString("utf8").trim();
    if (lines) ilAssertions.push(...lines.split("\n").map(JSON.parse));
  }
  const recordsOfficeAddress = ilAssertions.find((item) => item.predicate === "organization.records-office-address");
  assert.equal(recordsOfficeAddress.value.zip_code, "62701");
  assert.equal(recordsOfficeAddress.value.postal_code, "62701");
  assert.equal(recordsOfficeAddress.value.zip4, "4321");
  assert(ilAssertions.every((item) => item.export_policy === "local-review-only"));
  assert.equal(ilAssertions.some((item) => item.predicate.startsWith("site.") || item.predicate.startsWith("establishment.")), false);
  ...
  assert.equal(zipRows.find((row) => row.zip_code === "99501").registry_coverage.ak_active_business_license_organization_reported_address_count, 1);
  assert.equal(zipRows.find((row) => row.zip_code === "99501").registry_coverage.ak_active_business_license_provisional_physical_site_count, 1);
  ...
  const illinois = zipRows.find((row) => row.zip_code === "62701");
  assert.equal(illinois.postal_code, "62701");
  assert.equal(illinois.zip4, null);
  assert.equal(illinois.registry_coverage.il_business_registry_organization_records_office_address_count, 1);
  assert.equal(illinois.registry_coverage.physical_site_count, 0);
  assert.equal(illinois.registry_coverage.establishment_count, 0);
  assert.equal(illinois.source_contributions.il_business_registry_active_organizations.record_level_distribution, "local-review-only");
  assert.equal(illinois.source_contributions.il_business_registry_active_organizations.physical_site_inference_permitted, false);
  assert.equal(illinois.current_usps_validity.status, "not-listed-in-current-usps-area-district-file");
