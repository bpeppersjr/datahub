...
import {assertVtChildcareAcquisitionConfiguration} from './vt-childcare-acquisition.mjs';
...
export const VT_CHILDCARE_APP_VERSION='vt-childcare-app@1.0.0';
export const VT_CHILDCARE_APP_CONTRACT_SHA256='a8b16710b99c1d0ea0b3f6ae6ad88a2be645f86fad4ca5936fc620d6de33a52e';
const NORMALIZATION='0dc5abfd7adc46e1cc872ce1bbfed05b019b986699efb7b1367e48f6192bad8c';
const UUID=/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/;
const hash=v=>createHash('sha256').update(v).digest('hex');
const encode=v=>Buffer.from(JSON.stringify(v)+'\n');
const check=v=>{if(!v)throw Object.assign(Error('Vermont app evidence rejected.'),{code:'VT_CHILDCARE_APP_REJECTED'});};
const time=v=>typeof v==='string'&&Number.isFinite(Date.parse(v))&&new Date(v).toISOString()===v;
const claims=()=>({export_policy:'internal',public_export_authorized:false,national_reporting_integrated:false,native_execution_independently_verified:false,source_authenticity_verified:false,scheduled:false,physical_site_verified:false,identity_matching_eligible:false,address_role:'reported-address-role-unspecified',source_key_stability_verified:false,credential_deduplication_applied:false,geocodes_inferred:false,current_operations_verified:false,reporting_period:null,reporting_period_verified:false,coordinates_selected:false,source_reported_state_available:false});
const incomplete=()=>Object.assign(Error('Vermont app output requires inspection; preserve retained evidence before retry.'),{code:'VT_CHILDCARE_APP_INCOMPLETE'});
...
async function config(signal){const acquisition=await assertVtChildcareAcquisitionConfiguration(signal);check(hash(JSON.stringify(await readJson(path.join(APP_ROOT,'config/connectors/vt-childcare-centers-normalization.json'),100000,signal)))===NORMALIZATION);check(hash(JSON.stringify(await readJson(path.join(APP_ROOT,'config/connectors/vt-childcare-centers-app.json'),100000,signal)))===VT_CHILDCARE_APP_CONTRACT_SHA256);return {app_sha256:VT_CHILDCARE_APP_CONTRACT_SHA256,normalization_sha256:NORMALIZATION,...acquisition};}
...
async function inspect(receiptPath,signal,candidate=false){
  ...
  const s=start.value;check(s.schema_version===VT_CHILDCARE_APP_VERSION&&s.run_id===id&&same(s.configuration,configuration)&&time(s.started_at)&&['fixed-native-fetch','injected-test-transport','retained-local-verification'].includes(s.execution_mode));
  ...
  const a=await readVtChildcareAcquiredEvidence(acq.value.verification.manifest_path,{signal}),n=await readVtChildcareNormalizedRelease(norm.value.verification.manifest_path,{signal});
  ...
  check(same((await readVtChildcareAcquiredEvidence(a.verification.manifest_path,{signal})).verification,a.verification)&&same((await readVtChildcareNormalizedRelease(n.verification.manifest_path,{signal})).verification,n.verification));
...
export async function verifyVtChildcareAppJob(receiptPath,options={}){opts(options,['signal']);return inspect(receiptPath,options.signal);}
export async function runVtChildcareAppJob(options={}){opts(options,['outputRoot','signal','industryRunId','acquiredManifestPath']);return run(options,options.acquiredManifestPath===undefined?'fixed-native-fetch':'retained-local-verification');}
export async function runVtChildcareAppJobWithTransport(options={}){opts(options,['outputRoot','signal','industryRunId','fetchImpl','now','logger']);check(typeof options.fetchImpl==='function'&&(options.now===undefined||typeof options.now==='function')&&(options.logger===undefined||typeof options.logger==='function'));return run(options,'injected-test-transport');}
async function run(options,mode){
  ...
  const expire=()=>controller.abort(Object.assign(Error('Vermont app deadline exceeded.'),{code:'VT_CHILDCARE_APP_DEADLINE'}));const timer=setTimeout(expire,1_800_000),signal=controller.signal;
  ...
  try{
    ...
    const retained=mode==='retained-local-verification'?await readVtChildcareAcquiredEvidence(options.acquiredManifestPath,{signal}):null;
    const root=options.outputRoot??path.join(APP_ROOT,'data/business-sources/vt-childcare/app');check(typeof root==='string'&&root===path.resolve(root)&&!path.relative(APP_ROOT,root).split(path.sep).some(p=>p.toLowerCase()==='jobs'));await canonical(root,{output:true,signal});await rejectNestedAppWork(root);await canonical(root,{output:true,create:true,signal});
    ...
    start={schema_version:VT_CHILDCARE_APP_VERSION,run_id:id,industry_run_id:industry,execution_mode:mode,started_at:new Date().toISOString(),configuration,retained_acquired:retained?.verification??null};await assertOwned();startHash=await write(path.join(directory,'start.json'),start);
    const work=path.join(root,'runs',id);checkpoint();
    if(mode==='fixed-native-fetch'){
      const publisherRoot=path.join(APP_ROOT,'data/business-sources/vt-childcare/runtime');await canonical(publisherRoot,{create:true,output:true,signal});publisherPath=path.join(publisherRoot,'publisher.lock');
      try{publisher=await open(publisherPath,'wx');}catch(error){if(error.code==='EEXIST')throw Object.assign(Error('Vermont publisher ownership requires inspection.'),{code:'VT_CHILDCARE_PUBLISHER_BUSY'});throw error;}publisherIdentity=await publisher.stat({bigint:true});publisherValue={run_id:id,pid:process.pid};check(publisherIdentity.isFile()&&publisherIdentity.nlink===1n);await publisher.writeFile(encode(publisherValue));await publisher.sync();checkpoint();
    }
    acquired=retained?.verification??await (mode==='fixed-native-fetch'?buildVtChildcareAcquiredRelease({outputRoot:path.join(work,'acquired'),signal}):buildVtChildcareAcquiredReleaseWithTransport({outputRoot:path.join(work,'acquired'),signal,fetchImpl:options.fetchImpl,...(options.now?{now:options.now}:{})}));
    ...
    normalized=await buildVtChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:path.join(work,'normalized'),signal});await assertOwned();await write(path.join(directory,'normalized.json'),{retained_at:new Date().toISOString(),verification:normalized});if(options.logger)await options.logger({phase:'normalized-checkpoint'});checkpoint();
    ...
    const codes=['VT_CHILDCARE_PUBLICATION_INCOMPLETE','VT_CHILDCARE_NORMALIZATION_INCOMPLETE','VT_CHILDCARE_APP_DEADLINE','VT_CHILDCARE_APP_REJECTED','VT_CHILDCARE_PUBLISHER_BUSY','VT_CHILDCARE_DEFERRED','VT_CHILDCARE_PREFLIGHT_FAILED','VT_CHILDCARE_ACQUISITION_FAILED'];const code=codes.includes(errorCode)?errorCode:signal.aborted?'VT_CHILDCARE_APP_CANCELLED':'VT_CHILDCARE_APP_FAILED';
    ...
    throw Object.assign(Error('Vermont app did not complete; preserve and inspect retained evidence before retry.'),{code});
  }finally{
