...
import { normalizeCoChildcareAcquisition } from './co-childcare-normalization.mjs';

export const CO_CHILDCARE_NORMALIZED_RELEASE_VERSION='co-childcare-normalized-release@1.0.0';
const UUID=/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/;
const LIMITS={'normalized.jsonl':200_000_000,'quarantine.jsonl':30_000_000,'summary.json':1_000_000};
const PINS={connector_sha256:'c199eea013611813efa93fd08952592223c904dc8dce3cf6d99fd736fff5b5c8',policy_sha256:'2995c3c977b942307f24d0719b438602a5419716a8463bb5a966bd8c578c58bc'};
const encode=value=>Buffer.from(JSON.stringify(value)+'\n');
const hash=value=>createHash('sha256').update(value).digest('hex');
const check=value=>{if(!value)throw Error('Colorado normalized release rejected.');};
...
async function configuration(signal){
  for(const [file,pin]of [['config/connectors/co-childcare-centers-normalization.json',PINS.connector_sha256],['config/source-policies/co-childcare-centers-internal.json',PINS.policy_sha256]])check(hash(JSON.stringify(await readJson(path.join(APP_ROOT,file),100000,signal)))===pin);
}
const claims=()=>({export_policy:'internal',source_authenticity_verified:false,native_execution_independently_verified:false,public_export_authorized:false,national_reporting_integrated:false,app_enrolled:false,scheduled:false,physical_site_verified:false,identity_matching_eligible:false,address_role:'physical-address-as-reported-in-licensing-application',source_key_stability_verified:false,credential_deduplication_applied:false,geocodes_inferred:false,current_operations_verified:false,exact_address_geocodes_verified:false,coordinates_selected:false});
const inspection=()=>Object.assign(Error('Colorado normalization output may exist; preserve and inspect retained evidence before retry.'),{code:'CO_CHILDCARE_NORMALIZATION_INCOMPLETE'});
...
function manifestFor(runId,processedAt,input,summary,artifacts){return {schema_version:CO_CHILDCARE_NORMALIZED_RELEASE_VERSION,run_id:runId,status:'normalized-internal-evidence',source_release_id:`co-childcare-acquisition-${input.verification.run_id}`,processed_at:processedAt,configuration:{...PINS},acquired:sourceBinding(input),summary,claims:claims(),artifacts};}
async function replay(input,runId,processedAt,signal){
  check(time(processedAt)&&processedAt>=input.evidence.finished_at);
  return normalizeCoChildcareAcquisition(input.evidence,{runId,sourceReleaseId:`co-childcare-acquisition-${input.verification.run_id}`,processedAt},{signal});
}
async function inspect(manifestPath,signal,candidate=false){
  ...
  const input=await readCoChildcareAcquiredEvidence(manifest.acquired.manifest_path,{signal});check(same(manifest.acquired,sourceBinding(input)));
  ...
  const finalInput=await readCoChildcareAcquiredEvidence(manifest.acquired.manifest_path,{signal});check(same(finalInput.verification,input.verification)&&same(finalInput.evidence,input.evidence));
...
export async function readCoChildcareNormalizedRelease(manifestPath,options={}){optionsOnly(options,['signal']);return inspect(manifestPath,options.signal);}
export const verifyCoChildcareNormalizedRelease=readCoChildcareNormalizedRelease;

export async function buildCoChildcareNormalizedRelease(acquiredManifestPath,options={}){
  optionsOnly(options,['outputRoot','signal','processedAt']);const {signal,outputRoot=path.join(APP_ROOT,'data/business-sources/co-childcare/normalized')}=options;
  signal?.throwIfAborted();await configuration(signal);
  const input=await readCoChildcareAcquiredEvidence(acquiredManifestPath,{signal}),processedAt=options.processedAt??new Date().toISOString();check(time(processedAt)&&processedAt>=input.evidence.finished_at);
