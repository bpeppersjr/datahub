...
test("protects every live management endpoint while leaving only narrow liveness public", async (context) => {
  ...
  const protectedEndpoints = [
    ...
    ["GET", "/api/business-map/zip-quality"],
    ...
    ["GET", "/api/data-operations/zip-denominator-delta-review"],
      ...
      ["GET", "/api/data-operations/national-fdic-bankfind-coverage-status"],
    ...
    ["GET", "/api/data-operations/operations/fixture"],
    ["POST", "/api/data-operations/plan", "{}"],
  ...
  const connectorCatalogBody = JSON.parse(connectorCatalog.body);
    assert.equal(connectorCatalogBody.connector_count, 87);
    assert.equal(connectorCatalogBody.policy_profile_count, 68);
