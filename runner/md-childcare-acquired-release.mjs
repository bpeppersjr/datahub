...
import { acquireMdChildcareWithTransport, acquireMdChildcareNative, replayMdChildcareAcquisition, assertMdChildcareAcquisitionConfiguration } from './md-childcare-acquisition.mjs';

const VERSION='md-childcare-acquired-release@1.0.0';
const UUID=/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/;
const MAXIMUM=200_000_000;
const bytes=value=>Buffer.from(JSON.stringify(value)+'\n');
const hash=value=>createHash('sha256').update(value).digest('hex');
const check=value=>{if(!value)throw Error('Maryland acquired evidence rejected.');};
...
const inspection=()=>Object.assign(Error('Maryland acquisition publication may exist; preserve evidence and inspect before retry.'),{code:'MD_CHILDCARE_PUBLICATION_INCOMPLETE'});
async function derive(prerequisite,evidence,journal,signal){
  const replayed=await replayMdChildcareAcquisition(evidence,{signal});
...
export async function readMdChildcareAcquiredEvidence(manifestPath,options={}){
  optionsOnly(options,['signal']);const {signal}=options;signal?.throwIfAborted();
  await assertMdChildcareAcquisitionConfiguration(signal);
...
export const verifyMdChildcareAcquiredEvidence=readMdChildcareAcquiredEvidence;

export async function buildMdChildcareAcquiredReleaseWithTransport(options={}){
...
export async function buildMdChildcareAcquiredRelease(options={}){
  optionsOnly(options,['outputRoot','signal']);return build(options,'fixed-native-fetch');
}
async function build(options,mode){
  const {signal,now=()=>new Date(),outputRoot=path.join(APP_ROOT,'data/business-sources/md-childcare/acquired')}=options;
  ...
  try{
    ...
    const evidence=await(mode==='fixed-native-fetch'?acquireMdChildcareNative(callbacks):acquireMdChildcareWithTransport(callbacks));
    ...
    const {verification}=await readMdChildcareAcquiredEvidence(path.join(directory,'manifest.json'));
