...
export async function acquireOhChildcareWithTransport(options = {}) {
  check(optionsValid(options, ["fetchImpl", "signal", "sleep", "now", "timeoutMs", "maximumBytes"]), "unsupported options");
  ...
  const allowed = new Set([...metadataUrls, ohInventoryUrl()]);
  ...
  return { ...acquired, transport: { ...meter, mode: "injected-transport", source_authenticity_verified: false, accounting: "decoded-consumed-body-bytes-including-preflights-and-failed-attempts-not-wire-bytes" } };
}
