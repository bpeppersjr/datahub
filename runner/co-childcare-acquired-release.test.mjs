...
test('CO postcommit unlink failure with cancellation preserves published evidence and reports uncertainty',async()=>{
  const root=await mkdtemp(path.join(APP_ROOT,'data/tmp/co-acquired-commit-')),controller=new AbortController(),originalUnlink=fsPromises.unlink;
  let failed=false;
  ...
  try{
    const fixture=createCoChildcareFixture();
    await assert.rejects(buildCoChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot:root,signal:controller.signal}),{code:'CO_CHILDCARE_PUBLICATION_INCOMPLETE'});
    ...
    await assert.rejects(readCoChildcareAcquiredEvidence(path.join(directory,'manifest.json')),'uncertain leftover hardlink must not be silently treated as verified');
...
test('CO acquired builder rejects native transport overrides, missing transports and unsafe options before requests',async()=>{
  let requests=0;const fetchImpl=async()=>{requests++;throw Error('unexpected');};
  await assert.rejects(buildCoChildcareAcquiredRelease({fetchImpl}));
  await assert.rejects(buildCoChildcareAcquiredRelease({now:()=>new Date()}));
  await assert.rejects(buildCoChildcareAcquiredReleaseWithTransport({}));
  await assert.rejects(buildCoChildcareAcquiredReleaseWithTransport({fetchImpl,url:'https://example.invalid'}));
  await assert.rejects(buildCoChildcareAcquiredReleaseWithTransport({fetchImpl,outputRoot:APP_ROOT}));
  const signal=AbortSignal.abort();
  await assert.rejects(buildCoChildcareAcquiredReleaseWithTransport({fetchImpl,signal}),{name:'AbortError'});
  assert.equal(requests,0);
});

test('CO immutable acquisition persists before queries, excludes overlap, replays offline and preserves failure/cancellation boundaries',async()=>{
  const root=await mkdtemp(path.join(APP_ROOT,'data/tmp/co-acquired-test-'));
  try{
    await Promise.all([
      (async()=>{
        ...
        const fixture=createCoChildcareFixture({mutate:async(value,kind,{pageNumber})=>{
        ...
        const running=buildCoChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot});
        const atGate=await Promise.race([ready.then(()=>true),running.then(()=>false)]);assert.equal(atGate,true);
        try{
          let duplicateRequests=0;
          await assert.rejects(buildCoChildcareAcquiredReleaseWithTransport({outputRoot,fetchImpl:async()=>{duplicateRequests++;throw Error('duplicate');}}),{code:'EEXIST'});
        ...
        assert.equal(result.address_role,'physical-address-as-reported-in-licensing-application');assert.equal(result.current_operations_verified,false);
        ...
        const cli=await runFile(process.execPath,['--import',trap,'scripts/verify-co-childcare-acquired.mjs','--manifest',result.manifest_path],{cwd:APP_ROOT,windowsHide:true});
        ...
        await assert.rejects(readCoChildcareAcquiredEvidence(result.manifest_path));
        await writeFile(journalPath,original);await writeFile(result.manifest_path,manifestBytes);
        assert.equal((await readCoChildcareAcquiredEvidence(result.manifest_path)).verification.record_count,2);
        ...
        await assert.rejects(readCoChildcareAcquiredEvidence(result.manifest_path));await writeFile(journalPath,original);await writeFile(result.manifest_path,manifestBytes);
        ...
        await assert.rejects(buildCoChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot:path.join(root,'alias')}));
        await assert.rejects(buildCoChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot:directory}));
        const retainedRoster=await readdir(directory),retainedManifest=await readFile(result.manifest_path);
        await assert.rejects(buildCoChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot:path.join(directory,'nested-output')}));
        assert.deepEqual(await readdir(directory),retainedRoster);assert.deepEqual(await readFile(result.manifest_path),retainedManifest);
      })(),
      (async()=>{
        const outputRoot=path.join(root,'failed');
        const fixture=createCoChildcareFixture({mutate:(_value,kind)=>kind==='page'?new Response(null,{status:503}):undefined});
        await assert.rejects(buildCoChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot}));
        const [job]=await readdir(path.join(outputRoot,'jobs'));
      ...
      (async()=>{
        ...
        const fixture=createCoChildcareFixture({mutate:(_value,kind)=>{if(kind==='baseline-ids')controller.abort();}});
        await assert.rejects(buildCoChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot,signal:controller.signal}),{name:'AbortError'});
...
test('CO acquired verification CLI rejects invalid arguments without acquisition',async()=>{
  for(const args of [[],['--manifest'],['--manifest','relative.json'],['--url','https://example.invalid']])
    await assert.rejects(runFile(process.execPath,['scripts/verify-co-childcare-acquired.mjs',...args],{cwd:APP_ROOT,windowsHide:true}));
});
