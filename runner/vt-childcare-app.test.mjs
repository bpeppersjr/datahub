...
import { runVtChildcareAppJob, runVtChildcareAppJobWithTransport, verifyVtChildcareAppJob } from "./vt-childcare-app.mjs";
...
function fixture(mutate=()=>{}){return createVtChildcareFixture({mutate:async(v,k,c)=>{
  return mutate(v,k,c);
}});}
test("VT app rejects caller role/transport overrides, unsafe output and pre-abort before requests",async()=>{
  let calls=0;const fetchImpl=async()=>{calls++;throw Error("unexpected request");};
  await assert.rejects(runVtChildcareAppJob({fetchImpl}));
  await assert.rejects(runVtChildcareAppJobWithTransport({fetchImpl,role:"approved"}));
  await assert.rejects(runVtChildcareAppJobWithTransport({fetchImpl,outputRoot:APP_ROOT}));
  await assert.rejects(runVtChildcareAppJobWithTransport({fetchImpl,signal:AbortSignal.abort()}));
  assert.equal(calls,0);
});

test("VT app durable lifecycle links real synthetic acquisition and normalization, with offline retained reuse",async()=>{
  const root=await mkdtemp(path.join(APP_ROOT,"data/tmp/vt-app-test-"));
  try{
    ...
    const running=runVtChildcareAppJobWithTransport({outputRoot,fetchImpl:f.fetchImpl,industryRunId:"vt-test-industry"});
    const arrived=await Promise.race([ready.then(()=>true),running.then(()=>false)]);assert.equal(arrived,true);
    try{let duplicates=0;await assert.rejects(runVtChildcareAppJobWithTransport({outputRoot,fetchImpl:async()=>{duplicates++;throw Error("duplicate");}}));assert.equal(duplicates,0);}
    finally{release();}
    const result=await running;
    assert.equal(result.receipt.status,"SUCCEEDED");
    assert.equal(result.receipt.claims.reporting_period,null);assert.equal(result.receipt.claims.reporting_period_verified,false);assert.equal(result.receipt.claims.coordinates_selected,false);assert.equal(result.receipt.claims.source_reported_state_available,false);assert.equal(result.receipt.claims.current_operations_verified,false);
    assert.equal(result.receipt.industry_run_id,"vt-test-industry");
    ...
    const projected=await readVtChildcareNormalizedRelease(result.receipt.normalized.manifest_path);assert.equal(projected.records.length,2);assert.equal(projected.quarantine.length,0);assert.equal(projected.records[0].reported_address.street,null);assert.equal(projected.records[0].quality.address_unavailable_reason,"missing-source-address");assert.equal(projected.manifest.acquired.execution_mode,"injected-test-transport");
    ...
    let reused;
    try{
      await verifyVtChildcareAppJob(result.receiptPath);
      reused=await runVtChildcareAppJob({outputRoot:path.join(root,"industry-segments","runs","managed-test","source-VT"),acquiredManifestPath:sourceManifest});
      await verifyVtChildcareAppJob(reused.receiptPath);
    ...
    const cli=await execute(process.execPath,["scripts/build-vt-childcare.mjs","--acquired",sourceManifest,"--output",path.join(root,"cli-reuse")],{cwd:APP_ROOT,windowsHide:true});
    ...
    const cliVerified=JSON.parse((await execute(process.execPath,["scripts/verify-vt-childcare-app.mjs","--receipt",cliResult.receiptPath],{cwd:APP_ROOT,windowsHide:true})).stdout);
    ...
    await assert.rejects(verifyVtChildcareAppJob(result.receiptPath));
    ...
    for(const change of [r=>{r.claims.public_export_authorized=true;},r=>{r.acquired.execution_mode="fixed-native-fetch";},r=>{r.acquired=structuredClone(reused.receipt.acquired);r.acquired.manifest_path=reused.receipt.normalized.manifest_path;}]){
      const changed=JSON.parse(originalReceipt);change(changed);await writeFile(result.receiptPath,JSON.stringify(changed)+"\n");await assert.rejects(verifyVtChildcareAppJob(result.receiptPath));await writeFile(result.receiptPath,originalReceipt);
    }
    ...
    badCheckpoint.verification.record_count++;await writeFile(checkpointPath,JSON.stringify(badCheckpoint)+"\n");await assert.rejects(verifyVtChildcareAppJob(result.receiptPath));await writeFile(checkpointPath,checkpointBytes);
    await verifyVtChildcareAppJob(result.receiptPath);
    await assert.rejects(runVtChildcareAppJob({outputRoot:path.join(outputRoot,"runs",result.receipt.run_id,"nested"),acquiredManifestPath:sourceManifest}));
    await mkdir(path.join(root,"target"));await symlink(path.join(root,"target"),path.join(root,"alias"),"junction");
    await assert.rejects(runVtChildcareAppJobWithTransport({outputRoot:path.join(root,"alias"),fetchImpl:f.fetchImpl}));
  }finally{await rm(root,{recursive:true,force:true});}
});

test("VT app writes durable failed/cancelled outcomes and retains completed acquisition checkpoints",async()=>{
  const root=await mkdtemp(path.join(APP_ROOT,"data/tmp/vt-app-failure-test-"));
  try{
    const outcomes=await Promise.allSettled(["source","cancel","checkpoint-failure","checkpoint-cancel","normalized-cancel"].map(async kind=>{
      ...
      await assert.rejects(runVtChildcareAppJobWithTransport({outputRoot,fetchImpl:f.fetchImpl,signal:controller.signal,logger:async event=>{
      ...
      if(kind==="normalized-cancel"){assert.ok(receipt.normalized);assert.ok(await readFile(path.join(outputRoot,"jobs",id,"normalized.json")));await readVtChildcareNormalizedRelease(receipt.normalized.manifest_path);}
    }));
    for(const outcome of outcomes)if(outcome.status==="rejected")throw outcome.reason;
  }finally{await rm(root,{recursive:true,force:true});}
});

test("VT fixed native entry and historical retained acquisition both use the real pipeline",async()=>{
  const root=await mkdtemp(path.join(APP_ROOT,"data/tmp/vt-app-native-test-"));
  ...
  try{
    ...
    const nativeRun=runVtChildcareAppJob({outputRoot:path.join(root,"native")});
    const arrived=await Promise.race([ready.then(()=>true),nativeRun.then(()=>false)]);assert.equal(arrived,true);
    let gateError;
    try{
      const lockPath=path.join(APP_ROOT,"data/business-sources/vt-childcare/runtime/publisher.lock"),lockBytes=await readFile(lockPath);
      await assert.rejects(runVtChildcareAppJob({outputRoot:path.join(root,"native-other")}),{code:"VT_CHILDCARE_PUBLISHER_BUSY"});
    ...
    const results=await Promise.allSettled([
      nativeRun,
      buildVtChildcareAcquiredReleaseWithTransport({outputRoot:path.join(root,"historic-source"),fetchImpl:historicalFixture.fetchImpl,now:()=>new Date("2026-09-07T12:00:00.000Z")}),
    ]);
    ...
    const retained=await runVtChildcareAppJob({outputRoot:path.join(root,"historic-reuse"),acquiredManifestPath:oldSource.manifest_path});
    ...
    await verifyVtChildcareAppJob(retained.receiptPath);
  }finally{globalThis.fetch=originalFetch;await rm(root,{recursive:true,force:true});}
});


test("VT app CLI rejects wrong state, scope and unknown options without acquisition",async()=>{const execute=promisify(execFile);for(const args of [["--state","PA"],["--states","VT,PA"],["--url","https://example.invalid"],["--unknown"],["--acquired"],["--output","data/tmp/vt-cli-rejected","--output","data/tmp/vt-cli-rejected"]])await assert.rejects(execute(process.execPath,["scripts/build-vt-childcare.mjs",...args],{cwd:APP_ROOT,windowsHide:true}));assert.match((await execute(process.execPath,["scripts/build-vt-childcare.mjs","--help"],{cwd:APP_ROOT,windowsHide:true})).stdout,/Usage:/);for(const args of [[],["--receipt","relative"],["--receipt",APP_ROOT,"--extra"]])await assert.rejects(execute(process.execPath,["scripts/verify-vt-childcare-app.mjs",...args],{cwd:APP_ROOT,windowsHide:true}));});
