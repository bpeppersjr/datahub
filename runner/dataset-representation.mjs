...
const projections = {
  ...
  'national-irs-eo-bmf': ['irs-eo-bmf-organizations', 'irs_eo_bmf_organizations', 'IRS EO BMF'],
};

export function datasetRepresentation(plan, states, sources) {
  ...
  return {
    ...
    states: plan.states.map(code => {
      ...
      const datasets = expected.map(source => {
        ...
        const raw = source.profileId && counts?.[source.profileId];
