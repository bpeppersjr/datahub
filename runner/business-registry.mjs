...
export const OH_REPORTING_REGISTRY_PUBLISHER_VERSION = "2.15.0";
const OH_CHILDCARE_DATASET = "oh-dcy-publisher-open-childcare-centers";
// Historical/non-Ohio production plans do not execute an unpinned Ohio import
// closure. Ohio production enrollment must pin this closure when it is selected.
const ohioReporting = () => import("./oh-childcare-geographic-evidence.mjs");
function childcareSourceSummary(input) {
  return { dataset_id: input.datasetId, source_release_id: input.source.sourceReleaseId,
    dataset_release_id: input.source.releaseId, source_manifest_sha256: input.source.manifestSha256,
    observed_at: input.source.observedAt, accepted_source_records: input.counts.accepted,
    quarantined_source_records: input.counts.quarantined, physical_sites_published: input.counts.accepted,
    identity_matching_eligible: false, record_level_distribution: "local-review-only",
    general_operating_status_inferred: false, source_lineage: input.source };
}
...
function validateChildcareAssertionContract(record, geo) {
  const ma = geo.source.source_id === "ma-licensed-center-based-childcare";
  const exactKeys = (value, keys) => value && !Array.isArray(value) && isDeepStrictEqual(Object.keys(value).sort(), [...keys].sort());
  const text = (value, max, nullable = true, lineFeed = false) => (nullable && value === null)
    || (typeof value === "string" && value.length > 0 && value.length <= max && value.trim() === value
      && !(lineFeed ? /[\u0000-\u0009\u000b-\u001f\u007f]/u : /[\u0000-\u001f\u007f]/u).test(value));
  const contracts = {
    "site.address": ["address", ma ? "ADDRESS|CITY|ZIPCODE" : "address|address2|city|county|state|zip"],
    "site.zip-code": ["string", ma ? "ZIPCODE" : "zip"],
    "site.reported-location": ["object", ma ? "geometry.x|geometry.y" : "geometry.x|geometry.y|location_reference_desc|coord_source_type_desc|coord_sys_desc|coord_source_org_desc"],
    "site.source-geography": ["object", null],
    "establishment.name": ["string", ma ? "PROG_NAME" : "center_name"],
    "establishment.source-status": ["object", ma ? "LICENSED_STATUS|CAPACITY" : "licensed_capacity|license_approval_date|license_renewal_date"],
    "establishment.source-classification": ["object", ma ? "PROG_TYPE|LICENSED_FUNDED" : "foips|age_range|months_operational|sessions"],
    "establishment.source-affiliation": ["object", ma ? "PROG_UM" : null],
    "establishment.external-identifier": ["identifier", ma ? "PROV_NUM" : "center_id"],
    ...(ma ? { "site.external-identifier": ["identifier", "MAD_ID"] } : {}),
  };
  const contract = contracts[record.predicate], value = record.value;
  const subject = record.predicate.startsWith("site.") ? geo.site_entity_id : geo.establishment_entity_id;
  const expectedId = `assertion:${createHash("sha256").update(JSON.stringify([subject, record.predicate, value, geo.source.source_release_id, geo.source.source_record_id])).digest("hex").slice(0, 32)}`;
  const expected = { schema_version: "1.0.0", assertion_id: expectedId, subject_entity_id: subject,
    predicate: record.predicate, value, value_type: contract?.[0], assertion_status: "active", valid_from: null, valid_to: null,
    observed_at: geo.observed_at, first_seen: geo.observed_at, last_seen: geo.observed_at, confidence: 1,
    source: { ...geo.source, source_field: contract?.[1] }, export_policy: "local-review-only" };
  if (!contract || !isDeepStrictEqual(record, expected)) throw new Error("invalid childcare assertion contract");
  let valid = true;
  if (record.predicate === "site.zip-code") valid = value === geo.zip_code;
  if (record.predicate === "site.source-geography") valid = isDeepStrictEqual(value, { unique_business_identity_verified: false, current_usps_validity: "unverified", geographic_boundary_verified: false });
  if (record.predicate === "site.reported-location") {
    const extras = ma ? {} : { location_reference_source: 40, coordinate_source_type: 20, original_coordinate_system_source: 40, coordinate_source_organization: 20 };
    valid = exactKeys(value, ["latitude", "longitude", "crs", "source", "status", ...Object.keys(extras)])
      && value.latitude === geo.location.latitude && value.longitude === geo.location.longitude && value.crs === "EPSG:4326"
      && value.source === (ma ? "MassGIS/EEC" : "NJDEP/DCF")
      && value.status === (value.latitude === null ? "missing-source-point" : "publisher-geocoded-not-independently-verified")
      && Object.entries(extras).every(([key, max]) => text(value[key], max));
  }
  if (record.predicate === "establishment.source-classification") valid = ma
    ? isDeepStrictEqual(value, { category: "childcare", program_type: "Center-based Care", licensed_funded: "Licensed" })
    : exactKeys(value, ["category", "public_school_facility_source", "public_school_interpretation", "age_range_source", "months_operational_source", "sessions_source"])
      && value.category === "childcare" && value.public_school_interpretation === "publisher-value-preserved-not-boolean-inferred"
      && text(value.public_school_facility_source, 10) && text(value.age_range_source, 50) && text(value.months_operational_source, 50)
      && text(value.sessions_source, 175, true, geo.source.transformation_version.startsWith("nj-childcare-normalization@1.0.1 -> "));
  if (record.predicate === "establishment.source-affiliation") valid = ma
    ? exactKeys(value, ["program_umbrella_source", "parent_company", "ownership_verified"]) && text(value.program_umbrella_source, 255) && value.parent_company === null && value.ownership_verified === false
    : isDeepStrictEqual(value, { parent_company: null, ownership_verified: false });
  if (record.value_type === "identifier") valid = exactKeys(value, ["type", "value"])
    && value.type === (record.predicate === "site.external-identifier" ? "massgis_master_address_id" : ma ? "massachusetts_eec_provider_number" : "new_jersey_dcf_center_id")
    && text(value.value, ma ? 50 : 20, false);
  if (!valid) throw new Error("invalid childcare assertion value contract");
}
...
export function reconcileIlBusinessOrganization(record) {
  const identifier = record.external_identifiers?.find((item) => item.type === "il_sos_file_number");
  const fileNumber = identifier?.value;
  const entityKind = identifier?.entity_kind;
  const organizationId = record.entity_candidates?.organization_id;
  const address = record.records_office_address;
  const zipCode = address?.eligible_for_us_zip_coverage ? address.zip_code : null;
  if (!/^\d{8}$/.test(fileNumber ?? "") || !["corporation", "llc"].includes(entityKind)
    || organizationId !== `organization:il_sos_${entityKind}_${fileNumber}`
    || !record.legal_name || record.other_names?.length
    || record.entity_candidates?.brand_id || record.entity_candidates?.physical_site_id || record.entity_candidates?.establishment_id
    || !["00", "01"].includes(record.source_status?.code)
    || record.source_status?.value !== "listed-goodstanding-or-reinstated-in-illinois-sos-daily-files"
    || record.source_status?.semantics !== "source-registration-status-not-independent-proof-of-current-operations-or-a-physical-site"
    || record.registration_profile?.entity_kind !== entityKind
    || record.export_policy !== "local-review-only"
    || (entityKind === "corporation" && address !== null)
    || (entityKind === "llc" && !address)
    || (address && address.address_scope !== "secretary-of-state-records-office-address-not-verified-physical-operating-site")
    || (zipCode !== null && !/^\d{5}$/.test(zipCode))) {
    throw new Error(`Invalid Illinois Business Registry organization candidate ${record.normalized_record_id}.`);
  }
  const assertions = [
    assertion(record, organizationId, "organization.legal-name", record.legal_name, "string", "legal_name"),
    assertion(record, organizationId, "organization.il-registration-status", record.source_status, "object", "status_code"),
    assertion(record, organizationId, "organization.il-registration-profile", record.registration_profile, "object", "entity_type_code|jurisdiction_code|intent_code|incorporation_date|extended_date|transaction_date|purpose_code|management_type|jurisdiction_organized_code|organized_date|dissolution_date|status_date|source_flags"),
  ];
  if (record.annual_report_evidence) {
    assertions.push(assertion(record, organizationId, "organization.il-annual-report-evidence", record.annual_report_evidence, "object", "current_report_run_date|current_report_paid_date"));
  }
  if (address) {
    assertions.push(assertion(record, organizationId, "organization.records-office-address", address, "address", "records_office_street|records_office_city|records_office_jurisdiction|records_office_zip"));
  }
  for (const item of record.external_identifiers ?? []) {
    assertions.push(assertion(record, organizationId, "organization.external-identifier", item, "identifier", "file_number"));
  }
  if (zipCode) {
    assertions.push(assertion(record, organizationId, "organization.records-office-zip-code", zipCode, "string", "records_office_zip"));
    assertions.push(assertion(record, organizationId, "organization.records-office-zcta", record.geography, "object", "records_office_zip"));
  }
  return {
    fileNumber,
    entityKind,
    hashPrefix: digest(`${entityKind}:${fileNumber}`)[0],
    zipCode,
    entity: canonicalEntity(organizationId, "organization", record.observed_at),
    assertions,
  };
...
async function loadIlBusinessRelease(pointerPath) {
  const pointer = JSON.parse(await readFile(pointerPath, "utf8"));
  const pointerDirectory = path.dirname(pointerPath);
  const manifestPath = path.resolve(pointerDirectory, pointer.manifest ?? "");
  assertContained(pointerDirectory, manifestPath, "Illinois Business Registry manifest path");
  const manifestBuffer = await readFile(manifestPath);
  const manifest = JSON.parse(manifestBuffer.toString("utf8"));
  if (manifest.dataset_id !== "il-business-registry-active-organizations" || manifest.status !== "published"
    || manifest.complete_official_daily_file_set !== true || manifest.export_policy !== "local-review-only"
    || manifest.coverage?.active_organizations_published !== manifest.coverage?.active_corporations_published + manifest.coverage?.active_llcs_published
    || manifest.coverage?.physical_sites !== null || manifest.coverage?.establishments !== null) {
    throw new Error("A complete governed Illinois active-organization source release is required.");
  }
  const releaseDirectory = path.dirname(manifestPath);
  const organizationArtifacts = manifest.artifacts.filter((artifact) => artifact.artifact_type === "normalized-il-business-organization-jsonl-gzip").sort((a, b) => a.path.localeCompare(b.path));
  if (organizationArtifacts.length !== 16 || organizationArtifacts.some((artifact) => artifact.export_policy !== "local-review-only")) {
    throw new Error("Illinois Business Registry source release has an incomplete or misclassified normalized partition set.");
  }
  const zipArtifact = manifest.artifacts.find((artifact) => artifact.artifact_type === "il-business-registry-zip-coverage-jsonl");
  if (!zipArtifact || zipArtifact.export_policy !== "local-review-only") throw new Error("Illinois Business Registry source release has no governed ZIP coverage artifact.");
  for (const artifact of [...organizationArtifacts, zipArtifact]) {
    const filename = path.resolve(releaseDirectory, artifact.path);
    assertContained(releaseDirectory, filename, `Illinois Business Registry artifact ${artifact.path}`);
    const actual = await hashFile(filename);
    if (actual.bytes !== artifact.bytes || actual.sha256 !== artifact.sha256) throw new Error(`Illinois Business Registry artifact ${artifact.path} failed checksum validation.`);
  }
  const zipRows = (await readFile(path.join(releaseDirectory, zipArtifact.path), "utf8")).trim().split("\n").filter(Boolean).map(JSON.parse);
  if (zipRows.length !== zipArtifact.record_count || zipRows.length !== manifest.coverage.zip_union_records) throw new Error("Illinois Business Registry ZIP coverage record count does not match its manifest.");
  return { manifest, manifestSha256: sha256Buffer(manifestBuffer), releaseDirectory, organizationArtifacts, zipRows };
}
...
function registryZipCoverage({ childcareInputs = [], snap, nppes, fdic, ncua, fsis, echo, fmcsa, irsEo, ctBusiness, deBusiness, akBusiness, coBusiness, waLniActiveContractors, orBusiness, iaBusiness, nyBusiness, flBusiness, paBusiness, ilBusiness, laActiveBusinesses, txActiveSalesTax, chicagoActiveBusinessLicenses, dcBasicBusinessLicenses, caAbcActiveLicenses, nyRetailFoodStores, nycDcwpActiveLicenses, uspsZips, snapCounts, nppesPrimaryCounts, nppesSecondaryCounts, fdicLocationCounts, ncuaLocationCounts, fsisEstablishmentCounts, echoFacilityCounts, fmcsaRecordCounts, irsEoOrganizationCounts, ctBusinessOrganizationCounts, deBusinessOrganizationCounts, akBusinessOrganizationCounts, akBusinessSiteCounts, coBusinessOrganizationCounts, waLniMailingAddressCounts, orBusinessRegistrationCounts, iaBusinessOrganizationCounts, nyBusinessOrganizationCounts, flBusinessOrganizationCounts, paBusinessOrganizationCounts, ilBusinessOrganizationCounts, laActiveBusinessLocationCounts, txActiveSalesTaxOutletCounts, chicagoActiveBusinessLicenseSiteCounts, dcBasicBusinessLicenseSiteCounts, caAbcActiveLicenseSiteCounts, nyRetailFoodStoreZipEvidenceCounts, nyRetailFoodStoreSiteCounts, nycDcwpActiveLicenseSiteCounts }) {
  ...
  const ilBusinessRows = new Map((ilBusiness?.zipRows ?? []).map((row) => [row.zip_code, row]));
  ...
  const zipCodes = [...new Set([...childcareInputs.flatMap((input) => input.contributions.map((row) => row.zipCode).filter((zip) => zip !== null)), ...snapRows.keys(), ...nppesRows.keys(), ...fdicRows.keys(), ...ncuaRows.keys(), ...fsisRows.keys(), ...echoRows.keys(), ...fmcsaRows.keys(), ...irsEoRows.keys(), ...ctBusinessRows.keys(), ...deBusinessRows.keys(), ...akBusinessRows.keys(), ...coBusinessRows.keys(), ...waLniActiveContractorRows.keys(), ...orBusinessRows.keys(), ...iaBusinessRows.keys(), ...nyBusinessRows.keys(), ...flBusinessRows.keys(), ...paBusinessRows.keys(), ...ilBusinessRows.keys(), ...laActiveBusinessRows.keys(), ...txActiveSalesTaxRows.keys(), ...chicagoActiveBusinessLicenseRows.keys(), ...dcBasicBusinessLicenseRows.keys(), ...caAbcActiveLicenseRows.keys(), ...nyRetailFoodStoreRows.keys(), ...nycDcwpActiveLicenseRows.keys(), ...uspsRows.keys(), ...snapCounts.keys(), ...nppesPrimaryCounts.keys(), ...nppesSecondaryCounts.keys(), ...fdicLocationCounts.keys(), ...ncuaLocationCounts.keys(), ...fsisEstablishmentCounts.keys(), ...echoFacilityCounts.keys(), ...fmcsaRecordCounts.keys(), ...irsEoOrganizationCounts.keys(), ...ctBusinessOrganizationCounts.keys(), ...deBusinessOrganizationCounts.keys(), ...akBusinessOrganizationCounts.keys(), ...akBusinessSiteCounts.keys(), ...coBusinessOrganizationCounts.keys(), ...waLniMailingAddressCounts.keys(), ...orBusinessRegistrationCounts.keys(), ...iaBusinessOrganizationCounts.keys(), ...nyBusinessOrganizationCounts.keys(), ...flBusinessOrganizationCounts.keys(), ...paBusinessOrganizationCounts.keys(), ...ilBusinessOrganizationCounts.keys(), ...laActiveBusinessLocationCounts.keys(), ...txActiveSalesTaxOutletCounts.keys(), ...chicagoActiveBusinessLicenseSiteCounts.keys(), ...dcBasicBusinessLicenseSiteCounts.keys(), ...caAbcActiveLicenseSiteCounts.keys(), ...nyRetailFoodStoreZipEvidenceCounts.keys(), ...nyRetailFoodStoreSiteCounts.keys(), ...nycDcwpActiveLicenseSiteCounts.keys()])].sort();
  return zipCodes.map((zipCode) => {
    ...
    const ilBusinessRow = ilBusinessRows.get(zipCode);
    ...
    const ilBusinessOrganizations = ilBusinessOrganizationCounts.get(zipCode) ?? 0;
    ...
    if (ilBusiness && ilBusinessOrganizations !== (ilBusinessRow?.il_business_registry_active_snapshot?.organization_records_office_address_count ?? 0)) {
      throw new Error(`ZIP ${zipCode} Illinois Business Registry records-office-address counts do not reconcile.`);
    }
    ...
    return {
      ...
      registry_coverage: {
        ...
        ma_childcare_center_site_count: childCounts.ma_childcare_centers ?? 0,
        nj_childcare_center_site_count: childCounts.nj_childcare_centers ?? 0,
        ...(childcareInputs.some(input => input.datasetId === TN_CHILDCARE_DATASET) ? { tn_childcare_center_site_count: childCounts.tn_childcare_centers ?? 0 } : {}),
        ...(childcareInputs.some(input => input.datasetId === OH_CHILDCARE_DATASET) ? { oh_childcare_center_site_count: childCounts.oh_childcare_centers ?? 0 } : {}),
        ...
        il_business_registry_organization_records_office_address_count: ilBusinessOrganizations,
      ...
      source_contributions: {
        ...Object.fromEntries(childcareInputs.map((input) => [input.key, { reported_center_count: childCounts[input.key], source_release_id: input.source.sourceReleaseId, observed_at: input.source.observedAt, record_level_distribution: "local-review-only", active_business_verified: false }])),
        usda_snap_retailers: {
        ...
        ...(ilBusiness ? {
          il_business_registry_active_organizations: {
            organization_records_office_address_count: ilBusinessOrganizations,
            source_release_id: ilBusiness.manifest.source_release_id,
            source_run_date: ilBusiness.manifest.source_run_date,
            record_level_distribution: "local-review-only",
            physical_site_inference_permitted: false,
          },
        } : {}),
...
export async function buildNationalBusinessRegistry({
  ...
  const cmsNursingHomeApi = cmsNursingHomeSelection === null ? null : await import('./cms-nursing-home-registry-input.mjs');
  const cmsNursingHomes = cmsNursingHomeApi ? await cmsNursingHomeApi.loadCmsNursingHomeRegistryInput(cmsNursingHomeSelection) : null;
  ...
  const ohInput = ohChildcareReceipt === null ? null : await (await ohioReporting()).loadOhChildcareGeographicInput(ohChildcareReceipt);
  if (ohInput) childcareInputs.push({ ...ohInput, key: "oh_childcare_centers", datasetId: OH_CHILDCARE_DATASET });
  const ohRowsBySite = new Map(ohInput?.rows.map(row => [row.site_entity_id, row]) ?? []);
  ...
  const tnWithoutZip = tnInput?.contributions.filter(row => row.zipCode === null) ?? [];
  const ohWithoutZip = ohInput?.contributions.filter(row => row.zipCode === null) ?? [];
  const reportingWithoutZip = [...tnWithoutZip, ...ohWithoutZip];
  const tnMissingZipReasons = { "missing-source-zip": 0, "invalid-source-zip-placeholder": 0 };
  for (const row of tnWithoutZip) tnMissingZipReasons[row.evidence.zip_unavailable_reason]++;
  ...
  const ilBusiness = ilBusinessPointer ? await loadIlBusinessRelease(ilBusinessPointer) : null;
  const ilBusinessSourceRecords = ilBusiness
    ? Object.values(ilBusiness.manifest.coverage.source_records ?? {}).reduce((sum, count) => sum + Number(count), 0)
    : 0;
  if (!Number.isSafeInteger(ilBusinessSourceRecords) || ilBusinessSourceRecords < 0) throw new Error("Illinois Business Registry source record counts are invalid.");
  ...
  if (cmsNursingHomes && createdAt < cmsNursingHomes.source.recovery_created_at) throw new Error('Registry processing time precedes CMS nursing-home recovery.');
  ...
  const ilBusinessOrganizationWriters = new Map();
  const ilBusinessOrganizationAssertionWriters = new Map();
  ...
  if (ilBusiness) {
    for (const prefix of "0123456789abcdef") {
      ilBusinessOrganizationWriters.set(prefix, await openGzipWriter(stagingDirectory, `entities/organizations/il-file-hash-prefix=${prefix}.jsonl.gz`));
      ilBusinessOrganizationAssertionWriters.set(prefix, await openGzipWriter(stagingDirectory, `assertions/organizations/il-file-hash-prefix=${prefix}.jsonl.gz`));
    }
  }
  ...
  const ilBusinessOrganizationCountsByZip = new Map();
  ...
  const ilBusinessFileNumbers = new Set();
  ...
  let ilBusinessOrganizations = 0;
  let ilBusinessCorporations = 0;
  let ilBusinessLlcs = 0;
  ...
  if (ilBusiness) {
    for (const artifact of ilBusiness.organizationArtifacts) {
      const partition = artifact.path.match(/id-hash-prefix=([0-9a-f])/)?.[1];
      if (!partition) throw new Error(`Cannot determine Illinois Business Registry file-number hash prefix for ${artifact.path}.`);
      const count = await forEachGzipRecord(path.join(ilBusiness.releaseDirectory, artifact.path), async (record) => {
        const reconciled = reconcileIlBusinessOrganization(record);
        if (reconciled.hashPrefix !== partition) throw new Error(`Illinois Business Registry entity ${reconciled.entityKind}:${reconciled.fileNumber} is in the wrong ID-hash partition.`);
        const identity = `${reconciled.entityKind}:${reconciled.fileNumber}`;
        if (ilBusinessFileNumbers.has(identity)) throw new Error(`Duplicate Illinois Business Registry file identity ${identity}.`);
        ilBusinessFileNumbers.add(identity);
        await writeGzipRecord(ilBusinessOrganizationWriters.get(partition), reconciled.entity);
        for (const item of reconciled.assertions) await writeGzipRecord(ilBusinessOrganizationAssertionWriters.get(partition), item);
        if (reconciled.zipCode) ilBusinessOrganizationCountsByZip.set(reconciled.zipCode, (ilBusinessOrganizationCountsByZip.get(reconciled.zipCode) ?? 0) + 1);
        if (reconciled.entityKind === "corporation") ilBusinessCorporations += 1;
        else ilBusinessLlcs += 1;
        assertions += reconciled.assertions.length;
      });
      if (count !== artifact.record_count) throw new Error(`Illinois Business Registry organization artifact ${artifact.path} record count mismatch.`);
      ilBusinessOrganizations += count;
      logger(`Reconciled ${ilBusinessOrganizations.toLocaleString("en-US")} Illinois active registered organizations.`);
    }
    if (ilBusinessOrganizations !== ilBusiness.manifest.coverage.active_organizations_published
      || ilBusinessCorporations !== ilBusiness.manifest.coverage.active_corporations_published
      || ilBusinessLlcs !== ilBusiness.manifest.coverage.active_llcs_published) {
      throw new Error("Registry Illinois organization counts do not match the source release.");
    }
    const allocated = [...ilBusinessOrganizationCountsByZip.values()].reduce((sum, count) => sum + count, 0);
    if (allocated !== ilBusiness.manifest.coverage.eligible_llc_records_office_addresses) throw new Error("Registry Illinois ZIP-address allocation count does not match the source release.");
  }
  ...
  const reportingWriters = new Map();
  if (reportingWithoutZip.length) {
    siteWriters.set("unassigned", await openGzipWriter(stagingDirectory, "entities/physical-sites/prefix=unassigned.jsonl.gz"));
    establishmentWriters.set("unassigned", await openGzipWriter(stagingDirectory, "entities/establishments/prefix=unassigned.jsonl.gz"));
    assertionWriters.set("unassigned", await openGzipWriter(stagingDirectory, "assertions/prefix=unassigned.jsonl.gz"));
    relationshipWriters.set("unassigned", await openGzipWriter(stagingDirectory, "relationships/prefix=unassigned.jsonl.gz"));
  }
  for (const input of childcareInputs) {
    for (const contribution of input.contributions) {
      const geo = input.datasetId === OH_CHILDCARE_DATASET ? ohRowsBySite.get(contribution.entities[0].entity_id)
        : input.datasetId === TN_CHILDCARE_DATASET ? (tnFreshChildcareManifest !== null ? createFreshTnChildcareGeographicEvidence : createTnChildcareGeographicEvidence)(contribution) : createChildcareGeographicEvidence(contribution);
      ...
      for (const row of contribution.assertions) await writeGzipRecord(assertionWriters.get(prefix), row);
      for (const row of contribution.relationships) await writeGzipRecord(relationshipWriters.get(prefix), row);
      await writeGzipRecord(reportingWriters.get(zip2), geo);
      assertions += contribution.assertions.length; relationships += contribution.relationships.length;
    }
  }
  const physicalSiteCount = reportingLocationCount + snapRecords + nppesPrimaryLocations + nppesSecondaryLocations + fdicLocations + ncuaLocations + fsisEstablishments + echoFacilities + fmcsaRecords + akBusinessPhysicalSites + laActiveBusinessLocations + txActiveSalesTaxOutlets + chicagoActiveBusinessLicenseSiteCount + dcBasicBusinessLicenseSiteCount + caAbcActiveLicenseSiteCount + nyRetailFoodStorePhysicalSites + nycDcwpActiveLicenseSiteCount;
  if (resolutionLocationProfiles + reportingLocationCount !== physicalSiteCount) {
  ...
  if (cmsNursingHomes) artifacts.push(await writeArtifact(stagingDirectory, cmsNursingHomeApi.CMS_NURSING_HOME_REGISTRY_PATH, jsonLines(cmsNursingHomes.records), {
    artifact_type: cmsNursingHomeApi.CMS_NURSING_HOME_REGISTRY_ARTIFACT, record_count: cmsNursingHomes.records.length, export_policy: 'local-review-only',
  }));
  ...
  if (ilBusiness) artifacts.push(...await closeGzipWriters([...ilBusinessOrganizationWriters.values()], "canonical-organization-jsonl-gzip"));
  ...
  if (ilBusiness) artifacts.push(...await closeGzipWriters([...ilBusinessOrganizationAssertionWriters.values()], "business-assertion-jsonl-gzip"));
  ...
  artifacts.push(await writeArtifact(stagingDirectory, "derived/zip-coverage.jsonl", jsonLines(zipCoverage), {
    ...
    distribution_policy: ilBusiness || reportingLocationCount > 0
      ? "local-review-only"
      : uspsZips
  ...
  const sourceContribution = {
    ...
    ...(ilBusiness ? {
      il_business_registry_active_organizations: {
        source_ids: ["illinois-sos-corporation-daily-files", "illinois-sos-llc-daily-files"],
        dataset_id: ilBusiness.manifest.dataset_id,
        source_release_id: ilBusiness.manifest.source_release_id,
        dataset_release_id: ilBusiness.manifest.release_id,
        source_run_date: ilBusiness.manifest.source_run_date,
        source_records: ilBusiness.manifest.coverage.source_records,
        active_organizations_published: ilBusinessOrganizations,
        active_corporations_published: ilBusinessCorporations,
        active_llcs_published: ilBusinessLlcs,
        eligible_llc_records_office_address_zip_contributions: ilBusiness.manifest.coverage.eligible_llc_records_office_addresses,
        possible_corporation_ngs_month_rule_not_evaluated: ilBusiness.manifest.coverage.possible_corporation_ngs_month_rule_not_evaluated,
        physical_sites_published: 0,
        establishments_published: 0,
        relationships_published: 0,
        identity_resolution: "one provisional organization per Illinois entity kind and eight-digit SOS file number; LLC records-office addresses remain organization assertions and do not create sites, establishments, owners, or relationships; no cross-source merge",
        record_level_distribution: "local-review-only",
        aggregate_distribution: "local-review-only",
        general_operating_status_inferred: false,
      },
    } : {}),
    ...(laActiveBusinesses ? {
  ...
  const manifest = {
    ...
    publisher: { id: "national-business-registry", version: ohInput ? OH_REPORTING_REGISTRY_PUBLISHER_VERSION : tnFreshChildcareManifest !== null ? TN_FRESH_REPORTING_REGISTRY_PUBLISHER_VERSION : tnInput ? TN_REPORTING_REGISTRY_PUBLISHER_VERSION : "2.12.0" },
    ...(ohInput ? { oh_childcare_source: structuredClone(ohInput.source), tn_childcare_origin: tnFreshChildcareManifest !== null ? "fresh" : tnInput ? "recovered" : null } : {}),
    ...
    publication_scope: `${[
      ...
      ...childcareInputs.map((input) => `${input.datasetId} reported ${input.datasetId === OH_CHILDCARE_DATASET ? "publisher-open-center" : "licensed-center"} source rows, reporting-only provisional sites without operating or matching claims`),
      ...
      ...(ilBusiness ? ["Illinois SOS Goodstanding or Reinstated corporations and LLCs from one complete official daily-file set, with eligible LLC records-office addresses preserved as local-review-only organization evidence and no site inference"] : []),
    ...
    coverage: {
      ...
      il_business_registry_source_records: ilBusinessSourceRecords,
      il_business_registry_active_organization_records: ilBusinessOrganizations,
      il_business_registry_active_corporation_records: ilBusinessCorporations,
      il_business_registry_active_llc_records: ilBusinessLlcs,
      il_business_registry_eligible_llc_records_office_addresses: ilBusiness?.manifest.coverage.eligible_llc_records_office_addresses ?? 0,
      il_business_registry_possible_corporation_ngs_month_rule_not_evaluated: ilBusiness?.manifest.coverage.possible_corporation_ngs_month_rule_not_evaluated ?? 0,
      ...
      organizations: nppesOrganizations + fdicInstitutions + ncuaInstitutions + irsEoOrganizations + ctBusinessOrganizations + deBusinessOrganizations + akBusinessOrganizations + coBusinessOrganizations + waLniActiveContractorOrganizations + orBusinessLegalEntityRegistrations + iaBusinessOrganizations + nyBusinessOrganizations + flBusinessOrganizations + paBusinessOrganizations + ilBusinessOrganizations + txActiveSalesTaxOrganizations + chicagoActiveBusinessLicenseOrganizationCount + dcBasicBusinessLicenseOrganizationCount + caAbcActiveLicenseOrganizationCount + nyRetailFoodStoreOrganizations + nycDcwpActiveLicenseOrganizationCount,
      brands: orBusinessAssumedNameRegistrations,
      physical_sites: physicalSiteCount,
      reporting_location_evidence: reportingLocationCount,
      ma_childcare_center_sites: childcareInputs.find((input) => input.key === "ma_childcare_centers")?.counts.accepted ?? 0,
      nj_childcare_center_sites: childcareInputs.find((input) => input.key === "nj_childcare_centers")?.counts.accepted ?? 0,
      ...(tnInput ? { tn_childcare_center_sites: tnInput.counts.accepted, tn_childcare_center_sites_with_zip: tnInput.counts.accepted - tnWithoutZip.length,
        tn_childcare_center_sites_without_zip: tnWithoutZip.length, reporting_location_evidence_without_zip: reportingWithoutZip.length,
        tn_childcare_missing_zip_reasons: tnMissingZipReasons } : {}),
      ...(ohInput ? { oh_childcare_center_sites: ohInput.counts.accepted, oh_childcare_center_sites_with_zip: ohInput.counts.accepted - ohWithoutZip.length,
        oh_childcare_center_sites_without_zip: ohWithoutZip.length, oh_childcare_missing_zip_reasons: structuredClone(ohInput.quality.zip_unavailable_reasons),
        reporting_location_evidence_without_zip: reportingWithoutZip.length } : {}),
    ...
    dependencies: [
      ...
      ...(cmsNursingHomes ? [cmsNursingHomeApi.cmsNursingHomeRegistryDependency(cmsNursingHomes)] : []),
      ...
      ...(ilBusiness ? [{
        dataset_id: ilBusiness.manifest.dataset_id,
        release_id: ilBusiness.manifest.release_id,
        manifest_sha256: ilBusiness.manifestSha256,
      }] : []),
      ...(laActiveBusinesses ? [{
        dataset_id: laActiveBusinesses.manifest.dataset_id,
      ...
      ...(ilBusiness?.manifest.dependencies ?? []),
    ...
    export_policy: uspsZips
      ? (reportingLocationCount > 0 || deBusiness || akBusiness || waLniActiveContractors || ilBusiness || laActiveBusinesses || txActiveSalesTax || chicagoActiveBusinessLicenses || dcBasicBusinessLicenses || caAbcActiveLicenses || nyRetailFoodStores || nycDcwpActiveLicenses)
        ? "Mixed source policies: Alaska, California ABC, New York retail food, Delaware, Illinois, Washington L&I, Los Angeles, Texas, Chicago, DC, and NYC license, permit, or registration record-level entities, assertions, relationships, match profiles, and applicable aggregates remain local-review-only when present; Alaska-derived aggregate fields additionally require terms review, Washington L&I aggregates require PDDL attribution and semantic limitations, California ABC aggregates require attribution and semantic limitations, New York retail-food aggregates require OPEN-NY terms, attribution, and source limitations, DC aggregates require CC BY 4.0 attribution and semantic limitations, and ZIP coverage derived from USPS assignments inherits the most restrictive applicable policy."
    ...
    limitations: [
      ...
      ...(ilBusiness ? [
        "The Illinois Business Registry layer covers Goodstanding or Reinstated corporations and LLCs in one complete official daily-file set, not every operating business in Illinois or the United States.",
        "Illinois source registration status is not independent proof of current operations, licensure, solvency, public access, or an open storefront.",
        "Only LLC records-office addresses are retained; they may be administrative or residential and never create a physical site, establishment, owner, or relationship.",
        "Corporation officer addresses, registered agents, LLC managers or members, assumed names, stock, finance, and other person-linked fields are excluded before persistence.",
        "The source's ambiguous corporation NGS month rule is not guessed, and the documented modified-modulus-11 check digit is not claimed as independently verified because the algorithm is unpublished.",
        "Illinois record-level and aggregate outputs remain local-review-only pending written source confirmation and a redistribution review.",
      ] : []),
      ...(laActiveBusinesses ? [
  ...
  if (cmsNursingHomes) {
    manifest.cms_nursing_home_directory_reporting = cmsNursingHomeApi.cmsNursingHomeRegistryDeclaration(cmsNursingHomes);
    manifest.coverage.cms_nursing_home_directory_rows = cmsNursingHomes.records.length;
    manifest.publication_scope += ', plus pinned CMS nursing-home directory reporting rows, not yet identity reconciled';
    manifest.export_policy += '; CMS nursing-home directory reporting artifacts remain local-review-only';
    manifest.limitations.push('CMS nursing-home recovery rows retain dated publisher active status, original address/TAB and source coordinate quality; they do not increase business/site/profile counts or establish independently verified operations or facility points.');
  }
  ...
  if (ohInput || retainedChildcare || mnCredentials || cmsHospitals || cmsNursingHomes) await verifyNationalBusinessRegistry(path.join(stagingDirectory, "manifest.json"));
...
const NUMERIC_ENTITY_ID_PATTERNS = Object.freeze({
  organization: Object.freeze([
    ...
    Object.freeze({ prefix: "organization:il_sos_corporation_", suffix: "" }),
    Object.freeze({ prefix: "organization:il_sos_llc_", suffix: "" }),
...
export async function verifyNationalBusinessRegistry(manifestPath) {
  ...
  try {
    const cmsNursingAdmission = Object.hasOwn(manifest, 'cms_nursing_home_directory_reporting')
      || Object.hasOwn(manifest.coverage ?? {}, 'cms_nursing_home_directory_rows')
      || (manifest.artifacts ?? []).some(a => a.path === 'reporting/cms-nursing-homes/directory.jsonl' || a.artifact_type === 'cms-nursing-home-directory-reporting-jsonl')
      || (manifest.dependencies ?? []).some(d => d.dataset_id === 'cms-nursing-home-provider-information')
      || await lstat(path.join(releaseDirectory, 'reporting/cms-nursing-homes/directory.jsonl')).then(() => true, error => { if (error.code === 'ENOENT') return false; throw error; });
    if (cmsNursingAdmission) {
      const {verifyCmsNursingHomeRegistryExtension} = await import('./cms-nursing-home-registry-input.mjs');
      await verifyCmsNursingHomeRegistryExtension(manifest, releaseDirectory);
    }
  ...
  const tnFreshReportingSupported = manifest.publisher?.version === TN_FRESH_REPORTING_REGISTRY_PUBLISHER_VERSION || (ohReportingSupported && manifest.tn_childcare_origin === "fresh");
  const tnReportingSupported = manifest.publisher?.version === TN_REPORTING_REGISTRY_PUBLISHER_VERSION || tnFreshReportingSupported || (ohReportingSupported && manifest.tn_childcare_origin === "recovered");
  const childcareSourceIds = new Set(["ma-licensed-center-based-childcare", "nj-licensed-childcare-centers", TN_CHILDCARE_DATASET, OH_CHILDCARE_DATASET]);
  ...
  if (!tnReportingSupported && ["tn_childcare_center_sites", "tn_childcare_center_sites_with_zip", "tn_childcare_center_sites_without_zip", ...(!ohReportingSupported ? ["reporting_location_evidence_without_zip"] : []), "tn_childcare_missing_zip_reasons"].some(key => Object.hasOwn(manifest.coverage, key))) failures.push({ path: "manifest.json", reason: "TN coverage fields require registry 2.13" });
  let ohInput = null, ohModule = null;
  const ohExpected = { entities: new Map(), assertions: new Map(), relationships: new Map() };
  const ohSeen = { entities: new Set(), assertions: new Set(), relationships: new Set() };
  try {
    if (childcareDependencies.some(d => d.dataset_id === OH_CHILDCARE_DATASET) !== ohReportingSupported) throw new Error("Ohio requires explicit registry 2.15 and source dependency");
    if (!ohReportingSupported && (Object.hasOwn(manifest, "oh_childcare_source") || Object.hasOwn(manifest, "tn_childcare_origin")
      || Object.keys(manifest.coverage).some(key => key.startsWith("oh_childcare_")))) throw new Error("Ohio fields require registry 2.15");
    if (ohReportingSupported) {
      if (![null, "fresh", "recovered"].includes(manifest.tn_childcare_origin)) throw new Error("invalid 2.15 Tennessee origin");
      ohModule = await ohioReporting();
      ohInput = await ohModule.loadOhChildcareGeographicInput(manifest.oh_childcare_source?.receiptPath);
      const dependency = childcareDependencies.find(d => d.dataset_id === OH_CHILDCARE_DATASET);
      if (!isDeepStrictEqual(manifest.oh_childcare_source, ohInput.source) || dependency.release_id !== ohInput.source.releaseId
        || dependency.manifest_sha256 !== ohInput.source.manifestSha256) throw new Error("Ohio app source dependency mismatch");
      for (const c of ohInput.contributions) {
        const prefix = c.zipCode?.[0] ?? "unassigned";
        for (const row of c.entities) ohExpected.entities.set(row.entity_id, { row, path: `entities/${row.entity_type === "physical_site" ? "physical-sites" : "establishments"}/prefix=${prefix}.jsonl.gz` });
        for (const row of c.assertions) ohExpected.assertions.set(row.assertion_id, { row, path: `assertions/prefix=${prefix}.jsonl.gz` });
        for (const row of c.relationships) ohExpected.relationships.set(row.relationship_id, { row, path: `relationships/prefix=${prefix}.jsonl.gz` });
      }
    }
  } catch (error) { failures.push({ path: "manifest.json", reason: `Ohio source validation failed: ${error.message}` }); }
  const isOhEntity = id => /^(site|establishment):oh_childcare_/.test(id ?? "");
  function verifyOhCanonical(row, kind, artifactPath) {
    if (row.source?.source_id !== OH_CHILDCARE_DATASET && ![row.entity_id, row.subject_entity_id, row.object_entity_id].some(isOhEntity)) return;
    const id = row.entity_id ?? row.assertion_id ?? row.relationship_id, expected = ohExpected[kind].get(id);
    if (!expected || !isDeepStrictEqual(row, expected.row) || artifactPath !== expected.path || ohSeen[kind].has(id)) throw new Error("Ohio canonical source ownership, value, partition or duplicate mismatch");
    ohSeen[kind].add(id);
  }
  ...
  const illinoisIntegrated = Boolean(illinoisDependency) || illinoisOrganizationCount > 0;
  ...
  if (illinoisIntegrated && !versionAtLeast(manifest.publisher?.version, "2.11.0")) {
    failures.push({ path: "manifest.json", reason: "Illinois integration requires national business registry publisher 2.11.0 or later" });
  }
  if (Boolean(illinoisDependency) !== (illinoisOrganizationCount > 0)) {
    failures.push({ path: "manifest.json", reason: "Illinois dependency and organization coverage declarations are inconsistent" });
  }
  if (illinoisIntegrated) {
    const illinoisCounts = [
      manifest.coverage?.il_business_registry_source_records,
      illinoisOrganizationCount,
      manifest.coverage?.il_business_registry_active_corporation_records,
      manifest.coverage?.il_business_registry_active_llc_records,
      manifest.coverage?.il_business_registry_eligible_llc_records_office_addresses,
      manifest.coverage?.il_business_registry_possible_corporation_ngs_month_rule_not_evaluated,
    ];
    if (illinoisCounts.some((count) => !Number.isSafeInteger(count) || count < 0)
      || illinoisOrganizationCount !== manifest.coverage.il_business_registry_active_corporation_records + manifest.coverage.il_business_registry_active_llc_records
      || manifest.coverage.il_business_registry_eligible_llc_records_office_addresses > manifest.coverage.il_business_registry_active_llc_records
      || manifest.coverage.il_business_registry_possible_corporation_ngs_month_rule_not_evaluated > manifest.coverage.il_business_registry_active_corporation_records
      || manifest.coverage.il_business_registry_source_records < illinoisOrganizationCount) {
      failures.push({ path: "manifest.json", reason: "Illinois source, organization, entity-kind, or records-office counts are invalid" });
    }
  }
  ...
  const tnEntities = new Map(), tnRelationships = new Map(), tnEntityPaths = new Map(), tnAssertionPaths = new Map(), tnRelationshipPaths = new Map();
  const illinoisEntityCounts = { corporation: 0, llc: 0 };
  ...
  for (const artifact of entityArtifacts) {
    try {
      const consume = (record) => {
        ...
        if (record.entity_id.startsWith("organization:il_sos_")) {
          const identity = record.entity_id.match(/^organization:il_sos_(corporation|llc)_(\d{8})$/);
          if (!identity) throw new Error(`invalid Illinois organization entity ID ${record.entity_id}`);
          illinoisEntityCounts[identity[1]] += 1;
        }
        verifyOhCanonical(record, "entities", artifact.path);
        if (artifact.path.includes("unassigned") && !(tnReportingSupported && /^(site|establishment):tn_childcare_/.test(record.entity_id)) && !(ohReportingSupported && isOhEntity(record.entity_id))) throw new Error("unassigned entities must be supported reporting members");
        entityIdsByType.get(entityType).add(record.entity_id);
        if (/^(site|establishment):tn_childcare_/.test(record.entity_id)) { tnEntities.set(record.entity_id, record); tnEntityPaths.set(record.entity_id, artifact.path); }
  ...
  if (illinoisEntityCounts.corporation !== (manifest.coverage?.il_business_registry_active_corporation_records ?? 0)
    || illinoisEntityCounts.llc !== (manifest.coverage?.il_business_registry_active_llc_records ?? 0)
    || illinoisEntityCounts.corporation + illinoisEntityCounts.llc !== illinoisOrganizationCount) {
    failures.push({ path: "manifest.json", reason: "Illinois entity-kind counts do not reconcile" });
  }

  let resolutionProfileCount = 0;
if (["1.2.0", "1.3.0", "1.4.0", "1.5.0", "1.6.0", "1.7.0", "1.8.0", "1.9.0", "2.0.0", "2.1.0", "2.2.0", "2.3.0", "2.4.0", "2.5.0", "2.6.0", "2.7.0", "2.8.0", "2.9.0", "2.10.0", "2.11.0", "2.12.0", "2.13.0", "2.14.0", "2.15.0"].includes(manifest.publisher?.version) && resolutionProfileArtifacts.length !== 100) {
  ...
  const profileSiteIds = new ExactPartitionedSet();
  const reportingRows = new Map(), reportingAssertions = new Map(), reportingRelationships = new Set();
  const reportingCounts = { "ma-licensed-center-based-childcare": 0, "nj-licensed-childcare-centers": 0, [TN_CHILDCARE_DATASET]: 0, [OH_CHILDCARE_DATASET]: 0 };
  const reportingPaths = new Set();
  for (const artifact of (manifest.artifacts ?? []).filter((entry) => entry.artifact_type === CHILDCARE_GEOGRAPHIC_ARTIFACT_TYPE)) {
    try {
      if (!reportingEvidenceSupported || artifact.export_policy !== "local-review-only") throw new Error("reporting location version or policy");
      const zip2 = artifact.path.match(/^reporting\/location-evidence\/zip2=(\d{2}|unassigned)\/records\.jsonl\.gz$/)?.[1];
      if (!zip2 || (zip2 === "unassigned" && ((!tnReportingSupported && !ohReportingSupported) || artifact.record_count < 1)) || reportingPaths.has(artifact.path)) throw new Error("reporting location partition path");
      reportingPaths.add(artifact.path);
      const count = await forEachGzipRecord(path.join(releaseDirectory, artifact.path), (row) => {
        if (row.source?.source_id === OH_CHILDCARE_DATASET) {
          ohModule.validateOhChildcareGeographicEvidence(row, ohInput?.verificationContext);
        } else if (row.source?.source_id === TN_CHILDCARE_DATASET) {
        ...
        if ((row.zip_code?.slice(0, 2) ?? "unassigned") !== zip2 || reportingRows.has(row.site_entity_id)
          || !hasEntityId(row.site_entity_id) || !hasEntityId(row.establishment_entity_id)
          || !dependency || dependency.release_id !== row.evidence.release_id || dependency.manifest_sha256 !== row.evidence.manifest_sha256) throw new Error("reporting location identity or lineage mismatch");
        reportingRows.set(row.site_entity_id, row); reportingCounts[row.source.source_id]++;
      });
      if (count !== artifact.record_count) throw new Error("reporting location record count");
    } catch (error) { failures.push({ path: artifact.path, reason: error.message }); }
  }
  if (reportingEvidenceSupported && (reportingRows.size !== manifest.coverage.reporting_location_evidence
    || reportingCounts["ma-licensed-center-based-childcare"] !== manifest.coverage.ma_childcare_center_sites
    || reportingCounts["nj-licensed-childcare-centers"] !== manifest.coverage.nj_childcare_center_sites
    || childcareDependencies.some((dependency) => dependency.dataset_id !== OH_CHILDCARE_DATASET && !reportingCounts[dependency.dataset_id]))) failures.push({ path: "manifest.json", reason: "reporting-only childcare counts or dependencies do not reconcile" });
  const tnRows = [...reportingRows.values()].filter(row => row.source.source_id === TN_CHILDCARE_DATASET), tnUnassignedRows = tnRows.filter(row => row.zip_code === null);
  const ohRows = [...reportingRows.values()].filter(row => row.source.source_id === OH_CHILDCARE_DATASET), ohUnassignedRows = ohRows.filter(row => row.zip_code === null);
  const unassignedRows = [...tnUnassignedRows, ...ohUnassignedRows];
  const expectedUnassigned = (tnReportingSupported || ohReportingSupported) && unassignedRows.length ? ["entities/physical-sites/prefix=unassigned.jsonl.gz", "entities/establishments/prefix=unassigned.jsonl.gz", "assertions/prefix=unassigned.jsonl.gz", "relationships/prefix=unassigned.jsonl.gz", "reporting/location-evidence/zip2=unassigned/records.jsonl.gz"].sort() : [];
  if (!isDeepStrictEqual(manifest.artifacts.filter(a => a.path.includes("unassigned")).map(a => a.path).sort(), expectedUnassigned)) failures.push({ path: "manifest.json", reason: "unassigned artifact roster must exactly match TN missing ZIP membership" });
  const missingZipReasons = { "missing-source-zip": 0, "invalid-source-zip-placeholder": 0 };
  for (const row of tnUnassignedRows) missingZipReasons[row.evidence.zip_unavailable_reason]++;
  if (tnReportingSupported && (manifest.coverage.tn_childcare_center_sites !== tnRows.length || manifest.coverage.tn_childcare_center_sites_with_zip !== tnRows.length - tnUnassignedRows.length
    || manifest.coverage.tn_childcare_center_sites_without_zip !== tnUnassignedRows.length || manifest.coverage.reporting_location_evidence_without_zip !== unassignedRows.length
    || !isDeepStrictEqual(manifest.coverage.tn_childcare_missing_zip_reasons, missingZipReasons))) failures.push({ path: "manifest.json", reason: "TN reporting ZIP availability or reasons do not reconcile" });
  ...
  if (["1.2.0", "1.3.0", "1.4.0", "1.5.0", "1.6.0", "1.7.0", "1.8.0", "1.9.0", "2.0.0", "2.1.0", "2.2.0", "2.3.0", "2.4.0", "2.5.0", "2.6.0", "2.7.0", "2.8.0", "2.9.0", "2.10.0", "2.11.0", "2.12.0", "2.13.0", "2.14.0", "2.15.0"].includes(manifest.publisher?.version)
  ...
  const allowedSourceStatuses = new Set([
    ...
    "listed-goodstanding-or-reinstated-in-illinois-sos-daily-files",
  ...
  let illinoisStatusAssertionCount = 0;
  for (const artifact of assertionArtifacts) {
    try {
      ...
      const count = await forEachGzipRecord(path.join(releaseDirectory, artifact.path), (record) => {
        ...
        const isChildcare = childcareSourceIds.has(record.source.source_id);
        if (isChildcare) {
          const siteId = record.subject_entity_id.replace(/^establishment:/, "site:"), geo = reportingRows.get(siteId);
          const baseSource = { ...record.source }; delete baseSource.source_field;
          if (!geo || record.export_policy !== "local-review-only" || JSON.stringify(baseSource) !== JSON.stringify(geo.source)
            || record.observed_at !== geo.observed_at || record.first_seen !== geo.observed_at || record.last_seen !== geo.observed_at
            || record.assertion_status !== "active" || record.valid_from !== null || record.valid_to !== null
            || record.value_type === "geometry") throw new Error("childcare assertion lost reporting provenance or policy");
          if (![TN_CHILDCARE_DATASET, OH_CHILDCARE_DATASET].includes(record.source.source_id)) validateChildcareAssertionContract(record, geo);
          if (["site.address", "establishment.name", "establishment.source-status"].includes(record.predicate)) {
            const expected = record.predicate === "site.address" ? geo.address : record.predicate === "establishment.name" ? geo.names[0].raw : geo.source_status;
            if (JSON.stringify(record.value) !== JSON.stringify(expected)) throw new Error("childcare assertion differs from geographic evidence");
          }
          if (record.predicate === "site.reported-location" && (record.value.latitude !== geo.location.latitude || record.value.longitude !== geo.location.longitude)) throw new Error("childcare location assertion differs");
          if (record.predicate === "establishment.source-affiliation" && (record.value.parent_company !== null || record.value.ownership_verified !== false)) throw new Error("childcare ownership inference");
          if (!reportingAssertions.has(siteId)) reportingAssertions.set(siteId, []);
          reportingAssertions.get(siteId).push(record);
          if (record.source.source_id === TN_CHILDCARE_DATASET) tnAssertionPaths.set(record.assertion_id, artifact.path);
        }
        ...
        if (!isChildcare && ["establishment.source-status", "establishment.la-active-business-status", "organization.irs-eo-source-status", "organization.ct-registration-status", "organization.de-current-license-status", "organization.co-registration-status", "organization.wa-lni-active-contractor-status", "organization.or-registration-status", "brand.or-registration-status", "organization.ia-registration-status", "organization.ny-active-extract-status", "organization.fl-active-quarterly-status", "organization.pa-active-registration-dataset-status", "organization.il-registration-status"].includes(record.predicate) && !allowedSourceStatuses.has(record.value?.value)) {
        ...
        if (record.source.policy_id === "il-business-registry") {
          const identity = record.subject_entity_id.match(/^organization:il_sos_(corporation|llc)_(\d{8})$/);
          const registrationProfileSourceFields = "entity_type_code|jurisdiction_code|intent_code|incorporation_date|extended_date|transaction_date|purpose_code|management_type|jurisdiction_organized_code|organized_date|dissolution_date|status_date|source_flags";
          const allowedPredicates = new Map([
            ["organization.legal-name", { valueType: "string", sourceFields: "legal_name" }],
            ["organization.il-registration-status", { valueType: "object", sourceFields: "status_code" }],
            ["organization.il-registration-profile", { valueType: "object", sourceFields: registrationProfileSourceFields }],
            ["organization.il-annual-report-evidence", { valueType: "object", sourceFields: "current_report_run_date|current_report_paid_date" }],
            ["organization.records-office-address", { valueType: "address", sourceFields: "records_office_street|records_office_city|records_office_jurisdiction|records_office_zip" }],
            ["organization.external-identifier", { valueType: "identifier", sourceFields: "file_number" }],
            ["organization.records-office-zip-code", { valueType: "string", sourceFields: "records_office_zip" }],
            ["organization.records-office-zcta", { valueType: "object", sourceFields: "records_office_zip" }],
          ]);
          const allowedSourceFields = new Set([
            "legal_name", "status_code", "entity_type_code", "jurisdiction_code", "intent_code", "incorporation_date",
            "extended_date", "transaction_date", "purpose_code", "management_type", "jurisdiction_organized_code",
            "organized_date", "dissolution_date", "status_date", "source_flags", "current_report_run_date",
            "current_report_paid_date", "records_office_street", "records_office_city", "records_office_jurisdiction",
            "records_office_zip", "file_number",
          ]);
          const sourceFields = String(record.source.source_field ?? "").split("|");
          const expectedSourceId = identity ? `illinois-sos-${identity[1]}-daily-files` : null;
          const predicateContract = allowedPredicates.get(record.predicate);
          if (!identity || record.source.source_id !== expectedSourceId || record.source.source_record_id !== identity[2]
            || !predicateContract || record.value_type !== predicateContract.valueType || record.source.source_field !== predicateContract.sourceFields
            || record.export_policy !== "local-review-only") {
            throw new Error(`Illinois assertion lost its subject, predicate, provenance, or local-review policy ${record.assertion_id}`);
          }
          if (sourceFields.some((field) => !allowedSourceFields.has(field))) throw new Error(`Illinois excluded or unsupported source field leaked for ${record.assertion_id}`);
          if ((identity[1] === "corporation" && record.predicate.startsWith("organization.records-office-"))
            || (identity[1] === "llc" && record.predicate === "organization.il-annual-report-evidence")) {
            throw new Error(`Illinois assertion crossed its corporation/LLC field boundary ${record.assertion_id}`);
          }
          if (record.predicate === "organization.il-registration-status"
            && (!["00", "01"].includes(record.value?.code)
              || record.value?.label !== (record.value.code === "00" ? "Goodstanding" : "Reinstated")
              || record.value?.value !== "listed-goodstanding-or-reinstated-in-illinois-sos-daily-files"
              || record.value?.semantics !== "source-registration-status-not-independent-proof-of-current-operations-or-a-physical-site")) {
            throw new Error(`Illinois assertion has an unsupported registration status ${record.assertion_id}`);
          }
          if (record.predicate === "organization.il-registration-status") illinoisStatusAssertionCount += 1;
          if (record.predicate === "organization.il-registration-profile" && record.value?.entity_kind !== identity[1]) {
            throw new Error(`Illinois registration profile has the wrong entity kind ${record.assertion_id}`);
          }
          if (record.predicate === "organization.external-identifier"
            && (record.value?.type !== "il_sos_file_number" || record.value?.value !== identity[2] || record.value?.entity_kind !== identity[1])) {
            throw new Error(`Illinois external identifier does not match its subject ${record.assertion_id}`);
          }
          if (record.predicate === "organization.records-office-address"
            && record.value?.address_scope !== "secretary-of-state-records-office-address-not-verified-physical-operating-site") {
            throw new Error(`Illinois records-office address overstates its scope ${record.assertion_id}`);
          }
        }
  ...
  for (const artifact of relationshipArtifacts) {
    try {
      ...
      const count = await forEachGzipRecord(path.join(releaseDirectory, artifact.path), (record) => {
        ...
        verifyOhCanonical(record, "relationships", artifact.path);
        if (artifact.path.includes("unassigned") && !(tnReportingSupported && tnEntities.has(record.subject_entity_id) && tnEntities.has(record.object_entity_id))
          && !(ohReportingSupported && isOhEntity(record.subject_entity_id) && isOhEntity(record.object_entity_id))) throw new Error("unassigned relationship must be supported reporting member");
        if ((tnEntities.has(record.subject_entity_id) || tnEntities.has(record.object_entity_id)) && record.source.source_id !== TN_CHILDCARE_DATASET) throw new Error("TN relationship source does not own endpoint");
        if (childcareSourceIds.has(record.source.source_id)) {
          const geo = reportingRows.get(record.object_entity_id);
          if (!geo || record.relationship_type !== "located_at" || record.subject_entity_id !== geo.establishment_entity_id
            || record.observed_at !== geo.observed_at || JSON.stringify(record.source) !== JSON.stringify(geo.source)
            || reportingRelationships.has(geo.site_entity_id)) throw new Error("invalid childcare located-at relationship");
          const expectedRelationship = { schema_version: "1.0.0",
            relationship_id: `relationship:${createHash("sha256").update(JSON.stringify(["located_at", geo.establishment_entity_id, geo.site_entity_id, geo.source.source_release_id, geo.source.source_record_id])).digest("hex").slice(0, 32)}`,
            relationship_type: "located_at", subject_entity_id: geo.establishment_entity_id, object_entity_id: geo.site_entity_id,
            status: "active", valid_from: null, valid_to: null, observed_at: geo.observed_at, confidence: 1, source: geo.source };
          if (!isDeepStrictEqual(record, expectedRelationship)) throw new Error("invalid childcare relationship contract");
          reportingRelationships.add(geo.site_entity_id);
          if (record.source.source_id === TN_CHILDCARE_DATASET) { tnRelationships.set(geo.site_entity_id, record); tnRelationshipPaths.set(geo.site_entity_id, artifact.path); }
        }
        ...
        if (record.source.policy_id === "il-business-registry") throw new Error(`Illinois records-office-address record created a relationship ${record.relationship_id}`);
  ...
  for (const [siteId, geo] of reportingRows) {
    const rows = [...(reportingAssertions.get(siteId) ?? [])].sort((a, b) => a.assertion_id.localeCompare(b.assertion_id));
    if (geo.source.source_id === OH_CHILDCARE_DATASET) {
      if (digest(JSON.stringify(rows)) !== geo.evidence.assertions_sha256 || !reportingRelationships.has(siteId)) failures.push({ path: "reporting/location-evidence", reason: "Ohio assertion or relationship membership mismatch" });
      continue;
    }
    if (geo.source.source_id === TN_CHILDCARE_DATASET) {
      try {
        ...
        if (tnEntityPaths.get(siteId) !== `entities/physical-sites/prefix=${prefix}.jsonl.gz`
          || tnEntityPaths.get(geo.establishment_entity_id) !== `entities/establishments/prefix=${prefix}.jsonl.gz`
          || rows.some(row => tnAssertionPaths.get(row.assertion_id) !== `assertions/prefix=${prefix}.jsonl.gz`)
          || tnRelationshipPaths.get(siteId) !== `relationships/prefix=${prefix}.jsonl.gz`) throw new Error("TN canonical partition membership mismatch");
      } catch (error) { failures.push({ path: "reporting/location-evidence", reason: error.message }); }
      continue;
    }
    const requiredPredicates = ["site.address", "site.zip-code", "site.reported-location", "site.source-geography", "establishment.name", "establishment.source-status", "establishment.source-classification", "establishment.source-affiliation", "establishment.external-identifier"];
    if (requiredPredicates.some((predicate) => rows.filter((row) => row.predicate === predicate).length !== 1)
      || rows.filter((row) => row.predicate === "site.external-identifier").length > 1) failures.push({ path: "reporting/location-evidence", reason: "childcare assertion roster differs from source adapter" });
    if (digest(JSON.stringify(rows)) !== geo.evidence.assertions_sha256 || !reportingRelationships.has(siteId)) failures.push({ path: "reporting/location-evidence", reason: "childcare assertions or relationships do not reproduce geographic evidence" });
  }
  if (tnEntities.size !== tnRows.length * 2) failures.push({ path: "entities", reason: "TN entity membership does not match reporting rows" });
  if (reportingRows.size && !String(manifest.export_policy).includes("local-review-only")) failures.push({ path: "manifest.json", reason: "childcare local-review policy lost" });
  ...
  if (illinoisStatusAssertionCount !== illinoisOrganizationCount) {
    failures.push({ path: "manifest.json", reason: "Illinois registration-status assertion count does not reconcile" });
  }
  ...
  if (illinoisOrganizationCount > 0
    && !String(manifest.export_policy ?? "").includes("local-review-only")) {
    failures.push({ path: "manifest.json", reason: "Illinois Business Registry record-level and aggregate local-review policy is missing from the registry export policy" });
  }
  ...
  try {
    ...
    if (siteTotal + unassignedRows.length !== manifest.coverage.physical_sites) throw new Error("ZIP physical-site counts do not reconcile");
    for (const [datasetId, key, field] of [["ma-licensed-center-based-childcare", "ma_childcare_centers", "ma_childcare_center_site_count"], ["nj-licensed-childcare-centers", "nj_childcare_centers", "nj_childcare_center_site_count"], [TN_CHILDCARE_DATASET, "tn_childcare_centers", "tn_childcare_center_site_count"], [OH_CHILDCARE_DATASET, "oh_childcare_centers", "oh_childcare_center_site_count"]]) {
      for (const row of rows) {
        const expected = [...reportingRows.values()].filter((entry) => entry.source.source_id === datasetId && entry.zip_code === row.zip_code);
        if ((row.registry_coverage[field] ?? 0) !== expected.length) throw new Error("childcare ZIP counts do not reconcile");
        const contribution = row.source_contributions?.[key];
        if (childcareDependencies.some((entry) => entry.dataset_id === datasetId)
          && (!contribution || contribution.reported_center_count !== expected.length || contribution.record_level_distribution !== "local-review-only"
            || contribution.active_business_verified !== false
            || (expected.length && (contribution.source_release_id !== expected[0].source.source_release_id || contribution.observed_at !== (datasetId === OH_CHILDCARE_DATASET ? ohInput?.source.observedAt : expected[0].observed_at))))) throw new Error("childcare ZIP source evidence differs");
      }
    }
    ...
    const ilBusinessOrganizationTotal = rows.reduce((sum, row) => sum + (row.registry_coverage.il_business_registry_organization_records_office_address_count ?? 0), 0);
    if (ilBusinessOrganizationTotal !== (manifest.coverage.il_business_registry_eligible_llc_records_office_addresses ?? 0)) throw new Error("ZIP Illinois records-office-address counts do not reconcile");
    ...
    const expectedZipDistributionPolicy = illinoisIntegrated || reportingRows.size > 0
      ? "local-review-only"
      : uspsDependency
        ? uspsDenominator.distribution_policy
        : "public-source-layer";
    if (zipArtifact.distribution_policy !== expectedZipDistributionPolicy) {
      throw new Error("ZIP artifact does not inherit the most restrictive Illinois or USPS distribution policy");
    }
  } catch (error) {
    failures.push({ path: zipArtifact?.path ?? "derived/zip-coverage.jsonl", reason: `ZIP coverage validation failed: ${error.message}` });
  }

  if (ohReportingSupported && ohInput) {
    try {
      const summaryArtifacts = manifest.artifacts.filter(a => a.path === "derived/source-contributions.json");
      if (summaryArtifacts.length !== 1 || summaryArtifacts[0].artifact_type !== "registry-source-contribution-summary") throw new Error("Ohio source summary artifact missing or duplicate");
      const summaryPath = path.join(releaseDirectory, "derived/source-contributions.json");
      const { ohioBoundedRead } = await import("./oh-childcare-release.mjs");
      const summaryRaw = await ohioBoundedRead(summaryPath, 2_000_000);
      if (summaryRaw.length !== summaryArtifacts[0].bytes || digest(summaryRaw) !== summaryArtifacts[0].sha256) throw new Error("Ohio source summary integrity changed");
      const summary = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(summaryRaw));
      if (!isDeepStrictEqual(summary.oh_childcare_centers, childcareSourceSummary({ ...ohInput, datasetId: OH_CHILDCARE_DATASET }))) throw new Error("Ohio source summary differs from verified input");
      const proof = await ohModule.verifyOhChildcareGeographicMembership(ohRows, ohInput.verificationContext);
      if (manifest.coverage.oh_childcare_center_sites !== proof.rows || manifest.coverage.oh_childcare_center_sites_with_zip !== proof.withSourceZip
        || manifest.coverage.oh_childcare_center_sites_without_zip !== proof.withoutSourceZip || manifest.coverage.reporting_location_evidence_without_zip !== unassignedRows.length
        || !isDeepStrictEqual(manifest.coverage.oh_childcare_missing_zip_reasons, proof.quality.zip_unavailable_reasons)) throw new Error("Ohio source and ZIP counts differ");
      for (const kind of Object.keys(ohExpected)) if (ohSeen[kind].size !== ohExpected[kind].size) throw new Error(`Ohio ${kind} membership incomplete`);
    } catch (error) { failures.push({ path: "reporting/location-evidence", reason: error.message }); }
  }
  if (failures.length) {
