...
import { replayMiChildcareAcquisition } from "./mi-childcare-acquisition.mjs";
import { normalizeMiChildcareFeature, MI_CHILDCARE_TRANSFORMATION } from "./mi-childcare-normalization.mjs";
import { MI_CHILDCARE_LAYER, MI_CHILDCARE_WHERE } from "./mi-childcare-preflight.mjs";

const DATASET = "mi-licensed-childcare-centers";
const POLICY_SHA256 = "c3efbc31f7c13c1b4def1a0667e4490e78e0fc4ba74e00b63597b8f6567340e0";
const POLICY_PATH = path.join(APP_ROOT, "config/source-policies/mi-childcare-local-review.json");
...
function requireValue(ok, label) { if (!ok) throw new Error(`Michigan childcare release rejected: ${label}.`); }
...
async function policyFor(signal) {
  ...
  return { profile: "mi-childcare-local-review@1.0.0", configuration_sha256: POLICY_SHA256, ...policy };
}
...
async function derive(evidence, runId, signal) {
  signal?.throwIfAborted(); await yieldLoop(); const acquired = replayMiChildcareAcquisition(evidence, { signal });
  const { features, source } = acquired;
  const sourceReleaseId = `mi-childcare-${sha(json({ layer_url: source.layer_url, where: source.where,
  ...
  for (const [index, feature] of features.entries()) {
    ...
    try { normalized.push(normalizeMiChildcareFeature(feature, { runId, sourceReleaseId, observedAt: source.observed_at,
      itemModifiedEpochMs: source.item_modified_epoch_ms })); }
    catch (error) {
      if (error.code !== "MI_CHILDCARE_RECORD_REJECTED") throw error;
  ...
  const quality = { missing_source_zip: normalized.filter(row => row.physical_address.zip_code === null).length,
    missing_source_point: normalized.filter(row => row.geocode.latitude === null).length, coordinate_reference_unverified: normalized.length,
    governed_geographic_assignment_eligible: 0 };
  return { acquired, normalized, quarantine, sourceReleaseId, quality, counts: { selected: features.length, accepted: normalized.length, quarantined: quarantine.length } };
}
async function contents(result, signal) {
  return [await lines(result.acquired.features, signal), await lines(result.normalized, signal), await lines(result.quarantine, signal),
    `${json(result.acquired.evidence)}\n`, Buffer.from(result.acquired.publisher_metadata.before.base64, "base64")];
}
...
function manifestFor(runId, result, artifacts, policy, processedAt) {
  return { schema_version: "1.0.0", dataset_id: DATASET, connector_id: DATASET, connector_version: "1.0.0", transformation_version: MI_CHILDCARE_TRANSFORMATION,
    run_id: runId, release_id: `mi-childcare-${runId}`, source_release_id: result.sourceReleaseId, status: "published-partial", observed_at: result.acquired.source.observed_at, processed_at: processedAt,
    source_url: MI_CHILDCARE_LAYER, source_filter: MI_CHILDCARE_WHERE, policy, counts: result.counts, quality: result.quality, quarantine_max_fraction: 0.05,
    mode: "offline-retained-acquisition-release", acquisition_performed: false, normalization_policy_status: "proposed-not-approved",
    policy_status: "pending-review", nationalReportingIntegrated: false,
    scope: "Michigan publisher-listed DC/Center source rows only; other care types excluded, operating and license status unknown",
    claims: { active_business_verified: false, license_dates_verified: false, unique_business_identity_verified: false, national_coverage_complete: false,
      ...
      coordinate_reference_verified: false, governed_geographic_assignment_eligible: false, source_freshness_verified: false },
...
export async function verifyMiChildcareRelease(manifestPath, options = {}) {
  ...
  requireValue(uuid.test(manifest.run_id) && manifest.release_id === `mi-childcare-${manifest.run_id}`, "release identity");
...
export async function buildMiChildcareRelease(options = {}) {
  strictOptions(options, ["evidence", "outputRoot", "signal", "now", "logger"]);
  const { evidence, outputRoot = path.join(APP_ROOT, "data/business-sources", DATASET), logger = () => {}, now = () => new Date(), signal } = options;
  ...
  const snapshot = structuredClone(evidence), acquired = replayMiChildcareAcquisition(snapshot, { signal });
  const processedAt = now().toISOString(); requireValue(processedAt >= acquired.source.observed_at, "processing clock precedes source observation");
  ...
  const staging = path.join(stagingRoot, runId), release = path.join(releasesRoot, `mi-childcare-${runId}`), pointerTemp = path.join(root, `.current-${runId}.tmp`);
  ...
  function inspection() { return Object.assign(new Error("Michigan childcare publication ownership changed; inspection required. Foreign paths and prior releases preserved."), { code: "MI_CHILDCARE_INSPECTION_REQUIRED" }); }
  ...
  try {
    ...
    await logger("verify"); await verifyMiChildcareRelease(path.join(staging, "manifest.json"), { signal });
    ...
    try {
      ...
      requireValue(json(Object.keys(prior).sort()) === json(["dataset_id", "release_id", "manifest", "manifest_sha256"].sort())
        && prior.dataset_id === DATASET && typeof prior.release_id === "string" && uuid.test(prior.release_id.slice("mi-childcare-".length))
        && prior.release_id.startsWith("mi-childcare-") && prior.manifest === `releases/${prior.release_id}/manifest.json`
    ...
    const verification = await verifyMiChildcareRelease(path.join(staging, "manifest.json"), { signal });
