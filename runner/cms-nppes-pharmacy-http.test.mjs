...
function harness(query, method = 'GET') {
  const output = {};
  const response = {};
  const url = new URL(`http://localhost/api/business-map/pharmacies${query}`);
...
test('pharmacy API accepts bounded read-only filters', async () => {
  const testCase = harness('?state=TX&zip=75001&query=Main&limit=5');
  ...
  assert.deepEqual(testCase.output.query, { state: 'TX', zip: '75001', query: 'Main', limit: '5' });
});
