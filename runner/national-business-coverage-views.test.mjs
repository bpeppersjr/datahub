...
test("publishes and verifies governed national through ZIP coverage views", async (context) => {
  ...
  if(process.env.DATAHUB_TEST_CMS_RETAINED==='1')await context.test('CMS retained coverage admission conserves full cohort without Ohio or denominator migration',{timeout:180000},async()=>{
    const cms=await import('./cms-hospital-registry-input.mjs'),{APP_ROOT}=await import('./paths.mjs'),input=await cms.loadCmsHospitalRegistryInput(path.join(APP_ROOT,'config/cms-hospital-retained-selection.json'));
    const registryFile=path.join(registryRelease,'manifest.json'),original=await readFile(registryFile),originalFetch=globalThis.fetch;globalThis.fetch=()=>assert.fail('No network');
    try{const m=JSON.parse(original);m.publisher.version='2.12.0';m.created_at=new Date().toISOString();m.export_policy='local-review-only';m.dependencies=[cms.cmsHospitalRegistryDependency(input)];m.cms_hospital_directory_reporting=cms.cmsHospitalRegistryDeclaration(input);m.coverage.cms_hospital_directory_rows=input.records.length;m.artifacts.push(await writeArtifact(registryRelease,cms.CMS_HOSPITAL_REGISTRY_PATH,jsonLines(input.records),{artifact_type:cms.CMS_HOSPITAL_REGISTRY_ARTIFACT,record_count:input.records.length,export_policy:'local-review-only'}));await writeFile(registryFile,json(m));
      const built=await buildNationalBusinessCoverageViews({registryPointerPath:registry.pointerPath,geographyPointerPath:geography.pointerPath,crosswalkPointerPath:crosswalk.pointerPath,resolutionPointerPath:resolution.pointerPath,benchmarkPointerPath:benchmark.pointerPath,nonemployerPointerPath:nonemployer.pointerPath,outputRoot:path.join(root,'cms-coverage'),logger(){}});
      const file=path.join(built.releaseDirectory,'manifest.json');await verifyNationalBusinessCoverageViewsRelease(file);assert.equal(built.manifest.publisher.version,'2.8.0');assert.deepEqual(built.manifest.coverage,result.manifest.coverage);
      const readRows=async name=>(await readFile(path.join(built.releaseDirectory,name),'utf8')).trim().split('\n').map(JSON.parse),national=await readRows('views/national.jsonl'),groups=await readRows('reporting/cms-hospitals/state-zips.jsonl');assert.equal(national.find(r=>r.scope==='registry-union').cms_hospital_directory_reporting.directory_rows,5419);assert.equal(national.find(r=>r.scope==='50-states-and-dc').cms_hospital_directory_reporting.directory_rows,5354);assert.equal(built.manifest.cms_hospital_directory_reporting.counts.territory_retained_directory_rows,65);assert.equal(groups.reduce((n,r)=>n+r.directory_rows,0),5419);assert.ok((await readRows('views/counties.jsonl')).every(r=>r.cms_hospital_directory_reporting.directory_rows===null));
      const originalManifest=await readFile(file);for(const mutate of [v=>v.cms_hospital_directory_reporting.registry_manifest_sha256='f'.repeat(64),v=>v.cms_hospital_directory_reporting.counts.all_retained_directory_rows++,v=>delete v.cms_hospital_directory_reporting]){const changed=JSON.parse(originalManifest);mutate(changed);await writeFile(file,json(changed));await assert.rejects(verifyNationalBusinessCoverageViewsRelease(file));}await writeFile(file,originalManifest);
      const extension=await import('./cms-hospital-coverage-extension.mjs'),savedPath=path.join(built.releaseDirectory,'evidence/registry-manifest.json'),saved=await readFile(savedPath);let checkpoint=false;
      try{await assert.rejects(extension.verifyCmsHospitalCoverageWithTestHook(built.manifest,built.releaseDirectory,async stage=>{assert.equal(stage,'before-final-rechecks');checkpoint=true;await writeFile(savedPath,'{}');}));assert.equal(checkpoint,true);}finally{await writeFile(savedPath,saved);}
      await extension.verifyCmsHospitalCoverageExtension(built.manifest,built.releaseDirectory);
    }finally{globalThis.fetch=originalFetch;await writeFile(registryFile,original);await rm(path.join(registryRelease,'reporting/cms-hospitals'),{recursive:true,force:true});}
  });

  if(process.env.DATAHUB_TEST_NURSING_COVERAGE==='1')await context.test('nursing-only and combined hospital coverage conserve independent cohorts without Ohio',{timeout:180000},async()=>{
    const nursing=await import('./cms-nursing-home-registry-input.mjs'),hospital=await import('./cms-hospital-registry-input.mjs'),{APP_ROOT}=await import('./paths.mjs'),originalFetch=globalThis.fetch;globalThis.fetch=()=>assert.fail('No network');
    const registryFile=path.join(registryRelease,'manifest.json'),original=await readFile(registryFile);
    try{const ni=await nursing.loadCmsNursingHomeRegistryInput(path.join(APP_ROOT,'config/cms-nursing-home-retained-selection.json')),hi=await hospital.loadCmsHospitalRegistryInput(path.join(APP_ROOT,'config/cms-hospital-retained-selection.json'));
      for(const combined of [false,true]){const m=JSON.parse(original);m.publisher.version='2.12.0';m.created_at=new Date().toISOString();m.export_policy='local-review-only';m.dependencies=[nursing.cmsNursingHomeRegistryDependency(ni)];m.cms_nursing_home_directory_reporting=nursing.cmsNursingHomeRegistryDeclaration(ni);m.coverage.cms_nursing_home_directory_rows=ni.records.length;m.artifacts.push(await writeArtifact(registryRelease,nursing.CMS_NURSING_HOME_REGISTRY_PATH,jsonLines(ni.records),{artifact_type:nursing.CMS_NURSING_HOME_REGISTRY_ARTIFACT,record_count:ni.records.length,export_policy:'local-review-only'}));
        if(combined){m.dependencies.push(hospital.cmsHospitalRegistryDependency(hi));m.cms_hospital_directory_reporting=hospital.cmsHospitalRegistryDeclaration(hi);m.coverage.cms_hospital_directory_rows=hi.records.length;m.artifacts.push(await writeArtifact(registryRelease,hospital.CMS_HOSPITAL_REGISTRY_PATH,jsonLines(hi.records),{artifact_type:hospital.CMS_HOSPITAL_REGISTRY_ARTIFACT,record_count:hi.records.length,export_policy:'local-review-only'}));}await writeFile(registryFile,json(m));
        const built=await buildNationalBusinessCoverageViews({registryPointerPath:registry.pointerPath,geographyPointerPath:geography.pointerPath,crosswalkPointerPath:crosswalk.pointerPath,resolutionPointerPath:resolution.pointerPath,benchmarkPointerPath:benchmark.pointerPath,nonemployerPointerPath:nonemployer.pointerPath,outputRoot:path.join(root,combined?'both-cms':'nursing-coverage'),logger(){}}),file=path.join(built.releaseDirectory,'manifest.json');await verifyNationalBusinessCoverageViewsRelease(file);assert.equal(built.manifest.publisher.version,'2.8.0');assert.deepEqual(built.manifest.coverage,result.manifest.coverage);assert.equal(built.manifest.cms_nursing_home_directory_reporting.counts.all_retained_directory_rows,14690);assert.equal(built.manifest.cms_nursing_home_directory_reporting.counts.state_dc_retained_directory_rows,14680);assert.equal(built.manifest.cms_nursing_home_directory_reporting.counts.territory_retained_directory_rows,10);assert.equal(built.manifest.cms_hospital_directory_reporting?.counts.all_retained_directory_rows,combined?5419:undefined);
        const saved=await readFile(file);for(const field of combined?['cms_nursing_home_directory_reporting','cms_hospital_directory_reporting']:['cms_nursing_home_directory_reporting']){const changed=JSON.parse(saved);delete changed[field];await writeFile(file,json(changed));await assert.rejects(verifyNationalBusinessCoverageViewsRelease(file));}await writeFile(file,saved);
      }
    }finally{globalThis.fetch=originalFetch;await writeFile(registryFile,original);await rm(path.join(registryRelease,'reporting/cms-nursing-homes'),{recursive:true,force:true});await rm(path.join(registryRelease,'reporting/cms-hospitals'),{recursive:true,force:true});}
  });
