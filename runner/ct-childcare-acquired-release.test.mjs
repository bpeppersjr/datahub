...
test('CT acquired builder rejects native transport overrides, missing transports and unsafe options before requests',async()=>{
  let requests=0;const fetchImpl=async()=>{requests++;throw Error('unexpected');};
  await assert.rejects(buildCtChildcareAcquiredRelease({fetchImpl}));
  await assert.rejects(buildCtChildcareAcquiredReleaseWithTransport({}));
  await assert.rejects(buildCtChildcareAcquiredReleaseWithTransport({fetchImpl,url:'https://example.invalid'}));
  await assert.rejects(buildCtChildcareAcquiredReleaseWithTransport({fetchImpl,outputRoot:APP_ROOT}));
  const signal=AbortSignal.abort();
  await assert.rejects(buildCtChildcareAcquiredReleaseWithTransport({fetchImpl,signal}),{name:'AbortError'});
  assert.equal(requests,0);
});

test('CT immutable acquisition persists before queries, excludes overlap, replays offline and preserves failure/cancellation boundaries',async()=>{
  const root=await mkdtemp(path.join(APP_ROOT,'data/tmp/ct-acquired-test-'));
  try{
    await Promise.all([
      (async()=>{
        ...
        const fixture=createCtChildcareFixture({mutate:async(value,kind)=>{
        ...
        const running=buildCtChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot});
        const atGate=await Promise.race([ready.then(()=>true),running.then(()=>false)]);assert.equal(atGate,true);
        try{
          let duplicateRequests=0;
          await assert.rejects(buildCtChildcareAcquiredReleaseWithTransport({outputRoot,fetchImpl:async()=>{duplicateRequests++;throw Error('duplicate');}}),{code:'EEXIST'});
        ...
        const forgedMode=JSON.parse(manifestBytes);forgedMode.execution_mode='fixed-native-fetch';await writeFile(result.manifest_path,JSON.stringify(forgedMode)+'\n');await assert.rejects(readCtChildcareAcquiredEvidence(result.manifest_path));await writeFile(result.manifest_path,manifestBytes);
        const oldFetch=globalThis.fetch;globalThis.fetch=()=>assert.fail('Offline replay must not request data');
        try{
          const read=await readCtChildcareAcquiredEvidence(result.manifest_path);
          assert.deepEqual(read.verification,result);assert.equal(read.evidence.before_preflight.source.address_count,1);
        }finally{globalThis.fetch=oldFetch;}
        const cli=await runFile(process.execPath,['scripts/verify-ct-childcare-acquired.mjs','--manifest',result.manifest_path],{cwd:APP_ROOT,windowsHide:true});
        ...
        const changed=JSON.parse(original);changed.observation.payload[0].name='Rehashed synthetic substitution';
        ...
        await assert.rejects(readCtChildcareAcquiredEvidence(result.manifest_path));
        await writeFile(journalPath,original);await writeFile(result.manifest_path,manifestBytes);
        assert.equal((await readCtChildcareAcquiredEvidence(result.manifest_path)).verification.record_count,2);
        ...
        await assert.rejects(buildCtChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot:path.join(root,'alias')}));
        await assert.rejects(buildCtChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot:directory}));
      })(),
      (async()=>{
        const outputRoot=path.join(root,'failed');
        const fixture=createCtChildcareFixture({mutate:(_value,kind)=>kind==='page'?new Response(null,{status:503}):undefined});
        await assert.rejects(buildCtChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot}));
        const [job]=await readdir(path.join(outputRoot,'jobs'));
      ...
      (async()=>{
        ...
        const fixture=createCtChildcareFixture({mutate:(_value,kind)=>{if(kind==='baseline-ids')controller.abort();}});
        await assert.rejects(buildCtChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot,signal:controller.signal}),{name:'AbortError'});
...
test('CT acquired verification CLI rejects invalid arguments without acquisition',async()=>{
  for(const args of [[],['--manifest'],['--manifest','relative.json'],['--url','https://example.invalid']])
    await assert.rejects(runFile(process.execPath,['scripts/verify-ct-childcare-acquired.mjs',...args],{cwd:APP_ROOT,windowsHide:true}));
});
