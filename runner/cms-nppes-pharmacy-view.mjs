...
async function loadJsonArtifact(directory, manifest, relativePath, expectedKind, maxBytes = 100_000_000) {
  ...
  const bytes = await readFile(path.resolve(directory, relativePath));
...
async function loadJsonLinesArtifact(directory, manifest, relativePath, maxBytes = 30_000_000) {
  const artifact = (manifest.artifacts ?? []).find((item) => item.path === relativePath);
...
async function loadGeometry({ geographyPointerPath, expectedRelease = GEOGRAPHY_RELEASE, expectedManifestSha256 = GEOGRAPHY_MANIFEST_SHA256, expectedPointerSha256 = GEOGRAPHY_POINTER_SHA256, requiredZctas = new Set() }) {
  const pointerBuffer = await readFile(geographyPointerPath);
  ...
  const [stateIndex, stateGeo] = await Promise.all([
    loadJsonLinesArtifact(directory, manifest, 'derived/index/states.jsonl'),
    loadJsonArtifact(directory, manifest, 'source/states.geojson', 'state', 8_000_000),
  ]);
  ...
  const zctaIndex = await loadJsonLinesArtifact(directory, manifest, 'derived/index/zctas.jsonl', 30_000_000);
  ...
  for (const prefix of prefixes) {
    ...
    const partition = await loadJsonArtifact(directory, manifest, relativePath, 'zcta', 45_000_000);
    ...
    for (const feature of partition.value.features) {
      ...
      const indexed = zctaIndexByCode.get(code);
  ...
  const finalPointer = await readFile(geographyPointerPath); const finalManifest = await readFile(manifestPath);
  ...
  return { pins: { pointer_sha256: sha256(pointerBuffer), manifest_sha256: sha256(manifestBuffer), release_id: manifest.release_id, state_index: { path: 'derived/index/states.jsonl', bytes: stateIndex.artifact.bytes, sha256: stateIndex.artifact.sha256 }, state_geometry: { path: 'source/states.geojson', bytes: stateGeo.artifact.bytes, sha256: stateGeo.artifact.sha256 }, zcta_index: { path: 'derived/index/zctas.jsonl', bytes: zctaIndex.artifact.bytes, sha256: zctaIndex.artifact.sha256 }, zcta_geometry: prefixes.map((prefix) => { const artifact = manifest.artifacts.find((item) => item.path === `source/zctas/prefix=${prefix}.geojson`); return { path: artifact.path, bytes: artifact.bytes, sha256: artifact.sha256 }; }) }, stateFeatures, zctaFeatures };
}

export function createCmsNppesPharmacyView({ pointerPath = DEFAULT_POINTER, geographyPointerPath = DEFAULT_GEOGRAPHY_POINTER, geographyRelease = GEOGRAPHY_RELEASE, geographyManifestSha256 = GEOGRAPHY_MANIFEST_SHA256, geographyPointerSha256 = GEOGRAPHY_POINTER_SHA256 } = {}) {
  let cached;
  async function load() {
    if (cached) return cached;
    cached = (async () => {
      const pointerBuffer = await readFile(pointerPath); const pointer = JSON.parse(pointerBuffer.toString('utf8')); const manifestPath = path.resolve(path.dirname(pointerPath), pointer.manifest); const manifestBuffer = await readFile(manifestPath);
      ...
      const stateAggregate = JSON.parse(await readFile(path.join(releaseDirectory, stateArtifact.path), 'utf8')); const zipAggregate = (await readFile(path.join(releaseDirectory, zipArtifact.path), 'utf8')).split(/\r?\n/).filter(Boolean).map(JSON.parse); const rows = [];
      ...
      const sourceManifestPath = path.resolve(APP_ROOT, manifest.dependencies.nppes_organizations_manifest.path); const sourceManifestBuffer = await readFile(sourceManifestPath); if (sha256(sourceManifestBuffer) !== manifest.dependencies.nppes_organizations_manifest.sha256) throw new Error('NPPES source manifest hash drifted.'); const sourceManifest = JSON.parse(sourceManifestBuffer.toString('utf8')); const geoDependency = (sourceManifest.dependencies ?? []).find((item) => item.dataset_id === 'us-census-geography'); if (!geoDependency || geoDependency.release_id !== geographyRelease || geoDependency.manifest_sha256 !== geographyManifestSha256) throw new Error('NPPES source is bound to a different Census geography release.');
      const requiredZctas = new Set(rows.filter((row) => row.address?.state && MAP_STATES.has(row.address.state) && row.geography?.zcta_match_status === '2020-zcta-polygon-available').map((row) => row.geography.zcta_geoid)); const geography = await loadGeometry({ geographyPointerPath, expectedRelease: geographyRelease, expectedManifestSha256: geographyManifestSha256, expectedPointerSha256: geographyPointerSha256, requiredZctas });
      ...
      return { pointer, manifest, manifestPath, pointerBuffer, manifestBuffer, rows, stateRows: stateAggregate.rows, zipRows: zipAggregate, geography, byState, byZipState, states, stateFeatures, stateMedian, territoryRows, unmatchedRows };
  ...
  return {
    async get({ level = 'states', state, zip, query, limit = '25' } = {}) {
      ...
      const view = await load(); const selectedState = safeState ? view.byState.get(safeState) : null; const featureRows = level === 'states' ? [] : (selectedState ? [...selectedState.zcta_codes].map((code) => ({ code, ...view.byZipState.get(`${safeState}:${code}`) })).filter((row) => row.source_record_count > 0) : []); const zctaMedian = median(featureRows.map((row) => row.source_record_count));
      const features = level === 'states' ? view.stateFeatures : featureRows.map((row) => { const geometryFeature = view.geography.zctaFeatures.get(row.code); return { ...geometryFeature, properties: { ...geometryFeature.properties, level: 'zcta', state: safeState, source_record_count: row.source_record_count, exact_zcta_mapped_count: row.exact_zcta_mapped_count, nonpolygon_count: row.nonpolygon_count, unassigned_count: row.unassigned_count, relative_peer_median_percent: zctaMedian ? row.source_record_count / zctaMedian * 100 : null } }; });
