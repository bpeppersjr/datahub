...
import { TN_CHILDCARE_REGISTRY_TRANSFORMATION } from "./tn-childcare-registry-adapter.mjs";
...
function evidenceValid(e, source, observedAt) {
  check(exact(e, ["manifest_sha256", "policy_profile", "release_id", "input_feature_sha256", "attribution", "transformation_version", "normalized_provenance",
    ...
    "publisher_metadata_required", "publisher_notices_required", "legal_approval", "export_authorized", "assertions_sha256"]), "evidence keys");
  ...
  check(p && hex(e.manifest_sha256) && hex(e.assertions_sha256) && hex(e.input_feature_sha256) && e.input_feature_sha256 === p.input_feature_sha256
    && e.policy_profile === "tn-childcare-local-review@1.0.0" && e.release_id === `tn-childcare-recovered-${source.ingest_run_id}`
    ...
    && utc(e.processed_at) && e.processed_at >= observedAt && e.publisher_metadata_required === true && e.publisher_notices_required === true
  ...
  check(e.assertion_status_semantics === "current source assertion representation, not operating-business status"
    ...
    && e.temporal_scope === "source observation preserved; processed_at is recovery time, not a refreshed source observation"
...
export function validateTnChildcareGeographicEvidence(row) {
  ...
  check(row.schema_version === "1.0.0" && row.category === "childcare" && row.identity_matching_eligible === false && row.export_policy === "local-review-only"
  ...
  check(exact(source, ["source_id", "source_release_id", "source_record_id", "ingest_run_id", "transformation_version", "policy_id"])
    ...
    && source.transformation_version === `${TN_CHILDCARE_REPROCESS_TRANSFORMATION} -> ${TN_CHILDCARE_REGISTRY_TRANSFORMATION}`, "source contract");
  evidenceValid(row.evidence, source, row.observed_at);
...
export function createTnChildcareGeographicEvidence(contribution) {
  ...
  const row = validateTnChildcareGeographicEvidence({ schema_version: "1.0.0", site_entity_id: address.subject_entity_id, establishment_entity_id: name.subject_entity_id,
    ...
    observed_at: address.observed_at, identity_matching_eligible: false, export_policy: "local-review-only", category: "childcare", names: [{ raw: name.value }], source_status: structuredClone(status.value), evidence });
