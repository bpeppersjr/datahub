...
import { normalizeMdChildcareAcquisition } from './md-childcare-normalization.mjs';

export const MD_CHILDCARE_NORMALIZED_RELEASE_VERSION='md-childcare-normalized-release@1.0.0';
const UUID=/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/;
const LIMITS={'normalized.jsonl':200_000_000,'quarantine.jsonl':30_000_000,'summary.json':1_000_000};
const PINS={connector_sha256:'aff3e92723456753b1790aed2155603798ea7a61bba69650df08ca5b8136fe52',policy_sha256:'ebf6e4917dabd5347a175ecdf881909a1f9232a52cd37f330d6a84a7c324d940'};
const encode=value=>Buffer.from(JSON.stringify(value)+'\n');
const hash=value=>createHash('sha256').update(value).digest('hex');
const check=value=>{if(!value)throw Error('Maryland normalized release rejected.');};
...
async function configuration(signal){
  for(const [file,pin]of [['config/connectors/md-childcare-centers-normalization.json',PINS.connector_sha256],['config/source-policies/md-childcare-centers-internal.json',PINS.policy_sha256]])check(hash(JSON.stringify(await readJson(path.join(APP_ROOT,file),100000,signal)))===pin);
}
const claims=()=>({export_policy:'internal',source_authenticity_verified:false,native_execution_independently_verified:false,public_export_authorized:false,national_reporting_integrated:false,app_enrolled:false,scheduled:false,physical_site_verified:false,identity_matching_eligible:false,address_role:'reported-address-unverified',source_key_stability_verified:false,credential_deduplication_applied:false,geocodes_inferred:false,current_operations_verified:false,coordinate_basis:'provider-transformed-epsg4326-point-not-address-verified',publisher_cohort_date:'2026-02-13'});
const inspection=()=>Object.assign(Error('Maryland normalization output may exist; preserve and inspect retained evidence before retry.'),{code:'MD_CHILDCARE_NORMALIZATION_INCOMPLETE'});
...
function manifestFor(runId,processedAt,input,summary,artifacts){return {schema_version:MD_CHILDCARE_NORMALIZED_RELEASE_VERSION,run_id:runId,status:'normalized-internal-evidence',source_release_id:`md-childcare-acquisition-${input.verification.run_id}`,processed_at:processedAt,configuration:{...PINS},acquired:sourceBinding(input),summary,claims:claims(),artifacts};}
async function replay(input,runId,processedAt,signal){
  check(time(processedAt)&&processedAt>=input.evidence.finished_at);
  return normalizeMdChildcareAcquisition(input.evidence,{runId,sourceReleaseId:`md-childcare-acquisition-${input.verification.run_id}`,processedAt},{signal});
}
async function inspect(manifestPath,signal,candidate=false){
  ...
  const input=await readMdChildcareAcquiredEvidence(manifest.acquired.manifest_path,{signal});check(same(manifest.acquired,sourceBinding(input)));
  ...
  const finalInput=await readMdChildcareAcquiredEvidence(manifest.acquired.manifest_path,{signal});check(same(finalInput.verification,input.verification)&&same(finalInput.evidence,input.evidence));
...
export async function readMdChildcareNormalizedRelease(manifestPath,options={}){optionsOnly(options,['signal']);return inspect(manifestPath,options.signal);}
export const verifyMdChildcareNormalizedRelease=readMdChildcareNormalizedRelease;

export async function buildMdChildcareNormalizedRelease(acquiredManifestPath,options={}){
  optionsOnly(options,['outputRoot','signal','processedAt']);const {signal,outputRoot=path.join(APP_ROOT,'data/business-sources/md-childcare/normalized')}=options;
  signal?.throwIfAborted();await configuration(signal);
  const input=await readMdChildcareAcquiredEvidence(acquiredManifestPath,{signal}),processedAt=options.processedAt??new Date().toISOString();check(time(processedAt)&&processedAt>=input.evidence.finished_at);
