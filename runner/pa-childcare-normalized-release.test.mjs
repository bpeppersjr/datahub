...
test('PA normalized release and CLI reject invalid modes and early cancellation',async()=>{
  const signal=AbortSignal.abort();
  await assert.rejects(buildPaChildcareNormalizedRelease('unread',{signal}),{name:'AbortError'});
  await assert.rejects(buildPaChildcareNormalizedRelease('unread',{url:'https://example.invalid'}));
  await assert.rejects(readPaChildcareNormalizedRelease('unread',{url:'https://example.invalid'}));
  for(const args of [[],['--acquired'],['--verify','relative.json'],['--acquired','x','--verify','y'],['--url','https://example.invalid']])
    await assert.rejects(execute(process.execPath,['scripts/reprocess-pa-childcare.mjs',...args],{cwd:APP_ROOT,windowsHide:true}));
});

test('PA normalized releases conserve source rows, replay offline and reject rehashed substitutions without repulling',paFixtureOptions,async()=>{
  const root=await mkdtemp(path.join(APP_ROOT,'data/tmp/pa-normalized-test-'));
  try{
    const fixture=createPaChildcareFixture({count:3,mutate:(rows,kind)=>{
      if(kind!=='page')return;
      rows.forEach(row=>{row.facility_address='123 Synthetic Street';row.facility_city='Harrisburg';row.facility_state='Pennsylvania';});
      rows[0].facility_zip_code='';rows[1].facility_zip_code='01234-0067';delete rows[2].facility_address;
    }});
    const acquired=await buildPaChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot:path.join(root,'acquired')});
    ...
    try{
      first=await buildPaChildcareNormalizedRelease(acquired.manifest_path,{outputRoot});
      second=await buildPaChildcareNormalizedRelease(acquired.manifest_path,{outputRoot});
      const result=await readPaChildcareNormalizedRelease(first.manifest_path);
      ...
      assert.equal(result.records[0].physical_address.zip_code,null);assert.equal(result.records[0].physical_address.zip4,null);
      assert.equal(result.records[0].geocode.latitude,null);
      assert.equal(result.records[1].physical_address.zip_code,'01234');assert.equal(result.records[1].physical_address.postal_code,'01234');assert.equal(result.records[1].physical_address.zip4,'0067');
      assert.equal(result.records[1].quality.geographic_boundary_verified,false);
      ...
      await assert.rejects(buildPaChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:path.dirname(acquired.manifest_path)}));
      await assert.rejects(buildPaChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:APP_ROOT}));
      await assert.rejects(buildPaChildcareNormalizedRelease(acquired.manifest_path,{processedAt:'2000-01-01T00:00:00.000Z',outputRoot}));
      await mkdir(path.join(root,'alias-target'));await symlink(path.join(root,'alias-target'),path.join(root,'alias'),'junction');
      await assert.rejects(buildPaChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:path.join(root,'alias')}));
      await writeFile(path.join(outputRoot,'.owner.lock'),'foreign ownership');
      await assert.rejects(buildPaChildcareNormalizedRelease(acquired.manifest_path,{outputRoot}),{code:'EEXIST'});
    ...
    try{
      await assert.rejects(buildPaChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:cancelledRoot,signal:controller.signal}),{name:'AbortError'});
    ...
    const verified=await execute(process.execPath,['scripts/reprocess-pa-childcare.mjs','--verify',first.manifest_path],{cwd:APP_ROOT,windowsHide:true});
    assert.equal(JSON.parse(verified.stdout).record_count,2);
    const rebuilt=await execute(process.execPath,['scripts/reprocess-pa-childcare.mjs','--acquired',acquired.manifest_path,'--output',path.join(root,'cli-normalized')],{cwd:APP_ROOT,windowsHide:true});
    ...
    await writeFile(first.manifest_path,JSON.stringify(manifest)+'\n');await assert.rejects(readPaChildcareNormalizedRelease(first.manifest_path));
    await writeFile(recordsPath,recordsBytes);await writeFile(first.manifest_path,manifestBytes);
    assert.equal((await readPaChildcareNormalizedRelease(first.manifest_path)).verification.record_count,2);
    const corrupt=JSON.parse(inputBytes);corrupt.observations[1].payload[0].facility_name='Changed acquired row';await writeFile(acquisitionPath,JSON.stringify(corrupt)+'\n');
    await assert.rejects(readPaChildcareNormalizedRelease(first.manifest_path));await writeFile(acquisitionPath,inputBytes);
    assert.equal((await readPaChildcareNormalizedRelease(first.manifest_path)).verification.record_count,2);
    await writeFile(path.join(directory,'unexpected.txt'),'extra evidence');await assert.rejects(readPaChildcareNormalizedRelease(first.manifest_path));
