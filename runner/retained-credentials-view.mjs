...
export function createRetainedCredentialsView({loader=loadMnConstructionReportingEnrollment}={}) {
  let pending;
  return {async get(params=new URLSearchParams()) {
    for(const key of params.keys())if(!['state','offset','limit'].includes(key)||params.getAll(key).length!==1)invalid();
