...
export function deriveCurrentMatrixAuthorizationWave(projection, projectionManifestBytes, projectionManifest, projectionArtifactSha256) {
  ...
  const selected = sorted.slice(0, CURRENT_MATRIX_AUTHORIZATION_WAVE_SIZE);
  const expected = ["IL", "MS", "AR", "KY", "HI", "KS", "NV", "UT", "WA", "OK"];
  if (JSON.stringify(selected.map((row) => row.state_abbreviation)) !== JSON.stringify(expected) || ["AK", "DC", "TX"].some((code) => selected.some((row) => row.state_abbreviation === code))) fail("current wave is not the exact top ten historical priorities among current gaps");
  ...
  const held = sorted.slice(CURRENT_MATRIX_AUTHORIZATION_WAVE_SIZE);
  return {
    ...
    scope: {
      ...
      conservation_total: states.length + held.length,
      acquisition_authorized: false,
...
export async function verifyCurrentMatrixAuthorizationWave(manifestPath, { allowOwnedStaging = false } = {}) {
  ...
  const expected = await loadCurrentWave();
