...
test('CO normalized release and CLI reject invalid modes and early cancellation',async()=>{
  const signal=AbortSignal.abort();
  await assert.rejects(buildCoChildcareNormalizedRelease('unread',{signal}),{name:'AbortError'});
  await assert.rejects(buildCoChildcareNormalizedRelease('unread',{url:'https://example.invalid'}));
  await assert.rejects(readCoChildcareNormalizedRelease('unread',{url:'https://example.invalid'}));
  for(const args of [[],['--manifest'],['--manifest','relative.json'],['--manifest','x','--verify','y'],['--url','https://example.invalid']])
    await assert.rejects(execute(process.execPath,['scripts/verify-co-childcare-normalized.mjs',...args],{cwd:APP_ROOT,windowsHide:true}));
});

test('CO normalized releases conserve source rows, replay offline and reject rehashed substitutions without repulling',async()=>{
  const root=await mkdtemp(path.join(APP_ROOT,'data/tmp/co-normalized-test-'));
  try{
    const fixture=createCoChildcareFixture({count:3,mutate:(rows,kind)=>{
      if(kind!=='page'||rows.length===0)return;
      rows[0].zip='invalid';rows[1].zip='01234-0067';rows[0].street_address='';
      for(const field of ['provider_name','street_address'])rows[2][field]='x'.repeat(20000);
    }});
    const acquired=await buildCoChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot:path.join(root,'acquired')});
    ...
    try{
      first=await buildCoChildcareNormalizedRelease(acquired.manifest_path,{outputRoot});
      second=await buildCoChildcareNormalizedRelease(acquired.manifest_path,{outputRoot});
      const result=await readCoChildcareNormalizedRelease(first.manifest_path);
      ...
      assert.equal(result.records[1].reported_address.state,'CO');assert.equal(result.records[1].reported_address.country,null);
      assert.equal(result.records[1].source.selected_fields.zip,'01234-0067');
      ...
      await assert.rejects(buildCoChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:path.dirname(acquired.manifest_path)}));
      await assert.rejects(buildCoChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:APP_ROOT}));
      await assert.rejects(buildCoChildcareNormalizedRelease(acquired.manifest_path,{processedAt:'2000-01-01T00:00:00.000Z',outputRoot}));
      await mkdir(path.join(root,'alias-target'));await symlink(path.join(root,'alias-target'),path.join(root,'alias'),'junction');
      await assert.rejects(buildCoChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:path.join(root,'alias')}));
      await writeFile(path.join(outputRoot,'.owner.lock'),'foreign ownership');
      await assert.rejects(buildCoChildcareNormalizedRelease(acquired.manifest_path,{outputRoot}),{code:'EEXIST'});
    ...
    try{
      await assert.rejects(buildCoChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:cancelledRoot,signal:controller.signal}),{name:'AbortError'});
    ...
    try{
      await assert.rejects(buildCoChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:uncertainRoot,signal:publicationAbort.signal}),{code:'CO_CHILDCARE_NORMALIZATION_INCOMPLETE'});
      ...
      await assert.rejects(readCoChildcareNormalizedRelease(path.join(retained,'manifest.json')));
      assert.deepEqual(await readFile(acquisitionPath),inputBytes);
    }finally{fsPromises.unlink=originalUnlink;syncBuiltinESMExports();}
    const offlineImport='data:text/javascript,'+encodeURIComponent("globalThis.fetch=()=>process.exit(91)");
    const independent=await execute(process.execPath,['--import',offlineImport,'scripts/verify-co-childcare-normalized.mjs','--manifest',first.manifest_path],{cwd:APP_ROOT,windowsHide:true});
    assert.equal(JSON.parse(independent.stdout).quarantine_count,1);
    await assert.rejects(execute(process.execPath,['scripts/verify-co-childcare-normalized.mjs'],{cwd:APP_ROOT,windowsHide:true}));
    ...
    await writeFile(first.manifest_path,JSON.stringify(manifest)+'\n');await assert.rejects(readCoChildcareNormalizedRelease(first.manifest_path));
    await writeFile(recordsPath,recordsBytes);await writeFile(first.manifest_path,manifestBytes);
    assert.equal((await readCoChildcareNormalizedRelease(first.manifest_path)).verification.record_count,2);
    ...
    await assert.rejects(readCoChildcareNormalizedRelease(first.manifest_path));await writeFile(acquisitionPath,inputBytes);
    assert.equal((await readCoChildcareNormalizedRelease(first.manifest_path)).verification.record_count,2);
    await writeFile(path.join(directory,'unexpected.txt'),'extra evidence');await assert.rejects(readCoChildcareNormalizedRelease(first.manifest_path));
