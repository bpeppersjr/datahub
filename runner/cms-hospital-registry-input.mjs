...
export async function verifyCmsHospitalRegistryExtension(manifest,directory,{signal}={}){
 ...
 await canonical(directory,{signal});const owner=await lstat(directory,{bigint:true}),selection=path.join(APP_ROOT,'config/cms-hospital-retained-selection.json'),input=await loadCmsHospitalRegistryInput(selection,{signal}),artifact=validateCmsHospitalRegistryEnvelope(manifest,input),meter={},records=[];
 ...
 const after=await loadCmsHospitalRegistryInput(selection,{signal});check(same(cmsHospitalRegistryDeclaration(after),cmsHospitalRegistryDeclaration(input)));const finalMeter={};for await(const ignored of readLines(path.join(directory,CMS_HOSPITAL_REGISTRY_PATH),input.source.selected_artifact.bytes,signal,finalMeter)){void ignored;}check(finalMeter.bytes===meter.bytes&&finalMeter.sha256===meter.sha256&&finalMeter.records===meter.records&&['ino','dev','size','mtimeNs','ctimeNs'].every(key=>finalMeter.identity[key]===meter.identity[key]));const finalOwner=await lstat(directory,{bigint:true});check(finalOwner.isDirectory()&&!finalOwner.isSymbolicLink()&&finalOwner.ino===owner.ino&&finalOwner.dev===owner.dev);signal?.throwIfAborted();return input;
}
