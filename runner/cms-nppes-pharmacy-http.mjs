export async function cmsNppesPharmacyHttp(request, response, url, view, json) {
  ...
  const allowed = new Set(['level', 'state', 'zip', 'query', 'limit']);
  ...
  try { json(response, 200, await view.get({ level: url.searchParams.get('level') || 'states', state: url.searchParams.get('state'), zip: url.searchParams.get('zip'), query: url.searchParams.get('query'), limit: url.searchParams.get('limit') })); }
