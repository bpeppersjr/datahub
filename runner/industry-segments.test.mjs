...
test('PA childcare enrollment selects one fixed app task without broadening source scope',async()=>{
  ...
  assert.equal(plan.taskCount,1);assert.equal(plan.tasks[0].sourceId,'state-pa-childcare-centers');assert.equal(plan.tasks[0].state,'PA');
  assert.equal(plan.tasks[0].script,'scripts/build-pa-childcare.mjs');assert.ok(plan.tasks[0].prerequisites.includes('config/connectors/pa-childcare-centers-app.json'));
  assert.ok(plan.gaps.some(g=>g.state==='WI'));assert.ok(plan.warnings.some(w=>w.includes('do not promote national reporting')));
  assert.equal(buildIndustryPlan(config,{industries:['childcare'],states:['PA']}).taskCount,1);
...
test('MD childcare enrollment selects one bounded app job without manufacturing nationwide coverage',async()=>{
  const config=await loadIndustryConfig();
  const selected={industries:['childcare'],states:['MD','WI'],sourceIds:['state-md-childcare-centers']};
  const plan=buildIndustryPlan(config,selected);
  assert.equal(plan.taskCount,1);assert.equal(plan.tasks[0].state,'MD');
  assert.equal(plan.tasks[0].script,'scripts/build-md-childcare.mjs');
  assert.equal(plan.tasks[0].prerequisites.length,6);
  assert.ok(plan.tasks[0].prerequisites.includes('config/connectors/md-childcare-centers-app.json'));
  assert.ok(plan.gaps.some(g=>g.state==='WI'));
  assert.ok(plan.warnings.some(w=>w.includes('February 13, 2026')));
  assert.ok(plan.warnings.some(w=>w.includes('does not promote national reporting')));
  assert.equal(buildIndustryPlan(config,{industries:['childcare'],states:['MD']}).taskCount,1);
  assert.throws(()=>buildIndustryPlan(config,{...selected,states:['WI']}),/not applicable/);
  assert.throws(()=>buildIndustryPlan(config,{...selected,industries:['retail-consumer']}),/not applicable/);
  assert.equal(config.sources['state-md-childcare-centers'].state_filter_supported,false);
});

test('VT childcare enrollment selects one fixed app worker and preserves other-state gaps',async()=>{
  const config=await loadIndustryConfig();
  const selected={industries:['childcare'],states:['VT','WI'],sourceIds:['state-vt-childcare-centers']};
  const plan=buildIndustryPlan(config,selected);
  assert.equal(plan.taskCount,1);assert.equal(plan.tasks[0].state,'VT');
  assert.equal(plan.tasks[0].script,'scripts/build-vt-childcare.mjs');
  assert.equal(plan.tasks[0].prerequisites.length,6);
  assert.ok(plan.tasks[0].prerequisites.includes('config/connectors/vt-childcare-centers-app.json'));
  assert.ok(plan.gaps.some(g=>g.state==='WI'));
  assert.ok(plan.warnings.some(w=>w.includes('Exact reporting period remains unknown')));
  assert.ok(plan.warnings.some(w=>w.includes('does not promote national reporting')));
  assert.equal(buildIndustryPlan(config,{industries:['childcare'],states:['VT']}).taskCount,1);
  assert.throws(()=>buildIndustryPlan(config,{...selected,states:['WI']}),/not applicable/);
  assert.throws(()=>buildIndustryPlan(config,{...selected,industries:['retail-consumer']}),/not applicable/);
  assert.equal(config.sources['state-vt-childcare-centers'].state_filter_supported,false);
});

test('CO childcare enrollment selects only the bounded Colorado app worker',async()=>{
  const config=await loadIndustryConfig(),selected={industries:['childcare'],states:['CO','WI'],sourceIds:['state-co-childcare-centers']};
  const plan=buildIndustryPlan(config,selected);
  assert.equal(plan.taskCount,1);assert.equal(plan.tasks[0].state,'CO');
  assert.equal(plan.tasks[0].script,'scripts/build-co-childcare.mjs');
  assert.equal(plan.tasks[0].prerequisites.length,7);
  assert.ok(plan.tasks[0].prerequisites.includes('config/co-childcare-source-contract.json'));
  assert.ok(plan.gaps.some(g=>g.state==='WI'));
  assert.ok(plan.warnings.some(w=>w.includes('does not promote national reporting')));
  assert.equal(buildIndustryPlan(config,{industries:['childcare'],states:['CO']}).taskCount,1);
  assert.throws(()=>buildIndustryPlan(config,{...selected,states:['WI']}),/not applicable/);
  assert.throws(()=>buildIndustryPlan(config,{...selected,industries:['retail-consumer']}),/not applicable/);
