...
import { normalizePaChildcareAcquisition } from './pa-childcare-normalization.mjs';

export const PA_CHILDCARE_NORMALIZED_RELEASE_VERSION='pa-childcare-normalized-release@1.0.0';
const UUID=/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/;
const LIMITS={'normalized.jsonl':200_000_000,'quarantine.jsonl':30_000_000,'summary.json':1_000_000};
const PINS={connector_sha256:'111bbf0aaee18e8b7b6eb09b6f364ac132a3b1d847e9771042db7e4cb4957a8d',policy_sha256:'421da74613a3390c16e29e921854c563be83030bdaaa0c949a8f3f1aa0955f06'};
const encode=value=>Buffer.from(JSON.stringify(value)+'\n');
const hash=value=>createHash('sha256').update(value).digest('hex');
const check=value=>{if(!value)throw Error('Pennsylvania normalized release rejected.');};
...
async function configuration(signal){
  for(const [file,pin]of [['config/connectors/pa-childcare-centers-normalization.json',PINS.connector_sha256],['config/source-policies/pa-childcare-centers-internal.json',PINS.policy_sha256]])check(hash(JSON.stringify(await readJson(path.join(APP_ROOT,file),100000,signal)))===pin);
}
const claims=()=>({export_policy:'internal',source_authenticity_verified:false,native_execution_independently_verified:false,public_export_authorized:false,national_reporting_integrated:false,app_enrolled:false,scheduled:false});
const inspection=()=>Object.assign(Error('Pennsylvania normalization output may exist; preserve and inspect retained evidence before retry.'),{code:'PA_CHILDCARE_NORMALIZATION_INCOMPLETE'});
const sourceBinding=input=>({manifest_path:input.verification.manifest_path,manifest_sha256:input.verification.manifest_sha256,run_id:input.verification.run_id});
function manifestFor(runId,processedAt,input,summary,artifacts){return {schema_version:PA_CHILDCARE_NORMALIZED_RELEASE_VERSION,run_id:runId,status:'normalized-internal-evidence',source_release_id:`pa-childcare-acquisition-${input.verification.run_id}`,processed_at:processedAt,configuration:{...PINS},acquired:sourceBinding(input),summary,claims:claims(),artifacts};}
async function replay(input,runId,processedAt,signal){
  check(time(processedAt)&&processedAt>=input.evidence.finished_at);
  return normalizePaChildcareAcquisition(input.evidence,{runId,sourceReleaseId:`pa-childcare-acquisition-${input.verification.run_id}`,processedAt},{signal});
}
async function inspect(manifestPath,signal,candidate=false){
  ...
  check(exact(manifest,['schema_version','run_id','status','source_release_id','processed_at','configuration','acquired','summary','claims','artifacts'])&&manifest.run_id===runId&&exact(manifest.acquired,['manifest_path','manifest_sha256','run_id'])&&Array.isArray(manifest.artifacts)&&manifest.artifacts.length===3);
  const input=await readPaChildcareAcquiredEvidence(manifest.acquired.manifest_path,{signal});check(same(manifest.acquired,sourceBinding(input)));
  ...
  const finalInput=await readPaChildcareAcquiredEvidence(manifest.acquired.manifest_path,{signal});check(same(finalInput.verification,input.verification)&&same(finalInput.evidence,input.evidence));
...
export async function readPaChildcareNormalizedRelease(manifestPath,options={}){optionsOnly(options,['signal']);return inspect(manifestPath,options.signal);}

export async function buildPaChildcareNormalizedRelease(acquiredManifestPath,options={}){
  optionsOnly(options,['outputRoot','signal','processedAt']);const {signal,outputRoot=path.join(APP_ROOT,'data/business-sources/pa-childcare-centers/normalized')}=options;
  signal?.throwIfAborted();await configuration(signal);
  const input=await readPaChildcareAcquiredEvidence(acquiredManifestPath,{signal}),processedAt=options.processedAt??new Date().toISOString();check(time(processedAt)&&processedAt>=input.evidence.finished_at);
