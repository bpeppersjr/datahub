...
import { acquireCtChildcareWithTransport, acquireCtChildcareNative, replayCtChildcareAcquisition, assertCtChildcareAcquisitionConfiguration } from './ct-childcare-acquisition.mjs';

const VERSION='ct-childcare-acquired-release@1.0.0';
const UUID=/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/;
const MAXIMUM=200_000_000;
const bytes=value=>Buffer.from(JSON.stringify(value)+'\n');
const hash=value=>createHash('sha256').update(value).digest('hex');
const check=value=>{if(!value)throw Error('Connecticut acquired evidence rejected.');};
...
const inspection=()=>Object.assign(Error('Connecticut acquisition publication may exist; preserve evidence and inspect before retry.'),{code:'CT_CHILDCARE_PUBLICATION_INCOMPLETE'});
async function derive(prerequisite,evidence,journal,signal){
  const replayed=await replayCtChildcareAcquisition(evidence,{signal});
...
export async function readCtChildcareAcquiredEvidence(manifestPath,options={}){
...
export async function buildCtChildcareAcquiredReleaseWithTransport(options={}){
...
export async function buildCtChildcareAcquiredRelease(options={}){
  optionsOnly(options,['outputRoot','signal']);return build(options,'fixed-native-fetch');
}
async function build(options,mode){
  const {signal,now=()=>new Date(),outputRoot=path.join(APP_ROOT,'data/business-sources/ct-childcare/acquired')}=options;
  check(typeof now==='function'&&typeof outputRoot==='string'&&outputRoot===path.resolve(outputRoot));signal?.throwIfAborted();
  await assertCtChildcareAcquisitionConfiguration(signal);
  ...
  try{
    ...
    const evidence=await(mode==='fixed-native-fetch'?acquireCtChildcareNative(callbacks):acquireCtChildcareWithTransport(callbacks));
    ...
    const {verification}=await readCtChildcareAcquiredEvidence(path.join(directory,'manifest.json'));
