...
async function loadRegistryProfiles(pointerPath) {
  ...
  if (manifest.dataset_id !== "national-business-registry" || manifest.status !== "published-partial"
    || !["1.2.0", "1.3.0", "1.4.0", "1.5.0"].includes(manifest.publisher?.version) || manifest.complete_national_business_registry !== false) {
    throw new Error("A compatible national business registry 1.2.0 through 1.5.0 partial release with match profiles is required.");
  }
