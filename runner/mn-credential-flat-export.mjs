...
export function validateMnCredentialFlatRequest(value) { return options(value); }
...
async function verify(directory,name,input,synthetic,expectedHash,signal,operationId){
 const mm={},m=await readJson(path.join(directory,name),100000,signal,mm);check(mm.sha256===expectedHash&&m.schema_version===(operationId?'mn-credential-flat-export@1.1.0':VERSION)&&m.run_id===path.basename(directory)&&m.execution_mode===(synthetic?'synthetic-test-input':'verified-retained-input')&&same(m.claims,claims())&&same(m.source,input.source)&&same(m.selection,input.selection));
 check(same(Object.keys(m).sort(),('schema_version run_id execution_mode created_at format fields reported_states source_credential_rows filtered_out_credential_rows credential_rows_written csv_formula_safety source selection claims artifacts'+(operationId?' operation_id':'')).split(' ').sort()));
 if(operationId)check(m.operation_id===operationId);
...
async function build(input,opts,synthetic,hook,revalidate,binding){
 inputCheck(input);const {signal,format,fields,states}=opts,records=input.records.filter(r=>!states.length||states.includes(r.record.reported_address.state));signal?.throwIfAborted();const root=binding?path.join(binding.output,'jobs'):path.join(APP_ROOT,synthetic?'data/tmp/mn-credential-flat-tests':'data/exports/mn-credentials');await canonical(root,{create:true,output:true,signal});const run=randomUUID(),directory=path.join(root,run);await mkdir(directory);const owner=await lstat(directory,{bigint:true}),owned=new Map();let published=false,manifestHash;
 ...
 try{
  ...
  const m={schema_version:binding?'mn-credential-flat-export@1.1.0':VERSION,...(binding?{operation_id:binding.operationId}:{}),run_id:run,execution_mode:synthetic?'synthetic-test-input':'verified-retained-input',created_at:new Date().toISOString(),format,fields,reported_states:states,source_credential_rows:input.records.length,filtered_out_credential_rows:input.records.length-records.length,credential_rows_written:records.length,csv_formula_safety:'apostrophe-prefix',source:input.source,selection:input.selection,claims:claims(),artifacts};
  manifestHash=(await write('manifest.tmp',[JSON.stringify(m)+'\n'],100000)).sha256;
  await hook?.('before-publication',{directory});await verify(directory,'manifest.tmp',input,synthetic,manifestHash,signal,binding?.operationId);
  ...
  await hook?.('after-publication',{directory});await verify(directory,'manifest.json',input,synthetic,manifestHash,undefined,binding?.operationId);return descriptor();
...
function operationBinding(value) {
 ...
 check(typeof value.operationId==='string'&&/^[a-zA-Z0-9_-]{1,120}$/.test(value.operationId)&&typeof value.output==='string'&&value.output===path.resolve(value.output));
 const relative=path.relative(path.join(APP_ROOT,'data'),value.output);
 check(relative&&!relative.startsWith('..')&&!path.isAbsolute(relative)&&path.basename(value.output)==='output'&&path.basename(path.dirname(value.output))===value.operationId);
 return value;
}
function operationManifest(manifest,binding) {
 check(typeof manifest==='string'&&manifest===path.resolve(manifest)&&path.basename(manifest)==='manifest.json'
  &&/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/.test(path.basename(path.dirname(manifest)))
  &&path.dirname(path.dirname(manifest))===path.join(binding.output,'jobs'));
}
export async function exportMnCredentialFlatForOperation(value,bound) {
 const opts=options(value),binding=operationBinding(bound);await canonical(path.dirname(binding.output));
 const input=await nativeInput(opts.signal);return build(input,opts,false,undefined,()=>nativeInput(opts.signal),binding);
}
export async function verifyMnCredentialFlatForOperation(manifest,expectedHash,bound,{signal}={}) {
 const binding=operationBinding(bound);operationManifest(manifest,binding);check(digest(expectedHash));await canonical(path.dirname(manifest),{signal});
 const input=await nativeInput(signal);return verify(path.dirname(manifest),'manifest.json',input,false,expectedHash,signal,binding.operationId);
}
export async function exportMnCredentialFlatForOperationWithTestInput(input,value,bound,hook) {
 const opts=options(value),binding=operationBinding(bound);await canonical(path.dirname(binding.output));
 return build(structuredClone(input),opts,true,hook,undefined,binding);
}
export async function verifyMnCredentialFlatForOperationWithTestInput(manifest,expectedHash,bound,input) {
 const binding=operationBinding(bound);operationManifest(manifest,binding);check(digest(expectedHash));inputCheck(input);
 return verify(path.dirname(manifest),'manifest.json',input,true,expectedHash,undefined,binding.operationId);
}
