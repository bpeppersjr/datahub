...
test('CMS adoption card shows dated scope, separate clocks, restrictions and explicit action only',()=>{
  ...
  assert.match(text(tree),/2 dated hospital directory rows/);assert.match(text(tree),/1 rows in states\/DC/);assert.match(text(tree),/1 territory rows/);assert.match(text(tree),/0 other\/unresolved rows/);assert.match(text(tree),/Original manifest SHA-256/);assert.match(text(tree),/a{64}/);assert.match(text(tree),/Selected artifact SHA-256/);assert.match(text(tree),/b{64}/);assert.match(text(tree),/Current source bytes have not been replayed by this history read/);assert.match(text(tree),/No public redistribution or downloads/);
