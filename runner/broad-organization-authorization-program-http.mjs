...
export async function broadOrganizationAuthorizationProgramHttp(request, response, url, loadView, json) {
  if (request.method !== "GET") {
    json(response, 405, { error: "Authorization program view is read-only." });
    return;
  }
  if ([...url.searchParams.keys()].length > 0) {
    json(response, 400, { error: "Authorization program view does not accept query parameters." });
    return;
  }
  ...
  if ((contentLength !== undefined && contentLength !== "0") || request.headers?.["transfer-encoding"] !== undefined || await hasActualBody(request)) {
    json(response, 400, { error: "Authorization program view accepts an empty GET only." });
    return;
  }
  try {
    json(response, 200, await loadView());
  } catch {
    json(response, 503, { error: "The verified authorization program is unavailable. No action was taken." });
  }
}
