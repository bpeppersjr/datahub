...
/** Read-only verified source-row reporting; no national integration or ZIP inference. */
export async function loadTnChildcareReportingInput(manifestPath, options = {}) {
  ...
  const input = await loadTnChildcareRegistryInput(manifestPath, { signal });
  ...
  for (const [index, contribution] of input.contributions.entries()) {
    ...
    const row = createTnChildcareGeographicEvidence(contribution);
    check(row.export_policy === "local-review-only" && row.identity_matching_eligible === false && row.source.source_id === "tn-dhs-active-childcare-centers"
      ...
      && row.evidence.processed_at === input.source.processedAt && row.evidence.recovery.failed_run_id === input.source.failedRunId,
