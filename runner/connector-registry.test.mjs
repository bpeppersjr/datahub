...
test("loads the complete repository registry deterministically without secret values", async () => {
  ...
  assert.equal(registry.connectorCount, 73);
  assert.deepEqual(registry.get("ut-childcare-centers-app").allowed_hosts, []);
  assert.equal(registry.get("ut-childcare-centers-app").resource_class, "bounded-offline-state-industry-app-job");
  assert.equal(registry.get("ut-childcare-centers-normalization").resource_class, "bounded-offline-pdf-normalization");
  assert.deepEqual(registry.get("ut-childcare-centers-normalization").allowed_hosts, []);
  ...
  assert.equal(registry.get("vt-childcare-centers-acquisition").resource_class, "bounded-state-industry-acquisition");
  ...
  assert.equal(registry.get("md-childcare-preflight").version, "1.0.0");
  assert.equal(registry.get("md-childcare-preflight").resource_class, "bounded-metadata-only");
  assert.equal(registry.get("ct-childcare-centers-app").version, "1.0.0");
  assert.equal(registry.get("ct-childcare-centers-normalization").version, "1.0.0");
  assert.equal(registry.get("ct-childcare-centers-acquisition").version, "1.0.0");
  assert.equal(registry.get("ct-childcare-preflight").version, "1.0.0");
  assert.equal(registry.get("pa-childcare-centers-app").version, "1.0.0");
  ...
  assert.equal(registry.policyProfileCount, 57);
