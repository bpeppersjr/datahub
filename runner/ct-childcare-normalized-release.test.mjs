...
test('CT normalized release and CLI reject invalid modes and early cancellation',async()=>{
  const signal=AbortSignal.abort();
  await assert.rejects(buildCtChildcareNormalizedRelease('unread',{signal}),{name:'AbortError'});
  await assert.rejects(buildCtChildcareNormalizedRelease('unread',{url:'https://example.invalid'}));
  await assert.rejects(readCtChildcareNormalizedRelease('unread',{url:'https://example.invalid'}));
  for(const args of [[],['--acquired'],['--verify','relative.json'],['--acquired','x','--verify','y'],['--url','https://example.invalid']])
    await assert.rejects(execute(process.execPath,['scripts/reprocess-ct-childcare.mjs',...args],{cwd:APP_ROOT,windowsHide:true}));
});

test('CT normalized releases conserve source rows, replay offline and reject rehashed substitutions without repulling',async()=>{
  const root=await mkdtemp(path.join(APP_ROOT,'data/tmp/ct-normalized-test-'));
  try{
    const fixture=createCtChildcareFixture({count:3,mutate:(rows,kind)=>{
      if(kind!=='page')return;
      rows.forEach(row=>{row.address2='123 Synthetic Street';row.city='Hartford';row.statecode='CT';});
      rows[0].zipcode='invalid';rows[1].zipcode='01234-0067';delete rows[0].address2;rows[2].name='x'.repeat(20000);rows[2].address3='x'.repeat(20000);
    }});
    const acquired=await buildCtChildcareAcquiredReleaseWithTransport({fetchImpl:fixture.fetchImpl,outputRoot:path.join(root,'acquired')});
    ...
    try{
      first=await buildCtChildcareNormalizedRelease(acquired.manifest_path,{outputRoot});
      second=await buildCtChildcareNormalizedRelease(acquired.manifest_path,{outputRoot});
      const result=await readCtChildcareNormalizedRelease(first.manifest_path);
      ...
      await assert.rejects(buildCtChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:path.dirname(acquired.manifest_path)}));
      await assert.rejects(buildCtChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:APP_ROOT}));
      await assert.rejects(buildCtChildcareNormalizedRelease(acquired.manifest_path,{processedAt:'2000-01-01T00:00:00.000Z',outputRoot}));
      await mkdir(path.join(root,'alias-target'));await symlink(path.join(root,'alias-target'),path.join(root,'alias'),'junction');
      await assert.rejects(buildCtChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:path.join(root,'alias')}));
      await writeFile(path.join(outputRoot,'.owner.lock'),'foreign ownership');
      await assert.rejects(buildCtChildcareNormalizedRelease(acquired.manifest_path,{outputRoot}),{code:'EEXIST'});
    ...
    try{
      await assert.rejects(buildCtChildcareNormalizedRelease(acquired.manifest_path,{outputRoot:cancelledRoot,signal:controller.signal}),{name:'AbortError'});
    ...
    const verified=await execute(process.execPath,['scripts/reprocess-ct-childcare.mjs','--verify',first.manifest_path],{cwd:APP_ROOT,windowsHide:true});
    assert.equal(JSON.parse(verified.stdout).record_count,2);
    const independent=await execute(process.execPath,['scripts/verify-ct-childcare-normalized.mjs','--manifest',first.manifest_path],{cwd:APP_ROOT,windowsHide:true});assert.equal(JSON.parse(independent.stdout).quarantine_count,1);
    await assert.rejects(execute(process.execPath,['scripts/verify-ct-childcare-normalized.mjs'],{cwd:APP_ROOT,windowsHide:true}));
    const rebuilt=await execute(process.execPath,['scripts/reprocess-ct-childcare.mjs','--acquired',acquired.manifest_path,'--output',path.join(root,'cli-normalized')],{cwd:APP_ROOT,windowsHide:true});
    ...
    await writeFile(first.manifest_path,JSON.stringify(manifest)+'\n');await assert.rejects(readCtChildcareNormalizedRelease(first.manifest_path));
    await writeFile(recordsPath,recordsBytes);await writeFile(first.manifest_path,manifestBytes);
    assert.equal((await readCtChildcareNormalizedRelease(first.manifest_path)).verification.record_count,2);
    const corrupt=JSON.parse(inputBytes);corrupt.observations[1].payload[0].name='Changed acquired row';await writeFile(acquisitionPath,JSON.stringify(corrupt)+'\n');
    await assert.rejects(readCtChildcareNormalizedRelease(first.manifest_path));await writeFile(acquisitionPath,inputBytes);
    assert.equal((await readCtChildcareNormalizedRelease(first.manifest_path)).verification.record_count,2);
    await writeFile(path.join(directory,'unexpected.txt'),'extra evidence');await assert.rejects(readCtChildcareNormalizedRelease(first.manifest_path));
