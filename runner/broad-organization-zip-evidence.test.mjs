...
async function fixture(t) {
  ...
  for (const [state, spec] of Object.entries(SOURCES)) {
    ...
    let address = { street: "1 Example Way", city: "Example", state_code: state, zip_code: "01234", zip4: "5678", eligible_for_us_zip_coverage: true };
    let record = { schema_version: "1.0.0", normalized_record_id: `${state.toLowerCase()}:record:1`, [spec.nameField]: `Example ${state}`, source_status: { status: "source-native status; not current operations" }, provenance: { source_id: spec.normalizedSourceId, source_release_id: spec.sourceReleaseId, transformation_version: spec.transformation }, export_policy: state === "DE" ? "local-review-only" : "public" };
    ...
    if (state === "DE") record.privacy = { record_level_distribution: "local-review-only" };
    if (state === "OR") {
    ...
    const sourceManifest = { dataset_id: spec.datasetId, release_id: spec.releaseId, ...(state === "IA" ? { complete_source_snapshot: true } : { status: "published" }), artifacts: [{ artifact_type: spec.artifactType, path: artifactPath, bytes: gz.length, sha256: digest(gz), record_count: rows.length }] };
...
test("immutable ZIP derivative preserves eight source identities, ZIP+4, Oregon brands, and DE restrictions", async t => {
  ...
  assert.equal(built.manifest.conservation.input_record_total, 9); assert.equal(built.manifest.conservation.eligible_zip_row_total, 9); assert.equal(built.manifest.conservation.missing_or_ineligible_row_total, 1);
  const verified = await verifyBroadOrganizationZipEvidence(built.releaseDirectory, { root }); assert.equal(verified.status, "verified"); assert.equal(verified.source_count, 8);
  await assert.rejects(buildBroadOrganizationZipEvidence({ root, asOf: new Date("2026-09-22T00:00:00.000Z") }), /already exists/);
  const zipShard = await readFile(path.join(built.releaseDirectory, "derived/organizations/zip-prefix=01.jsonl.gz"));
  const { createGunzip } = await import("node:zlib"); const rows = (await new Promise((resolve, reject) => { const gunzip = createGunzip(), chunks = []; gunzip.on("data", chunk => chunks.push(chunk)); gunzip.on("end", () => resolve(Buffer.concat(chunks).toString("utf8").trim().split("\n").map(JSON.parse))); gunzip.on("error", reject); gunzip.end(zipShard); }));
  ...
  assert.equal(missingPaths.length, 8, "each publisher gets a bounded missing/ineligible shard, including empty shards");
