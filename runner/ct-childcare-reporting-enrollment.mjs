...
const check=value=>{if(!value)throw Error('Connecticut reporting enrollment rejected.');};

export async function loadCtChildcareReportingEnrollment(options={}) {
  ...
  signal?.throwIfAborted();const config=path.join(root,'config/ct-childcare-reporting-enrollment.json'),meter={};let binding;
  ...
  check(binding&&Object.keys(binding).length===3&&binding.schema_version==='ct-childcare-reporting-enrollment@1.0.0'&&/^[a-f0-9]{64}$/.test(binding.app_receipt_sha256)
  ...
  const summary=await summarizeCtChildcareAppJob(receipt,{signal});
  ...
  return {status:'available',sourceId:'ct-oec-childcare-centers',publisherJurisdiction:'CT',enrollmentSha256:meter.sha256,summary};
}

export function projectCtChildcareStateEvidence(enrollment,state) {
  if(enrollment.status!=='available')return {status:enrollment.status,...(enrollment.reason?{reason:enrollment.reason}:{})};
  if(state!=='CT')return {status:'outside-publisher-scope',sourceId:enrollment.sourceId,publisherJurisdiction:'CT'};
  const s=enrollment.summary,entry=s.by_reported_state.find(item=>item.state===state);
  return {status:'verified-retained-cohort',sourceId:enrollment.sourceId,publisherJurisdiction:'CT',reportedAddressState:state,
    ...
    denominator:'accepted source candidate rows in this retained Connecticut cohort; not unique facilities or all U.S. businesses',
