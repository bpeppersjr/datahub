...
test('Nursing and hospital co-selected planner binds both rosters and declarations without fingerprint collisions', {timeout:120000},async t=>{
  ...
  t.mock.module('./cms-hospital-production-input.mjs',{namedExports:{pinCmsHospitalProductionInput:mockPin,cmsHospitalImplementationFiles:async()=>['package.json','package-lock.json']}});
  t.mock.method(globalThis,'fetch',()=>assert.fail('No network'));
  const f=await fixture(t),cms='config/cms.json',mn='config/mn.json',sources={cms:{dataset_id:'cms-nursing-home-general-information',release_id:'fixture-cms',manifest_path:'data/cms/manifest.json'},mn:{dataset_id:'cms-hospital-general-information',release_id:'fixture-mn',manifest_path:'data/mn/manifest.json'}};
  for(const [key,source] of Object.entries(sources)){await json(path.join(f.root,source.manifest_path),{dataset_id:source.dataset_id,release_id:source.release_id});source.manifest_sha256=sha(await readFile(path.join(f.root,source.manifest_path)));await json(path.join(f.root,key==='cms'?cms:mn),source);}await json(path.join(f.root,'package.json'),{});await json(path.join(f.root,'package-lock.json'),{});
...
test('Nursing hospital and MN triple selection checks every declaration', {timeout:120000},async t=>{
  ...
  t.mock.method(globalThis,'fetch',()=>assert.fail('No network'));
  const f=await fixture(t),members=[['cmsNursingHome','cms_nursing_home_directory_reporting','cms-nursing-home-provider-information'],['cmsHospital','cms_hospital_directory_reporting','cms-hospital-general-information'],['mnCredential','mn_construction_credential_reporting','mn-construction-credential-reporting']],options={...f};
  for(const [key,,dataset] of members){const source={dataset_id:dataset,release_id:`fixture-${key}`,manifest_path:`data/${key}/manifest.json`};await json(path.join(f.root,source.manifest_path),{dataset_id:dataset,release_id:source.release_id});source.manifest_sha256=sha(await readFile(path.join(f.root,source.manifest_path)));options[`${key}Selection`]=`config/${key}.json`;await json(path.join(f.root,options[`${key}Selection`]),source);}
