...
/** Verified local-review contributions only; no matching, acquisition or publication. */
export async function loadTnChildcareRegistryInput(manifestPath, options = {}) {
  ...
  const verification = await verifyTnChildcareRecoveredRelease(manifestPath, { signal });
  ...
  requireValue(manifest.transformation_version === "tn-childcare-normalization@1.0.1" && manifest.policy.profile === "tn-childcare-local-review@1.0.0"
    && manifest.policy.export_policy === "local-review-only", "recovered transformation or policy");
  ...
  for (const [index, line] of lines.entries()) {
    ...
    contributions.push(reconcileTnChildcareCenter(JSON.parse(line), { manifest, manifestSha256: verification.manifest_sha256 }));
  }
  // Repeat the complete evidence proof after cooperative processing; never return a mixed snapshot.
  const final = await verifyTnChildcareRecoveredRelease(manifestPath, { signal });
  ...
  return { source: { manifestPath, manifestSha256: verification.manifest_sha256, releaseId: manifest.release_id,
    sourceReleaseId: manifest.source_release_id, observedAt: manifest.observed_at, processedAt: manifest.processed_at,
    transformationVersion: manifest.transformation_version, recoveryVersion: manifest.recovery_version,
    failedRunId: manifest.recovery.failed_run_id, parentManifestSha256: null },
