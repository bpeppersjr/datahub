...
- Complete the handoff with an application operation ID and persisted receipt, not an agent promise or a running agent shell. After accepted dispatch, release the agent to other source/connector work; do not keep an agent slot or a Codex polling loop occupied for ordinary download progress. Return to agent work only for a concrete validation, connector, or recovery issue requiring investigation.
- Before dispatch, distinguish a needed acquisition or authorized refresh from local reprocessing of an already retained, verified release. Reuse the retained release for downstream builds; do not download it again merely to promote it. Do not claim universal freshness reuse, automatic retries, or restart recovery where a connector or scheduler does not implement them.
