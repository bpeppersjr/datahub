...
# Reporting integration evidence — September 12, 2026

The versioned eight-source reporting catalog is now projected from verified selected coverage aggregates, separately from the unchanged six-source acquisition plan. A bounded native read with network denied returned 51 state/district rows and `local-review-only` policy. Illinois has 8/8 represented datasets, including 41,572 ECHO source profiles and 395 FSIS source profiles; all-business completeness remains null. FSIS reference date is 2026-08-24; ECHO's source-updated clock is 2026-08-30T06:36:03Z. These are distinct from observation clocks.

The response identifies catalog/version/hash, selected coverage manifest hash and aggregate hashes; it does not replay raw sources. Legacy six-source projection results retain their original scope/version. Source-only acquisition configuration, production pointers, ZIP4 semantics and public-export restrictions were not changed. Focused reporting/IRS/store tests passed 15/15. The related ZIP auditor now accepts the existing publisher's status-only outside-ZCTA shape while rejecting contradictory identifiers; 10/10 tests and the read-only production summary audit passed. Audit evidence: `data/ui-verification/zip-denominator-reporting-integration-audit.log`.

Final integration verification passed: 1,921 tests, 1,862 passed, 59 skipped, zero failures; lint, web/desktop builds and desktop control-plane smoke passed. TypeScript and production dependency audit passed with zero reported vulnerabilities. Full log: `data/ui-verification/check-eight-source-reporting-final.log`. Desktop UI checks covered 100/200% text, persistence, relaunch and all 51 displayed jurisdictions. The tester ran stop-collector before testing and again before final launch. Final launch succeeded at `2026-09-12T13:20:52.675Z`; authenticated health, coverage, representation and schedule checks returned 200. Representation used `national-reporting-eight@1.0.0`, preserved unknown all-business completeness and local-review-only policy, and no schedules were enabled. The app was left open. These checks do not establish business completeness or native Maine acquisition.
