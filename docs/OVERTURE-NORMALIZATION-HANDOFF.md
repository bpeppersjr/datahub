...
3. **Normalization resource limits.** The legacy gzip reader has no explicit decompressed/line bounds, build and verification maintain whole-source identity sets, and output writers lack byte/disk ceilings. Acquisition limits do not automatically govern these components. Implement bounded input/output and operation-owned storage, with bounded or disk-backed duplicate detection, before advertising a production-scale app normalization worker.
