...
2. **Separate local normalization from promotion.** The legacy builder unconditionally calls its publication function after verification; cancellation checks end before finalization. A managed local normalization operation needs cancellation throughout finalization, an internal retained-result receipt and recovery handling before any current-pointer mutation.
3. **Normalization resource limits.** The legacy gzip reader has since been replaced with the [bounded record reader](OVERTURE-BOUNDED-RECORD-READING.md). Build and verification still maintain whole-source identity sets, and output writers lack byte/disk ceilings. Acquisition limits do not automatically govern these components. Complete bounded output and operation-owned storage, with integration of disk-backed duplicate detection, before advertising a production-scale app normalization worker.
