...
Heatmap Builder exposes this exact release as a separate authenticated county choropleth. A fixed selection pins both the context release and the governed Census geography release; each request verifies the derived artifact and selected state geometry without replaying the 701,010-row upstream source. Users choose either NAICS 23 or 62441 and one of the 50 states or District of Columbia. The map distinguishes usable values, true zero, flagged values, not-published cells, and counties outside the retained native universe. Kalawao County (`15005`) is explicitly outside that universe rather than being counted as a missing selected-industry cell. County percentages use the official same-industry state and national rows and are annual aggregate shares—not market share, collection completeness, or current operations.
