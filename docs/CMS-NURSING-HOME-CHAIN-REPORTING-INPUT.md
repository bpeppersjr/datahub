...
Every output row carries its source manifest path and SHA-256, selected artifact path/hash/row count, failed source run ID, raw CSV path/hash, logical source row, source-record ID and exact CCN. Raw chain name, chain ID, reported facility count and publisher processing date remain available with their lexical status. The view also retains the loader's group reconciliation evidence and source-date/observation context. `validFrom`, `validTo`, `firstSeen` and `lastSeen` are null because this source does not provide those verified temporal facts; the publisher processing-date string is preserved as evidence without parsing it into a date. Summary conservation requires assertion-row count to equal the retained source-row count and chain-ID dispositions to sum to that count.

The explicit claim boundary is that publisher-reported affiliation is not verified legal parenthood, a canonical network, a unique business count, a physical site, or proof of current operation. This is a local-review-only assertion view, not an export or publication artifact. Its synthetic fixture seam always reports `nativeSourceVerified: false`; source-linkage mismatches and malformed page/options input fail closed. Focused tests cover lineage preservation, raw value and temporal evidence, pagination, conservation, tampering, immutability, no network, and the absence of persistence.
