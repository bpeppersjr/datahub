...
ZIP-denominator audit evidence: read-only audit `zip-denominator-audit-d48230b5f34ec2e88b360a25` verifies release-pointer and artifact hashes while keeping Census ZCTA polygons distinct from USPS operational evidence. Production publisher 2.9.0 contains 48,217 ZIP5 rows, including 33,791 governed ZCTA members and 14,385 source-reported ZIP5 values outside that denominator; all 48,217 USPS statuses are unverified and 4,607 lack a reason under the legacy contract. The isolated publisher 2.10.0 candidate contains 48,190 ZIP5 rows, the same 33,791 ZCTA members, 14,358 source-reported values outside the denominator, 4,604 missing USPS reasons, and 48,190 rows missing both the required `postal_code` alias and separate `zip4` field. It fails the 2.10 contract and must be rebuilt and independently verified before any cutover. See [the ZIP denominator audit contract](ZIP-DENOMINATOR-AUDIT.md).
