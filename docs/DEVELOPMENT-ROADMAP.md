...
The next state-business source contract is the [Texas Active Franchise Taxpayers preflight](TX-ACTIVE-FRANCHISE-TAXPAYERS-PREFLIGHT.md). It records official endpoints, 18 source fields, and strict organization/tax-status semantics, but no row-level data has been acquired. Its observed 3.4-million-row scale requires a separate operator-approved large-acquisition gate and it cannot be treated as physical-site or current-operating evidence.
