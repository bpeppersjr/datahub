...
- Remaining integration: add durable scheduling and explicit interrupted-run recovery; reconcile refreshed sources through registry, resolution, benchmark, and coverage dependency checks; continue additional industry/state acquisition and gap closure. The isolated registry, entity-resolution, and benchmark-sample artifacts have now independently verified; coverage remains pending. The benchmark has zero independent labels and its accuracy gate remains false. See [reconciliation evidence and commands](REFRESH-RECONCILIATION.md). Production pointers remain unchanged pending the complete dependency chain and cutover checks.
