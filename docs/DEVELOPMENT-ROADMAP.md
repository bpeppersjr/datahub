...
The [Texas Active Franchise Taxpayers metadata-only connector](TX-ACTIVE-FRANCHISE-TAXPAYERS.md) now enforces the previously recorded [source preflight contract](TX-ACTIVE-FRANCHISE-TAXPAYERS-PREFLIGHT.md). Its live bounded preflight validated all 18 catalog fields, observed `3,435,798` rows and source refresh `2026-08-29T08:25:23Z`, and wrote an immutable receipt proving that it issued zero row requests and acquired no taxpayer records. Bulk acquisition remains unimplemented and default-denied behind a separate exact acknowledgement and rights review. Any future taxpayer address remains administrative-only evidence, never a physical site, geocode, or proof of current operation.
