...
The offline `broad-org-zip` derivative version 1.1 projects nine retained source layers: AK, CO, CT, DE, FL, IA, NY, OR, and PA. It reads their exact retained normalized releases and requires the exact selected national registry release to pin each source release and manifest. Source policy bytes and source transformation versions are included in the immutable manifest and independently rechecked by the verifier. Existing eight-source version 1.0 releases remain independently verifiable through a frozen compatibility contract.

Alaska license rows use qualifying source-reported U.S. physical addresses as organization-address ZIP evidence even when the source-specific provisional-site eligibility flag is false. They remain local-review-only, keep ZIP5 and ZIP+4 separate, and do not establish a physical site, unique business, current operation, or complete Alaska business population. The derivative preserves organization-address and provisional-site counts as distinct measures.
