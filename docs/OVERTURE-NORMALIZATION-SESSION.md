...
There is no new management endpoint or worker CLI in this release. The next integration must enroll this session behind a durable app operation, resolve allowed input IDs itself, verify returned descriptors against dispatched selections, keep artifacts private and mark any recovery/cancellation result inspection-required. Resource hardening of remaining prerequisite/verifier stages is still outstanding. Co*Tive remains local; no cloud deployment, production download, production normalization or promotion occurred.
