...
## Returned-field compatibility repair — September 8, 2026

Subsequent bounded diagnostics isolated two query-only metadata properties: `modelName` exactly matches each selected field name, and `visible` is explicitly `true`. The parser accepts those optional values only. It still rejects mismatched/nested model names, false/nonboolean visibility, unknown metadata including unobserved `sqlType`, altered lengths/nullability, non-null domains/defaults, and invalid business attribute types. Preflight schema, source permissions, ten-field selection, output schema and existing evidence versions are unchanged.

The corrected diagnostic at `data/tmp/oh-first-batch-diagnostic-c221b289-9a91-47c9-84c7-7297dc15e052` retained its first validated feature page in acquisition staging `20c87c4d-87d1-4864-95df-ed1785f7dde7/observation-0001.json`. Its wrapper counted a second attempted feature request but rejected it **before network fetch**. Therefore it downloaded only one feature page and intentionally did not complete acquisition. Four separately gated first-page diagnostics were used in this follow-up; no source records or aliases were printed. Structural property names and fixed comparisons were used to diagnose the mismatch.

Independent read-only review found no concrete gap in the narrow change. New tests reproduce rejection of the publisher-shaped metadata before the fix, verify acceptance afterward, and exercise negative metadata and unchanged attribute-type rules. This is connector compatibility evidence, not proof of complete Ohio business coverage or a completed statewide release. App redispatch must be recorded separately, preserve the original failed operation and check for retained completed acquisitions before starting.

Verification passed all 934 repository tests, lint, web/desktop builds and desktop smoke, with zero production dependency vulnerabilities. The retained diagnostic page separately replayed successfully for 100 selected records. No completed manifest was found in the original failed managed operation or the default Ohio source root. Rollback reverts the parser/test change without deleting evidence; the previous parser would again reject the live metadata and should not be used for unattended Ohio acquisition.
