...
The verifier hashes every artifact, parses every record, checks unique IDs and relationship endpoints, requires assertion provenance and export policy, and reconciles ZIP and manifest counts. For compatible publishers 1.2.0 through 2.0.0 it also requires exactly 100 ZIP2 match-profile partitions, validates their links back to provisional entities and their provenance hashes, and requires exactly one profile per provisional physical site. Its exact partitioned identity index keeps verification bounded for tens of millions of numeric source IDs and digest identities without weakening duplicate detection. It rejects excluded Connecticut, Colorado, Oregon, Iowa, New York, Florida, Pennsylvania, or Los Angeles fields, unsupported site/relationship inference, loss of source or local-review-only policies, unsupported business-completeness claims, and any USPS denominator without a checksummed governed dependency, exact scoped semantics, conservative deliverability flags, and inherited distribution policy.
...
The independently verified publisher-2.0 release `national-business-registry-20260831-210552807Z-93cdcd9e` contains 32,535,165 governed source rows. It publishes 18,073,874 organizations, 116,332 brands, 6,794,612 provisional physical sites, 6,794,612 provisional establishments, one service, 177,084,892 assertions, 9,235,266 relationships, and exactly 6,794,612 location match profiles across 463 checksummed artifacts totaling 10,724,393,992 bytes.

Its 48,142-row ZIP union has record-level source contributions in 47,929 rows. Los Angeles adds 633,332 accepted location accounts across 5,437 ZIPs, each represented as one provisional physical site, establishment, relationship, and location profile; 566,943 have source coordinates. Of 633,782 source rows, 450 are quarantined, including 365 with an invalid or unmapped U.S. ZIP and 85 without a business-location address. Pennsylvania adds 2,347,229 organizations and 2,088,998 eligible reported-business-address contributions from 4,537 source ZIPs; one duplicate filing-number row is collapsed and 2,320 Pennsylvania-address portal geocodes outside broad state bounds remain flagged. Florida adds 4,109,230 organizations and 3,928,280 eligible reported-principal-address contributions across 19,064 source-reported ZIP values; two malformed active rows remain quarantined and 8,698,964 inactive rows are explicitly excluded. New York adds 4,275,497 organizations and 352,234 eligible reported-location/ZIP contributions across 8,653 source ZIPs. Iowa adds 347,200 organizations, 334,176 eligible home-office/ZIP contributions, and 330,664 source-geocoded organization assertions across 9,901 source ZIPs. Oregon adds 443,158 legal-entity registrations as organizations and 116,332 assumed-name registrations as brands, with 559,141 eligible registration/ZIP contributions across 9,177 source ZIPs. The seven state-registry address layers create no physical sites, establishments, relationships, or location profiles; the Los Angeles location-account layer is intentionally modeled differently and remains local-review-only at record level. The release remains explicitly `published-partial`, and `authoritative_current_usps_zip_denominator` remains `null`; a source-reported five-digit value is not represented as a current USPS assignment.

This 2.0 release supersedes the verified 1.9 registry. Its 100 source-preserving ZIP2 match-profile artifacts now include the accepted Los Angeles location accounts without weakening source identity, temporal qualifiers, or the source's local-review-only policy. The USPS connector and optional registry integration are implemented, but this current release still omits USPS evidence until an operator truthfully selects an authorized use basis and publishes a local governed USPS source release.
