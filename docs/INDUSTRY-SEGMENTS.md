...
This prevents simultaneous duplicate acquisition, not sequential redundant refreshes. [App-owned interval scheduling](MANAGED-REFRESH-SCHEDULER.md) now adds deterministic scheduled occurrences and default-disabled API controls. Freshness-based reuse, global resource budgets across disjoint runs, and automatic interrupted-run recovery remain separate work. No live provider request is made by enrolling the protection.
