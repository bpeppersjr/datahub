...
The executable readiness inventory, frozen pointer/manifest hashes, safe source order, current blockers, and registry fail-fast gate are documented in `docs/NORMALIZED-US-POSTAL-MIGRATION.md`. `npm run postal-migration:status` is read-only; `npm run postal-migration:check` and `npm run registry:build` fail until all 25 current source releases meet the corrected connector floors.
