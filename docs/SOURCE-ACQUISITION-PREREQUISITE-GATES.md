...
This increment implements an explicit policy hold for the proposed Nebraska childcare PDF workflow. It does **not** implement PDF capture, a decoder, an acquired roster or an approval UI. The [source review](states/NE-PDF-INTERNAL-USE-2026-09-09.md) identifies a concrete linked-notice scope question; unspecified rights alone are not treated as a statewide prohibition.
...
- Authenticated `POST /api/data-operations/source-prerequisites` accepts exactly `{"sourceId":"ne-childcare-pdf"}`. It returns HTTP 409 with an actionable error message. It does not reserve a managed slot, allocate a run, create a receipt, invoke a worker or request a source.
...
- No new operation kind is registered. Existing collection/export/snapshot lifecycle, cancellation and restart handling are unchanged. This gate applies to the proposed source-prerequisite workflow; it is not a claim that every legacy generic URL/job route now enforces a universal source-policy engine.

The gate is code-defined and returns a fresh catalog projection. Changing a catalog result in memory cannot approve a capture. Even a future returning assertion cannot cause the current start method to dispatch: it refuses unenrolled capture explicitly. A future approved implementation needs a reviewed source profile, bounded transport, mixed-content retention controls, decoder prerequisite, immutable source evidence and verified operation handoff before this hold can be replaced.
