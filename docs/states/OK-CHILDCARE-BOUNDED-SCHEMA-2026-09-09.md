...
This originally implemented a standalone application-code prerequisite, not a statewide collector. The later [managed operation integration](OK-MANAGED-SCHEMA-PREREQUISITE.md) adds app dispatch without expanding its source scope. It follows the [published filter contract](OK-CHILDCARE-FILTER-CONTRACT-2026-09-09.md) and [narrow source-use assessment](OK-SOURCE-USE-2026-09-09.md). No source records are committed to Git.
...
This CLI is reusable Co*Tive code and does not require an AI model. Its original run above predates [managed enrollment](OK-MANAGED-SCHEMA-PREREQUISITE.md); that standalone run ID must not be relabeled as a managed operation ID. No recurring refresh is implemented. Before handing routine statewide acquisition to the app, validate source-wide limits/coverage, address parsing, identifiers and temporal semantics. Do not run a ZIP sweep from this prerequisite. Retain unknown currentness and coverage explicitly; unknown quality does not mean zero businesses.
