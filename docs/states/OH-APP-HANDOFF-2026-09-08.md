...
## Retained Ohio production planning and matching boundary

Production planning accepts `--oh-childcare-receipt <completed app receipt>` alongside MA/NJ and exactly one fresh/recovered TN input. The fixed-native app entry is required; injected transport receipts are rejected. This mode label is wrapper provenance, not independent authentication of publisher responses. Planning and stage revalidation are read-only and perform no source requests.

The plan pins the terminal/start/acquisition receipts, acquired manifest and complete declared artifact roster, normalized manifest and artifacts, enrollment, both source policies, source-use decision, connector configuration, and the selected Ohio runtime closure. Registry build receives the receipt path, while emitted registry dependencies reconcile against the normalized release identity. Execution reconstructs the receipt selection, checks pins before/after every stage, and requires registry 2.15 and coverage 2.11 with the exact app-source descriptor. Ohio is not accepted as an addition to historical benchmark/resolution recovery.

Resolution and benchmark compatibility now includes registry 2.15. The resolver reloads source-bound Ohio evidence and verifies whole-cohort membership, handles optional TN origin and separate missing-ZIP counts, and rejects Ohio source IDs and both site/establishment endpoints in matching inputs. Four synthetic scenarios cover Ohio mixed/all-null ZIPs and fresh/recovered TN; benchmark status remains awaiting independent labels. No matching or accuracy approval follows from compatibility.

The production fixture uses mocked publisher responses at the native entry, then rejects all fetches during retained planning/execution; its eight stage outputs are synthetic controller evidence, not a live national promotion. Tests cover all four optional selections, retained/config/implementation drift, invalid receipt selections, historical recovery rejection, emitted version/lineage drift and injected-transport rejection. Existing recovery behavior remains intact. Real promotion requires a new explicitly pinned production plan and application-owned execution; do not redownload Ohio or repeat a running plan.
