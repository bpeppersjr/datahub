...
Every row from the registry's 48,142-ZIP union is retained with source counts, source temporal metadata, ZBP employer baseline, USPS-validity status, ZCTA status, and complete many-to-many jurisdiction topology when a ZCTA exists. A ZCTA relationship continues to mean polygon area only—not the distribution of people, addresses, establishments, or businesses.
...
Release `national-business-coverage-views-20260901-063928891Z-f98ea839` publishes seven independently verified artifacts totaling 443,964,694 bytes. It contains three national scopes, all 56 state equivalents, all 3,235 county equivalents, all 48,142 registry ZIP rows, 20 source views, and 27,947 explicit gap rows.
