...
Release `national-business-coverage-views-20260901-160206124Z-46dc9437` publishes seven independently verified artifacts totaling 501,285,784 bytes. It contains three national scopes, all 56 state equivalents, all 3,235 county equivalents, all 48,215 registry ZIP rows, 23 source views, and 28,106 explicit gap rows.

The publisher assessed all 7,903,112 source-preserving location profiles. Source-reported address states match a supported Census state equivalent for 7,902,721 profiles; 391 remain missing or unsupported. Exactly 1,012,507 profiles carry valid points, 972,320 have one county assignment, 40,180 fall outside every generalized county polygon, and seven sit on ambiguous county boundaries. Forty-nine profiles have a conflict between their reported state and coordinate-derived county state; both observations remain visible. DC contributes 54,890 profiles: 42,749 carry valid transformed coordinates, 42,748 receive one county assignment, one coordinate is invalid, and one valid point is outside the generalized county polygons. Alaska contributes 94,486 reported-state profiles without coordinates. NYC DCWP contributes 31,163 profiles, including 26,519 with one county assignment and four retained state conflicts. Chicago contributes 42,864 profiles, including 39,683 with one county assignment and 32 retained conflicts.

County point coverage currently comes from SNAP, FDIC, FSIS, Los Angeles, Chicago BACP, DC DLCP, and NYC DCWP profile layers. The current registry match-profile layer carries no point for Alaska, NPPES, ECHO, FMCSA, or NCUA, so their 5,919,164 point-missing profiles are not silently placed in counties. ECHO retains source-reported coordinates and precision metadata in its own governed assertion layer, but those assertions are not generalized into premise-level match-profile points.
...
The release remains local aggregate, partial, and non-deduplicated. Connecticut, Delaware, Colorado, Iowa, New York, Florida, and Pennsylvania organization addresses plus Oregon organization/brand addresses stay out of physical-site state/county counts and remain explicit in the entity-only allocation gap. Those sources contribute no location profiles. DC DLCP, Alaska, Los Angeles, Texas permit outlets, Chicago licensed sites, and NYC DCWP licensed sites contribute source-preserving profiles under local-review-only policies; Alaska and Texas contribute no coordinates, while Los Angeles, Chicago, DC, and NYC contribute source coordinates. DC-derived aggregates require CC BY 4.0 attribution; Alaska-derived aggregate distribution remains review-required. The identity gate records zero benchmark labels, entity resolution unapplied, and export authorization false.
