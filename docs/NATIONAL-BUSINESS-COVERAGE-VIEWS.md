...
Every row from the registry's 48,148-ZIP union is retained with source counts, source temporal metadata, ZBP employer baseline, USPS-validity status, ZCTA status, and complete many-to-many jurisdiction topology when a ZCTA exists. A ZCTA relationship continues to mean polygon area only—not the distribution of people, addresses, establishments, or businesses.
...
Release `national-business-coverage-views-20260901-102201272Z-f1036378` publishes seven independently verified artifacts totaling 457,220,965 bytes. It contains three national scopes, all 56 state equivalents, all 3,235 county equivalents, all 48,148 registry ZIP rows, 21 source views, and 27,963 explicit gap rows.

The publisher assessed all 7,753,736 source-preserving location profiles. Source-reported address states match a supported Census state equivalent for 7,753,345 profiles; 391 remain missing or unsupported. Exactly 969,758 profiles carry valid points, 929,572 have one county assignment, 40,179 fall outside every generalized county polygon, and seven sit on ambiguous county boundaries. Forty-nine profiles have a conflict between their reported state and coordinate-derived county state; both observations remain visible. NYC DCWP contributes 31,163 profiles, including 26,519 with one county assignment and four reported-state/coordinate-state conflicts retained as parallel evidence. Chicago contributes 42,864 profiles, including 39,683 with one county assignment and 32 retained conflicts.
...
Of the 48,148 ZIP rows, 47,945 have record-level source contributions and 203 are denominator-only. The ZIP/ZCTA join reconciles exactly to all 33,791 crosswalk ZCTAs, leaving 14,357 ZIP rows without a ZCTA polygon. Census ZBP publishes an employer baseline for 34,954 ZIP rows; 13,194 retain an explicit unpublished or missing-baseline gap. Source-reported ZIP values retain explicit ZCTA/ZBP gaps and unverified USPS status rather than being promoted to current valid postal assignments. The authoritative current USPS ZIP denominator remains unavailable, so no percentage over “all valid ZIP Codes” is published.
...
The release remains local aggregate, partial, and non-deduplicated. Connecticut, Delaware, Colorado, Iowa, New York, Florida, and Pennsylvania organization addresses plus Oregon organization/brand addresses stay out of physical-site state/county counts and remain explicit in the entity-only allocation gap. Those sources contribute no location profiles. Los Angeles, Texas permit outlets, Chicago licensed sites, and NYC DCWP licensed sites contribute source-preserving profiles under local-review-only policies; Los Angeles, Chicago, and NYC contribute source coordinates. The identity gate records zero benchmark labels, entity resolution unapplied, and export authorization false.
