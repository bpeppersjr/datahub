...
Every row from the registry's 47,009-ZIP union is retained with source counts, source temporal metadata, ZBP employer baseline, USPS-validity status, ZCTA status, and complete many-to-many jurisdiction topology when a ZCTA exists. A ZCTA relationship continues to mean polygon area only—not the distribution of people, addresses, establishments, or businesses.
...
Release `national-business-coverage-views-20260831-150759199Z-8b23bacf` publishes seven independently verified artifacts totaling 363,539,142 bytes. It contains three national scopes, all 56 state equivalents, all 3,235 county equivalents, all 47,009 registry ZIP rows, 15 source views, and 25,688 explicit gap rows.
...
Of the 47,009 ZIP rows, 46,771 have record-level source contributions and 238 are denominator-only. The ZIP/ZCTA join reconciles exactly to all 33,791 crosswalk ZCTAs, leaving 13,218 ZIP rows without a ZCTA polygon. Census ZBP publishes an employer baseline for 34,954 ZIP rows; 12,055 retain an explicit unpublished or missing-baseline gap. Florida contributes 1,603 five-digit source values not previously present in the integrated union; these retain explicit ZCTA/ZBP gaps and unverified USPS status rather than being promoted to current valid postal assignments. The authoritative current USPS ZIP denominator remains unavailable, so no percentage over “all valid ZIP Codes” is published.
