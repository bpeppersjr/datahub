...
Every row from the registry's 45,406-ZIP union is retained with source counts, source temporal metadata, ZBP employer baseline, USPS-validity status, ZCTA status, and complete many-to-many jurisdiction topology when a ZCTA exists. A ZCTA relationship continues to mean polygon area only—not the distribution of people, addresses, establishments, or businesses.
...
Release `national-business-coverage-views-20260831-094243685Z-5474d376` publishes seven independently checksummed artifacts totaling 337,663,273 bytes. It contains three national scopes, all 56 state equivalents, all 3,235 county equivalents, all 45,406 registry ZIP rows, 14 source views, and 22,485 explicit gap rows.
...
Of the 45,406 ZIP rows, 45,160 have record-level source contributions and 246 are denominator-only. The ZIP/ZCTA join reconciles exactly to all 33,791 crosswalk ZCTAs, leaving 11,615 ZIP rows without a ZCTA polygon. Census ZBP publishes an employer baseline for 34,954 ZIP rows; 10,452 retain an explicit unpublished or missing-baseline gap. The authoritative current USPS ZIP denominator remains unavailable, so no percentage over “all valid ZIP Codes” is published.
