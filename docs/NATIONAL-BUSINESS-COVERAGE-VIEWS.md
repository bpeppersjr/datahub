...
Release `national-business-coverage-views-20260901-131256359Z-b2f940d7` publishes seven independently verified artifacts totaling 483,965,310 bytes. It contains three national scopes, all 56 state equivalents, all 3,235 county equivalents, all 48,215 registry ZIP rows, 22 source views, and 28,102 explicit gap rows.

The publisher assessed all 7,848,222 source-preserving location profiles. Source-reported address states match a supported Census state equivalent for 7,847,831 profiles; 391 remain missing or unsupported. Exactly 969,758 profiles carry valid points, 929,572 have one county assignment, 40,179 fall outside every generalized county polygon, and seven sit on ambiguous county boundaries. Forty-nine profiles have a conflict between their reported state and coordinate-derived county state; both observations remain visible. Alaska contributes 94,486 reported-state profiles without coordinates. NYC DCWP contributes 31,163 profiles, including 26,519 with one county assignment and four reported-state/coordinate-state conflicts retained as parallel evidence. Chicago contributes 42,864 profiles, including 39,683 with one county assignment and 32 retained conflicts.

County point coverage currently comes from SNAP, FDIC, FSIS, Los Angeles, Chicago BACP, and NYC DCWP profile layers. The current registry match-profile layer carries no point for Alaska, NPPES, ECHO, FMCSA, or NCUA, so their 5,919,164 point-missing profiles are not silently placed in counties. ECHO retains source-reported coordinates and precision metadata in its own governed assertion layer, but those assertions are not generalized into premise-level match-profile points.

Of the 48,215 ZIP rows, 48,014 have record-level source contributions and 201 are denominator-only. The ZIP/ZCTA join reconciles exactly to all 33,791 crosswalk ZCTAs, leaving 14,424 ZIP rows without a ZCTA polygon. Census ZBP publishes an employer baseline for 34,954 ZIP rows; 13,261 retain an explicit unpublished or missing-baseline gap. Source-reported ZIP values retain explicit ZCTA/ZBP gaps and unverified USPS status rather than being promoted to current valid postal assignments. The authoritative current USPS ZIP denominator remains unavailable, so no percentage over “all valid ZIP Codes” is published.
...
The release remains local aggregate, partial, and non-deduplicated. Connecticut, Delaware, Colorado, Iowa, New York, Florida, and Pennsylvania organization addresses plus Oregon organization/brand addresses stay out of physical-site state/county counts and remain explicit in the entity-only allocation gap. Those sources contribute no location profiles. Alaska, Los Angeles, Texas permit outlets, Chicago licensed sites, and NYC DCWP licensed sites contribute source-preserving profiles under local-review-only policies; Alaska contributes no coordinates, while Los Angeles, Chicago, and NYC contribute source coordinates. Alaska-derived aggregate distribution remains review-required. The identity gate records zero benchmark labels, entity resolution unapplied, and export authorization false.
