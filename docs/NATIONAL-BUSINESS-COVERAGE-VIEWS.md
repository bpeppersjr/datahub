...
Every row from the registry's 44,585-ZIP union is retained with source counts, source temporal metadata, ZBP employer baseline, USPS-validity status, ZCTA status, and complete many-to-many jurisdiction topology when a ZCTA exists. A ZCTA relationship continues to mean polygon area only—not the distribution of people, addresses, establishments, or businesses.
...
Release `national-business-coverage-views-20260831-051812361Z-b9c75b07` publishes seven independently checksummed artifacts totaling 306,894,435 bytes. It contains three national scopes, all 56 state equivalents, all 3,235 county equivalents, all 44,585 registry ZIP rows, 12 source views, and 20,848 explicit gap rows.
...
Of the ZIP rows, 44,325 have record-level source contributions and 260 are denominator-only. The ZIP/ZCTA join reconciles exactly to all 33,791 crosswalk ZCTAs, leaving 10,794 ZIP rows without a ZCTA polygon. Census ZBP publishes an employer baseline for 34,954 ZIP rows; 9,631 retain an explicit unpublished or missing-baseline gap. The authoritative current USPS ZIP denominator remains unavailable, so no percentage over “all valid ZIP Codes” is published.
