...
Every row from the registry's 44,575-ZIP union is retained with source counts, source temporal metadata, ZBP employer baseline, USPS-validity status, ZCTA status, and complete many-to-many jurisdiction topology when a ZCTA exists. A ZCTA relationship continues to mean polygon area only—not the distribution of people, addresses, establishments, or businesses.
...
Release `national-business-coverage-views-20260831-030920613Z-ec4937ca` publishes seven independently checksummed artifacts totaling 280,460,398 bytes. It contains three national scopes, all 56 state equivalents, all 3,235 county equivalents, all 44,575 registry ZIP rows, 11 source views, and 20,823 explicit gap rows.
...
Of the ZIP rows, 44,315 have record-level source contributions and 260 are denominator-only. The ZIP/ZCTA join reconciles exactly to all 33,791 crosswalk ZCTAs, leaving 10,784 ZIP rows without a ZCTA polygon. Census ZBP publishes an employer baseline for 34,954 ZIP rows; 9,621 retain an explicit unpublished or missing-baseline gap. The authoritative current USPS ZIP denominator remains unavailable, so no percentage over “all valid ZIP Codes” is published.
...
The release remains local aggregate, partial, and non-deduplicated. Connecticut and Colorado organization addresses stay out of physical-site state/county counts and are declared in the organization-only allocation gap. The Colorado source contributes no location profiles, so the 6,161,280 physical-location profile total and every point-derived county count remain unchanged. The identity gate records zero benchmark labels, entity resolution unapplied, and export authorization false.
