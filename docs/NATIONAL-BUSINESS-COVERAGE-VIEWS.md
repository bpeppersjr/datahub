...
Every row from the registry's 48,215-ZIP union is retained with source counts, source temporal metadata, ZBP employer baseline, USPS-validity status, ZCTA status, and complete many-to-many jurisdiction topology when a ZCTA exists. A ZCTA relationship continues to mean polygon area only—not the distribution of people, addresses, establishments, or businesses.
...
The coverage publisher 2.5.0 release is aligned to national registry 2.8.0, its independently verified entity-resolution release, and the current deterministic benchmark sample. It adds Washington L&I organization and mailing-address evidence without adding physical-location profiles.

Release `national-business-coverage-views-20260902-041351389Z-7da58092` publishes seven independently verified artifacts totaling 542,727,820 bytes. It contains three national scopes, all 56 state equivalents, all 3,235 county equivalents, all 48,215 registry ZIP rows, 25 source views, and 28,117 explicit gap rows.

The publisher assessed all 7,987,597 source-preserving location profiles. Source-reported address states match a supported Census state equivalent for 7,987,206 profiles; 391 remain missing or unsupported. Exactly 1,012,507 profiles carry valid points, 972,320 have one county assignment, 40,180 fall outside every generalized county polygon, and seven sit on ambiguous county boundaries. Forty-nine profiles have a conflict between their reported state and coordinate-derived county state; both observations remain visible. California ABC contributes 84,485 reported-state profiles without coordinates. DC contributes 54,890 profiles: 42,749 carry valid transformed coordinates, 42,748 receive one county assignment, one coordinate is invalid, and one valid point is outside the generalized county polygons. Alaska contributes 94,486 reported-state profiles without coordinates. NYC DCWP contributes 31,163 profiles, including 26,519 with one county assignment and four retained state conflicts. Chicago contributes 42,864 profiles, including 39,683 with one county assignment and 32 retained conflicts.

County point coverage currently comes from SNAP, FDIC, FSIS, Los Angeles, Chicago BACP, DC DLCP, and NYC DCWP profile layers. The current registry match-profile layer carries no point for California ABC, Alaska, NPPES, ECHO, FMCSA, or NCUA, so their point-missing profiles are not silently placed in counties. ECHO retains source-reported coordinates and precision metadata in its own governed assertion layer, but those assertions are not generalized into premise-level match-profile points.

Of the 48,215 ZIP rows, 48,016 have record-level source contributions and 199 are denominator-only. The ZIP/ZCTA join reconciles exactly to all 33,791 crosswalk ZCTAs, leaving 14,424 ZIP rows without a ZCTA polygon. Census ZBP publishes an employer baseline for 34,954 ZIP rows; 13,261 retain an explicit unpublished or missing-baseline gap. Source-reported ZIP values retain explicit ZCTA/ZBP gaps and unverified USPS status rather than being promoted to current valid postal assignments. The authoritative current USPS ZIP denominator remains unavailable, so no percentage over “all valid ZIP Codes” is published.
...
The release remains local aggregate, partial, and non-deduplicated. Connecticut, Delaware, Colorado, Washington L&I, Iowa, New York, Florida, and Pennsylvania organization addresses plus Oregon organization/brand addresses stay out of physical-site state/county counts and remain explicit in the entity-only allocation gap. Those sources contribute no location profiles. California ABC, DC DLCP, Alaska, Los Angeles, Texas permit outlets, Chicago licensed sites, and NYC DCWP licensed sites contribute source-preserving profiles under local-review-only policies; California, Alaska, and Texas contribute no coordinates, while Los Angeles, Chicago, DC, and NYC contribute source coordinates. Washington-derived aggregates require Labor & Industries attribution under PDDL and retained contractor-license/mailing-address limitations; California-derived aggregates require source attribution and retained license-scope limitations, DC-derived aggregates require CC BY 4.0 attribution, and Alaska-derived aggregate distribution remains review-required. The identity gate records zero benchmark labels, entity resolution unapplied, and export authorization false.
