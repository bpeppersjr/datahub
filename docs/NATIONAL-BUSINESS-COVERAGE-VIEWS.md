...
Release `national-business-coverage-views-20260901-025753619Z-d985b2c1` publishes seven independently verified artifacts totaling 430,848,463 bytes. It contains three national scopes, all 56 state equivalents, all 3,235 county equivalents, all 48,142 registry ZIP rows, 19 source views, and 27,941 explicit gap rows.

The publisher assessed all 7,722,573 source-preserving location profiles. Source-reported address states match a supported Census state equivalent for 7,722,182 profiles; 391 remain missing or unsupported. Exactly 943,226 profiles carry valid points, 903,053 have one county assignment, 40,166 fall outside every generalized county polygon, and seven sit on ambiguous county boundaries. Forty-five profiles have a conflict between their reported state and coordinate-derived county state; both observations remain visible. Chicago contributes 42,864 profiles, including 39,683 with one county assignment and 32 reported-state/coordinate-state conflicts retained as parallel evidence.

County point coverage currently comes from SNAP, FDIC, FSIS, and Chicago BACP profile layers. The current registry match-profile layer carries no point for NPPES, ECHO, FMCSA, or NCUA, so their 5,824,678 point-missing profiles are not silently placed in counties. ECHO retains source-reported coordinates and precision metadata in its own governed assertion layer, but those assertions are not generalized into premise-level match-profile points.
