...
for (let i = 0; i < args.length; i += 1) {
  const flag = args[i];
  if (!new Set(["--out", "--wave"]).has(flag) || seen.has(flag) || !args[i + 1] || args[i + 1].startsWith("--")) throw new Error("Usage: node scripts/build-broad-organization-current-matrix-authorization-wave.mjs [--wave 1|2|3|4] [--out <data-directory>]");
  seen.add(flag);
  const value = args[++i];
  if (flag === "--out") outputRoot = path.resolve(ROOT, value);
  else if (["1", "2", "3", "4"].includes(value)) waveNumber = Number(value);
  else throw new Error("--wave must be 1, 2, 3, or 4.");
}
