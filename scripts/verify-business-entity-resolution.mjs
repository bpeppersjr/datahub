...
import { verifyBusinessEntityResolution } from "../runner/business-entity-resolution.mjs";
import { APP_ROOT, assertInsideApp } from "../runner/paths.mjs";

try {
  const requested = process.argv[2] ?? "data/business-entity-resolution/current.json";
  ...
  const manifestPath = input.manifest
    ...
    : requestedPath;
  const result = await verifyBusinessEntityResolution(manifestPath);
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
} catch (error) {
  process.stderr.write(`Business entity-resolution verification failed: ${error.message}\n`);
