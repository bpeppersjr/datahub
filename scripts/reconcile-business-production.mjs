...
function parse(args) {
  ...
  for(let index=0;index<args.length;index++){const key={'--run-id':'runId','--confirm':'confirm','--recover-benchmark-from':'recoverBenchmarkFrom','--recover-resolution-from':'recoverResolutionFrom','--ma-childcare':'maChildcare','--nj-childcare':'njChildcare'}[args[index]];if(!key||options[key]!==undefined)throw new Error('Unknown or repeated argument.');const value=args[++index];if(!value||value.startsWith('--'))throw new Error('Argument requires a value.');options[key]=value;}
...
try {
  ...
  else if(options.mode==='plan'){const plan=await planProductionReconciliation({...(options.runId?{runId:options.runId}:{}),...(options.recoverBenchmarkFrom?{recoverBenchmarkFrom:options.recoverBenchmarkFrom}:{}),...(options.recoverResolutionFrom?{recoverResolutionFrom:options.recoverResolutionFrom}:{}),...(options.maChildcare?{maChildcare:options.maChildcare}:{}),...(options.njChildcare?{njChildcare:options.njChildcare}:{})});const file=path.join(await safePlans(),`${plan.runId}.json`);await writeFile(file,`${JSON.stringify(plan,null,2)}\n`,{flag:'wx'});process.stdout.write(`${JSON.stringify({status:'PLANNED',run_id:plan.runId,plan_sha256:plan.planSha256,plan:file},null,2)}\n`);}
