...
import { verifyRetainedPlan } from '../runner/industry-retained-inputs.mjs';
...
Optional --retained-inputs-json <JSON> pins source-keyed acquisition and parent receipt paths/hashes for supported childcare sources; see docs/RETAINED-FEED-INPUTS.md.
`;
}
...
function parse(args) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    else if (flag === "--retained-inputs-json") { if(options.retainedInputs !== undefined) throw new Error('Duplicate retained inputs.'); options.retainedInputs=JSON.parse(value); }
...
try {
  ...
  const plan = buildIndustryPlan(config, { industries: options.industries, states: options.states, ...(options.sourceIds === undefined ? {} : {sourceIds:options.sourceIds}), retainedInputs: options.retainedInputs, runId: options.runId || undefined });
  if (options.mode === "plan") { await verifyRetainedPlan(plan, cancellation.signal); process.stdout.write(`${JSON.stringify(plan, null, 2)}\n`); process.exit(0); }
