...
Options:
  ...
  --cms-nursing-home-selection <path> Exact retained recovered nursing directory; no identity or current-operations inference
...
function parseArguments(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === '--cms-nursing-home-selection') {
      const value=args[++index];
      if(!value||value.startsWith('--')||options.cmsNursingHomeSelection)throw new Error('Choose one CMS nursing-home selection.');
      options.cmsNursingHomeSelection=value;continue;
    }
...
if (["--output", "--snap", "--nppes", "--fdic", "--ncua", "--fsis", "--echo", "--fmcsa", "--irs-eo", "--ct-business", "--de-business", "--ak-business", "--co-business", "--wa-lni-contractors", "--or-business", "--ia-business", "--ny-business", "--fl-business", "--pa-business", "--il-business", "--la-active-businesses", "--tx-sales-tax", "--chicago-licenses", "--dc-licenses", "--ca-abc", "--ny-retail-food", "--nyc-dcwp", "--usps-zips", "--ma-childcare", "--nj-childcare", "--tn-childcare", "--tn-fresh-childcare", "--oh-childcare-receipt"].includes(argument)) {
  ...
  if (options.tnChildcare && options.tnFreshChildcare) throw new Error("Fresh and recovered Tennessee inputs are mutually exclusive.");
  return options;
}
...
try {
  ...
  const result = await buildNationalBusinessRegistry({
    ...
    cmsNursingHomeSelection: options.cmsNursingHomeSelection ? assertInsideApp(path.resolve(APP_ROOT, options.cmsNursingHomeSelection)) : null,
