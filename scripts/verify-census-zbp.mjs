...
import { verifyCensusZbpRelease } from "../runner/census-zbp.mjs";
import { APP_ROOT, assertInsideApp } from "../runner/paths.mjs";

try {
  const requested = process.argv[2] ?? "data/business-baselines/census-zbp/current.json";
  ...
  const manifestPath = input.manifest
    ...
    : requestedPath;
  const result = await verifyCensusZbpRelease(manifestPath);
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
} catch (error) {
  process.stderr.write(`Census ZBP verification failed: ${error.message}\n`);
