...
try {
  ...
  const result = await verifyFlBusinessRegistry(manifestPath, { signal: cancellation.signal });
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
} catch (error) {
  process.stderr.write(cancellation.signal.aborted ? "Florida Business Registry verification cancelled.\n" : `Florida Business Registry verification failed: ${error.message}\n`);
