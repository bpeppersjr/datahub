...
import { verifyEpaEcho } from "../runner/epa-echo.mjs";
import { APP_ROOT, assertInsideApp } from "../runner/paths.mjs";

try {
  const requested = process.argv[2] ?? "data/business-sources/epa-echo-active-facilities/current.json";
  ...
  const manifestPath = input.manifest
    ...
    : requestedPath;
  const result = await verifyEpaEcho(manifestPath);
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
} catch (error) {
  process.stderr.write(`EPA ECHO verification failed: ${error.message}\n`);
