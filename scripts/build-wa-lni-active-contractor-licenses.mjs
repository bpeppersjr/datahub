...
Usage:
  node scripts/build-wa-lni-active-contractor-licenses.mjs [options]

Options:
  --output <path>       Output root (default: data/business-sources/wa-lni-active-contractor-organizations)
  --zbp <path>          Census ZBP current.json prerequisite
  --page-size <number>  Socrata page size (default: 25000; maximum: 50000)
  ...
  --resume-source-staging-run <UUID>
                        Revalidate and normalize a complete staged selected-field snapshot into a new run
  --help                Show this help
`;
}
...
function parseArguments(args) {
  const options = {
    output: "data/business-sources/wa-lni-active-contractor-organizations",
...
try {
  ...
  if (options.resumeSourceStagingRun && !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(options.resumeSourceStagingRun)) throw new Error("--resume-source-staging-run requires a UUID.");
