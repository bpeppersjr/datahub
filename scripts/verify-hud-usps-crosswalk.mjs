...
import { verifyHudUspsRelease } from "../runner/hud-usps-crosswalk.mjs";
import { APP_ROOT, assertInsideApp } from "../runner/paths.mjs";

try {
  const requested = process.argv[2] ?? "data/zip-validity/hud-usps/current.json";
  ...
  const manifestPath = input.manifest
    ...
    : requestedPath;
  const result = await verifyHudUspsRelease(manifestPath);
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
} catch (error) {
  process.stderr.write(`HUD-USPS verification failed: ${error.message}\n`);
