...
import { verifyNcuaQuarterly } from "../runner/ncua-quarterly.mjs";
import { APP_ROOT, assertInsideApp } from "../runner/paths.mjs";

try {
  const requested = process.argv[2] ?? "data/business-sources/ncua-quarterly-credit-unions/current.json";
  ...
  const manifestPath = input.manifest
    ...
    : requestedPath;
  const result = await verifyNcuaQuarterly(manifestPath);
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
} catch (error) {
  process.stderr.write(`NCUA quarterly verification failed: ${error.message}\n`);
