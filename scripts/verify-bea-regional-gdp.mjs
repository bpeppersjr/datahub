...
import { verifyBeaRegionalGdpRelease } from "../runner/bea-regional-gdp.mjs";
import { APP_ROOT, assertInsideApp } from "../runner/paths.mjs";

try {
  const requested = process.argv[2] ?? "data/business-baselines/bea-regional-gdp/current.json";
  ...
  const manifestPath = input.manifest
    ...
    : requestedPath;
  const result = await verifyBeaRegionalGdpRelease(manifestPath);
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
} catch (error) {
  process.stderr.write(`BEA regional GDP verification failed: ${error.message}\n`);
